...
function f3070_HTMLComboV2_saiu70numpaso($objDB, $objCombos, $valor, $vrsaiu70idtipotramite)
{
	...
	require $mensajes_todas;
	$iPasos = 1;
	$objCombos->nuevo('saiu70numpaso', $valor, true, '{' . $ETI['msg_seleccione'] . '}');
	//$objCombos->iAncho=450;
...
function f3070_HTMLComboV2_saiu70idcentro($objDB, $objCombos, $valor, $vrsaiu70idzona)
{
	...
	if ((int)$vrsaiu70idzona != 0) {
		$sSQL = 'SELECT unad24id AS id, unad24nombre AS nombre FROM unad24sede WHERE unad24idzona=' . $vrsaiu70idzona . '';
	}
	$res = $objCombos->html($sSQL, $objDB);
	return $res;
}
...
function f3070_HTMLComboV2_saiu70idprograma($objDB, $objCombos, $valor, $vrsaiu70idescuela)
{
	...
	if ((int)$vrsaiu70idescuela != 0) {
		$sSQL = 'SELECT core09id AS id, core09nombre AS nombre FROM core09programa WHERE core10idprograma=' . $vrsaiu70idescuela . '';
	}
	$res = $objCombos->html($sSQL, $objDB);
	return $res;
}
...
function f3070_HTMLComboV2_saiu70idresponsable($objDB, $objCombos, $valor, $vrsaiu70idgrupotrabajo)
{
	...
	if ((int)$vrsaiu70idgrupotrabajo != 0) {
		$sSQL = 'SELECT unad11id AS id, unad11razonsocial AS nombre FROM unad11terceros WHERE unad11id=' . $vrsaiu70idgrupotrabajo . '';
	}
	$res = $objCombos->html($sSQL, $objDB);
	return $res;
}
...
function f3070_Combosaiu70idprograma($aParametros)
{
	...
	//$objResponse->call('$("#saiu70idprograma").chosen()');
	return $objResponse;
}
function f3070_Combosaiu70idgrupotrabajo($aParametros)
{
	...
	//$objResponse->call('$("#saiu70idgrupotrabajo").chosen()');
	return $objResponse;
}
