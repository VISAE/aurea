...
function cadena_tildes($origen,$butf8=false){
	$nuevo=$origen;
	$sH=array('á','é','í','ó','ú', 'è','ì','ò','ñ','Ñ', 
	'Á','É','Í','Ó','Ú', '¿','¬','°','Â','Ç', 
	'©','¡','ª','­','–', '™','ê','ã','ç','â',
	'õ','“','”', '´', 'ü', 'Ü', '∩', 'π', '^');
	$sT=array('&aacute;','&eacute;','&iacute;','&oacute;','&uacute;', '&egrave;','&igrave;','&ograve;','&ntilde;','&Ntilde;', 
	'&Aacute;','&Eacute;','&Iacute;','&Oacute;','&Uacute;', '&iquest;','&not;','&deg;','&Acirc;','&Ccedil;', 
	'&copy;','&iexcl;','&ordf;','&shy;','&ndash;', '&trade;','&ecirc;','&atilde;','&ccedil;','&acirc;',
	'&otilde;','&ldquo;','&rdquo;','&acute;', '&uuml;', '&Uuml;', '&cap;', '&pi;', '&#94;');
	$iTotal=38;
...
\u00f1 = ñ
\u00d1 = Ñ 	*/
	$sH=array('\u00e1','\u00e9','\u00ed','\u00f3','\u00fa', '&egrave;','&igrave;','&ograve;','\u00f1','\u00d1', 
...
function cadena_Validar($semilla, $bTolerante = false){
	$permitidos='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890.,;!=+_ñáéíóúüÑÁÉÍÓÚÜ ';
	if ($bTolerante) {
			...
			if (substr_count($email,".")>= 1){
				...
				if (strlen($term_dom)>1 && strlen($term_dom)<6 && (!strstr($term_dom,"@")) ){
					...
					if ($caracter_ult != "@" && $caracter_ult != "."){
						...
						if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Validar correo <b>'.$email.'</b> No pasa el quinto control<br>';}
						}
					}else{
					if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Validar correo <b>'.$email.'</b> No pasa el cuarto control ['.$term_dom.']<br>';}
					}
				}else{
				if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Validar correo <b>'.$email.'</b> No pasa el tercer control<br>';}
				}
			}else{
			if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Validar correo <b>'.$email.'</b> No pasa el segundo control<br>';}
			}
		}else{
		if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Validar correo <b>'.$email.'</b> No pasa el primer control<br>';}
		}
...
function fecha_ArmarNumero($idia='', $imes='', $iagno='', $bEstricto=false){
	...
	if ($iagno!=0){
		if ($iagno>3000){$iagno=0;}
		if ($iagno<1){$iagno=0;}
		}
	if (!$bEstricto){
		if ($idia==0){$idia=date('d');}
		if ($imes==0){$imes=date('m');}
		if ($iagno==0){$iagno=date('Y');}
		}
...
function fecha_microtiempo($iFormato=0, $bMostrarMemoria=false){
	...
	if ($iFormato==0){
		...
		$sMemoria='';
		if ($bMostrarMemoria){$sMemoria=' ['.$iMemoria.' '.$sUnidadMemoria.'] ';}
...
<input id="'.$sNomCampoMin.'" name="'.$sNomCampoMin.'" type="hidden" value="'.$iMin.'"/>';
		if (((int)$iHora+(int)$iMin)==0){$iFormato=0;}
		if ($iFormato==1){
			$sVC='AM';
			if ($iHora>11){
				$sVN=$iHora-12;
...
<input id="'.$sNomCampoHora.'" name="'.$sNomCampoHora.'" type="hidden" value="'.$iHora.'"/>';
		...
		$sValorMin=$iMin;
		if (((int)$iHora+(int)$iMin)==0){$sValorMin='';}
...
function html_MenuGrupoV3($grupo, $idsistema, $iPiel, $objDB, $completo=true, $bDebug=false, $idTercero=0){
	...
	*/
	...
	if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Consulta id modulos grupo '.$grupo.' App '.$idsistema.' '.$sSQL.'<br>';}
	...
	ORDER BY T9.unad09orden';
	if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Modulos a mostrar grupo '.$grupo.' App '.$idsistema.' '.$sSQL.'<br>';}
	$resultm=$objDB->ejecutasql($sSQL);
...
function seg_auditar($idmodulo, $idtercero, $idaccion, $idregistro, $sdetalle, $objDB, $bDebug=false){
	...
	if ($objDB->dbmodelo=='M'){
		...
		$result=$objDB->ejecutasql($sSQL);
		if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' <b>AUDITORIA</b>: '.$sSQL.'';}
		}else{
		if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' <b>AUDITORIA</b>: Modelo de base de datos no reconocido ['.$objDB->dbmodelo.']';}
		}
	return array($res, $sDebug);
	}
...
function tabla_crear($idTabla, $sRef1, $sRef2, $objDB){
	...
	switch ($idTabla){
		...
		if ($result==false){
			...
			$result=$objDB->ejecutasql($sSQL);
			$sSQL='ALTER TABLE '.$sTabla.' ADD INDEX unad71sesion_host(unad71hostname)';
			$result=$objDB->ejecutasql($sSQL);
			}
		}
	return $bCrea;
