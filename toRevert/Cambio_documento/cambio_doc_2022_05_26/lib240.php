...
function f240_TablaDetalleV2($aParametros, $objDB, $bDebug = false) {
	...
	*/
	...
	while($filadet = $objDB->sf($tabladetalle)) {
		...
		$res = $res . '<tr' . $sClass . '>
		<td>' . $et_unae24idtercero_doc . '</td>
		<td>' . $et_unae24idtercero_nombre . '</td>
		...
		<td>' . $et_unae24idsolicita_doc . '</td>
		<td>' . $et_unae24idsolicita_nombre . '</td>
		...
		<td>' . $et_unae24idaprueba_doc . '</td>
		<td>' . $et_unae24idaprueba_nombre . '</td>
...
function f240_db_GuardarV2($DATA, $objDB, $bDebug = false, $idTercero = 0) {
	...
	*/
	...
	switch ($DATA['unae24estado']) {
		case 7:
		if ($DATA['unae24fechaapr'] == 0) {
			//$DATA['unae24fechaapr'] = fecha_DiaMod();
			$sError = $ERR['unae24fechaapr'] . $sSepara.$sError;
			}
		if ($DATA['unae24idaprueba'] == 0) {
			$sError = $ERR['unae24idaprueba'] . $sSepara.$sError;
		}
		//if ($DATA['unae24detalle'] == '') {
			//$sError = $ERR['unae24detalle'] . $sSepara.$sError;
		}
	...
	if ($sError == '') {
		if ($DATA['paso'] == 10) {
			...
			$DATA['unae24tipodocorigen'] = '';
			$DATA['unae24docorigen'] = '';
			//$DATA['unae24idsolicita'] = 0; //$_SESSION['u_idtercero'];
			$DATA['unae24horasol'] = 0;
			$DATA['unae24minsol'] = 0;
			$DATA['unae24idorigen'] = 0;
