...
if (!$bPeticionXAJAX) {
	if (noticias_pendientes($objDB)) {
		$objDB->CerrarConexion();
		header('Location:noticia.php?ret=bitaequipotrabajo.php');
		die();
	}
}
...
if ((int)$_REQUEST['paso'] > 0) {
	...
	if (isset($_REQUEST['bita28idtercero']) == 0) {
		...
		//$_REQUEST['bita28idtercero'] =  $idTercero;
	}
	...
	if (isset($_REQUEST['bita28fechaingreso']) == 0) {
		...
		//$_REQUEST['bita28fechaingreso'] = $iHoy;
	}
	if (isset($_REQUEST['bita28fechasalida']) == 0) {
		$_REQUEST['bita28fechasalida'] = '';
		//$_REQUEST['bita28fechasalida'] = $iHoy;
	}
}
...
$objCombos->nuevo('bita27activo', $_REQUEST['bita27activo'], true, $ETI['no'], 0);
$objCombos->addItem(1, $ETI['si']);
//$objCombos->addArreglo($abita27activo, $ibita27activo);
$html_bita27activo = $objCombos->html('', $objDB);
if ((int)$_REQUEST['paso'] == 0) {
} else {
...
//FORMA
require $APP->rutacomun . 'unad_forma_v2.php';
forma_cabeceraV3($xajax, $ETI['titulo_1527']);
echo $et_menu;
forma_mitad();
if (false) {
?>
...
<input id="titulo_1527" name="titulo_1527" type="hidden" value="<?php echo $ETI['titulo_1527']; ?>" />
<div class="titulos">
<div class="titulosD">
