...
if ($bEntra){
	...
	$saiu23unidad=$_REQUEST['v5'];
	$saiu23tema=$_REQUEST['v6'];
	$saiu23documento=$_REQUEST['v7'];
	if ($sError==''){
		...
		//Titulo 
		$objHoja->setCellValueByColumnAndRow(0, 10, 'Totales');
		...
		*/
		...
		$iFila++;
		$sCondi='';
		if ($saiu23agno!=''){$sCondi=$sCondi.' AND saiu23agno='.$saiu23agno.'';};
		if ($saiu23mes!=''){$sCondi=$sCondi.' AND saiu23mes='.$saiu23mes.'';};
		if ($saiu23unidad!=''){$sCondi=$sCondi.' AND saiu23unidad='.$saiu23unidad.'';};
		if ($saiu23tema!=''){$sCondi=$sCondi.' AND saiu23tema='.$saiu23tema.'';};
		if ($saiu23documento!=''){$sCondi=$sCondi.' AND saiu23documento='.$saiu23documento.'';};
		$sSQL='SELECT * 
...
WHERE ';
		...
		while ($fila=$objDB->sf($tabla)){
			$objHoja->setCellValueByColumnAndRow(0, $iFila, $fila['saiu23agno']);
			$objHoja->setCellValueByColumnAndRow(1, $iFila, $fila['saiu23mes']);
			$objHoja->setCellValueByColumnAndRow(2, $iFila, $fila['saiu23unidad']);
			$objHoja->setCellValueByColumnAndRow(3, $iFila, $fila['saiu23tema']);
			$objHoja->setCellValueByColumnAndRow(4, $iFila, $fila['saiu23documento']);
			$iFila++;
			}
