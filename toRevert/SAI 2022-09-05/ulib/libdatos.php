...
function f107_VerificarPerfiles($idTercero, $idPeriodo, $objDB, $bDebug = false)
{
	...
	if (true) {
		...
		if ($objDB->nf($tperfiles) > 0) {
			...
			$aAplican = array(
				0, $fP['ofer01perfiladmin'], $fP['ofer01perfilcoordinador'], $fP['ofer01perfildecano'],
				$fP['ofer01perfilrevisor'], $fP['ofer01perfilacreditador'], 0, 0, $fP['ofer01perfildirector']
			);
		} else {
...
function f146_FechaInicial($idPeriodo, $objDB, $bDebug = false, $bForzar = false)
{
	...
	if ($objDB->nf($tabla) > 0) {
		...
		if ($bIgualar) {
			...
			if ($bDebug) {
				$sDebug = $sDebug . ' <b>Fecha Inicial</b> Consultando historicos: ' . $sSQL . '<br>';
			}
			$tabla = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tabla) > 0) {
				$fila = $objDB->sf($tabla);
...
function f190_AddIpV2($sIp, $objDB, $id90 = 0, $bDebug = false, $bConUbicacion = false)
{
	...
	if ($objDB->nf($tabla90) == 0) {
		...
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' Insertando ' . $sql . '<br>';
		}
		$bRes = true;
	}
	return array($bRes, $sDebug);
...
function f226_ConsultaCombo($sWhere = '', $objDB = NULL)
{
	...
	$sSQL = 'SELECT unae26id AS id, CONCAT(unae26prefijo, unae26nombre) AS nombre FROM unae26unidadesfun WHERE unae26idzona=0 AND ' . $sWhere . ' unae26id>0 ORDER BY unae26activa DESC, unae26lugar, unae26orden, unae26nombre';
	return $sSQL;
}
	...
	if ($objDB->nf($tabla) > 0) {
		...
		if ($iRes != 0) {
			...
			if ($bIniciarContenedor) {
				//core03notahomologa Decimal(15,2) NULL DEFAULT 0, core03fechanotahomologa int NULL DEFAULT 0, 
			...
			if ($bIniciarContenedor) {
				...
				core04idcupoblear int NULL DEFAULT 0, core04res75campus Decimal(2,1) NULL DEFAULT 0)";
				$bResultado = $objDB->ejecutasql($sSQL);
...
function f2205th_DirectorDesdeTH($idPeriodo, $idCurso, $objDB, $bDebug = false)
{
	...
	if ($bDebug) {
		$sDebug = $sDebug . fecha_microtiempo() . 'Director desde TH: ' . $idDirector . ' <br>';
	}
...
function f2211_ResumenCreditosV2($core10id, $objDB)
{
	...
	$sSQL = 'SELECT core10estado FROM core10programaversion WHERE core10id=' . $core10id . '';
	$tabla = $objDB->ejecutasql($sSQL);
	if ($objDB->nf($tabla) > 0) {
		$fila = $objDB->sf($tabla);
	...
	if ($objDB->nf($tabla) > 0) {
		$fila = $objDB->sf($tabla);
		$sCondiLineaProf = ' AND core11idlineaprof IN (0, ' . $fila['core11idlineaprof'] . ')';
	}
	$sSQL = 'SELECT core11tiporegistro, SUM(core11numcreditos) AS Creditos, COUNT(core11id) AS Cursos 
	FROM core11plandeestudio 
	WHERE core11idversionprograma=' . $core10id . $sCondiLineaProf . $sCondiAprobados . ' 
...
function f2211_TituloCursoEnPlan($idCurso, $id10, $objDB)
{
	$sRes = '&nbsp;';
	if ((int)$id10 != 0) {
...
function f3000_TablasMes($iAgno, $iMes, $objDB, $bDebug = false)
{
	...
	if ($bIniciarContenedor) {
		...
		saiu05evalresolvio int NULL DEFAULT 0, saiu05evalsugerencias Text NULL, saiu05idcategoria int NULL DEFAULT 0, 
...
function TraerDBRyCV2($bDebug = false)
{
	...
	if (isset($APP->dbhostryc) != 0) {
		...
		if (!$objRyC->Conectar()) {
			...
			if ($bDebug) {
				$sDebug = $sDebug . '' . fecha_microtiempo() . ' Conectado con la base de datos ' . $APP->dbnameryc . ' [' . $sRutaIni . ']<br>';
			}
		}
	} else {
		if ($bDebug) {
