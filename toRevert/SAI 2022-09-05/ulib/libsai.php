...
function f3000_InfoContacto($idTercero, $idUsuario, $objDB, $bDebug = false)
{
	...
	if ($idTercero > 0) {
		...
		unad11idtablero, unad11necesidadesp, unad11carariesgo 
		...
		if ($objDB->nf($tabla) > 0) {
			...
			if ($bPuedeConsultar) {
				$sPersonal = $ETI['unad11telefono'] . ' <b>' . $fila['unad11telefono'] . '</b>';
				if (correo_VerificarDireccion($fila['unad11correo'])) {
					$sPersonal = $sPersonal . '<br>' . $ETI['unad11correo'] . ' <b>' . $fila['unad11correo'] . '</b>';
				}
			} else {
