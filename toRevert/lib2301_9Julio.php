...
function f2301_IniciarEncuesta($idTercero, $idPeraca, $objDB, $bDebug=false, $bForzarNueva=false){
	...
	if ($objDB->nf($tabla)==0){
		...
		if ($sError==''){
			...
			$sSQL='SELECT core16idconsejero FROM core16actamatricula WHERE core16tercero='.$idTercero.' AND core16peraca='.$idPeraca.'';
			$tabla=$objDB->ejecutasql($sSQL);
			if ($objDB->nf($tabla)>0){
				$fila=$objDB->sf($tabla);
...
cara01idescuela, cara01tipocaracterizacion, cara01idconsejero, cara01discversion';
