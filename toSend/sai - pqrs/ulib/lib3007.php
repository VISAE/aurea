...
function f3007_db_Guardar($valores, $objDB, $bDebug=false){
	...
	if ($saiu07hora==''){$saiu07hora=fecha_hora();}
	if ($saiu07minuto==''){$saiu07minuto=fecha_minuto();}
...
*/
	...
	*/
	...
	while($filadet=$objDB->sf($tabladetalle)){
		...
		if ($bConRevision && $filadet['saiu07idarchivo']!=0){
		if ($filadet['saiu07idvalidad']==0){
...
function f3007_AprobarDocumento($aParametros){
	...
	$saiu07id=$aParametros[3];
	$iAgno=$aParametros[97];
	$iMes=$aParametros[98];
	$iHoy=fecha_DiaMod();
	$iHora=fecha_hora();
	...
	if ($idProceso==0){
		$sSQL='UPDATE '.$sTabla07.' SET saiu07idvalidad=0 ,saiu07fechavalida=0 ,saiu07horavalida=0 ,saiu07minvalida=0, saiu07estado=1 WHERE saiu07id='.$saiu07id.'';
		}else{
		$sSQL='UPDATE '.$sTabla07.' SET saiu07idvalidad='.$_SESSION['unad_id_tercero'].' ,saiu07fechavalida='.$iHoy.' ,saiu07horavalida='.$iHora.' ,saiu07minvalida='.$iMinuto.' ,saiu07estado=3 WHERE saiu07id='.$saiu07id.'';
		}
	$result=$objDB->ejecutasql($sSQL);
