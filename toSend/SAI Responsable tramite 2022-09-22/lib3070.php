...
// -----------------------------------
function f3070_Decideresponsable($aParametros)
{
	$_SESSION['u_ultimominuto'] = iminutoavance();
	if (!is_array($aParametros)) {
		$aParametros = json_decode(str_replace('\"', '"', $aParametros), true);
	...
	$saiu70idzona = $aParametros[0];
	$saiu70idcentro = $aParametros[1];
	$saiu70idescuela = $aParametros[2];
	$saiu70idprograma = $aParametros[3];
	$sError = '';
	$sSQL = 'SELECT *
	FROM saiu70responsabletrami AS T
	WHERE (T.saiu70idzona = 0 OR T.saiu70idzona = ' . $saiu70idzona . ')
	AND (T.saiu70idcentro = 0 OR T.saiu70idcentro = ' . $saiu70idcentro . ')
	AND (T.saiu70idescuela = 0 OR T.saiu70idescuela = ' . $saiu70idescuela . ')
	AND (T.saiu70idprograma = 0 OR T.saiu70idprograma = ' . $saiu70idprograma . ')
	ORDER BY T.saiu70idzona DESC, T.saiu70idcentro DESC, T.saiu70idescuela DESC, T.saiu70idprograma DESC' . '';
	$tabla = $objDB->ejecutasql($sSQL);
	if ($objDB->nf($tabla) > 0) {
		$filabase = $objDB->sf($tabla);
	} else {
		$sError = 'No se encuentra el registro solicitado {Zona: ' . $saiu70idzona . ' Centro: ' . $saiu70idcentro . ' Escuela: ' . $saiu70idescuela . ' Programa: ' . $saiu70idprograma . '}';
	}
	if ($sError != '') {
