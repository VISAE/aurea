...
$sTitulos='Nombre, Lugar, Inifecha, Inihora, Iniminuto, Peraca, Zona, Cead';
...
'.html_lpp('lppf1902', $lineastabla, 'paginarf1902()').'
	...
	while($filadet=$objDB->sf($tabladetalle)){
		...
		if ($filadet['even02insfechaini']!='00/00/0000'){$et_even02insfecaini=$filadet['even02insfechaini'];}
...
function f1902_db_GuardarV2($DATA, $objDB, $bDebug=false){
	...
	*/
	...
	if (true){
		...
		if ($DATA['even02inifecha']>$DATA['even02finfecha']){$sError=$ERR['errfechaini'].$sSepara.$sError;}
		if ($DATA['even02insfechaini']>$DATA['even02insfechafin']){$sError=$ERR['errinsfechaini'].$sSepara.$sError;}
