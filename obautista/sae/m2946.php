...
switch ($datos[0]) {
	case '2946': /* visa46grupotipologia */
	if (isset($datos[1]) == 0) {
		$datos[1] = '';
	}
	if (isset($datos[2]) == 0) {
	...
	if ($bDebug) {echo 'Enviando visa46grupotipologia a ' . $idEntidad . ' ' . $sIdTercero . ' ' . $sIdMovil . ' ' . $sListaIds . '<br>';
	}
	...
	if (($sError == '') && ($sListaIds!=-99)) {
		...
		if ($sIds == '') {
			...
			$sCondicion = ' WHERE visa46id IN (' . $sIds . ')';
		}
	}
	if ($sError == '') {
		$data1 = array();
		...
		$sSQL = 'SELECT visa46consec, visa46id, visa46nombre FROM visa46grupotipologia' . $sCondicion;
		if ($bDebug) {
		...
		while ($fila = $objDB->sf($tabla)) {
			$data1[] = $fila;
			$sIdsPadre = $sIdsPadre . ', ' . $fila['visa46id'];
			$iTotal++;
		}
		$res = array();
		$res[] = $iTotal;
