...
*/
/** Archivo visaepersemanal.php.
 * Modulo 2956 visa56persemanal.
...
if (!$bPeticionXAJAX) {
	if (noticias_pendientes($objDB)) {
		$objDB->CerrarConexion();
		header('Location:noticia.php?ret=visaepersemanal.php');
		die();
	}
}
$seg_1707 = 0;
...
$xajax->register(XAJAX_FUNCTION, 'f2956_HtmlTabla');
$xajax->register(XAJAX_FUNCTION, 'f2956_ExisteDato');
$xajax->processRequest();
if ($bPeticionXAJAX) {
...
if (isset($_REQUEST['paginaf2956']) == 0) {
	$_REQUEST['paginaf2956'] = 1;
}
if (isset($_REQUEST['lppf2956']) == 0) {
	$_REQUEST['lppf2956'] = 20;
}
if (isset($_REQUEST['boculta2956']) == 0) {
	$_REQUEST['boculta2956'] = 0;
}
// -- Inicializar variables de datos.
if (isset($_REQUEST['visa56consec']) == 0) {
	$_REQUEST['visa56consec'] = '';
}
if (isset($_REQUEST['visa56consec_nuevo']) == 0) {
	$_REQUEST['visa56consec_nuevo'] = '';
}
if (isset($_REQUEST['visa56id']) == 0) {
	$_REQUEST['visa56id'] = '';
}
if (isset($_REQUEST['visa56fechaini']) == 0) {
	$_REQUEST['visa56fechaini'] = '';
	//$_REQUEST['visa56fechaini'] = $iHoy;
}
if (isset($_REQUEST['visa56fechafin']) == 0) {
	$_REQUEST['visa56fechafin'] = '';
	//$_REQUEST['visa56fechafin'] = $iHoy;
}
if (isset($_REQUEST['visa56estado']) == 0) {
	$_REQUEST['visa56estado'] = 0;
}
if (isset($_REQUEST['visa56fechacrea']) == 0) {
	$_REQUEST['visa56fechacrea'] = '';
	//$_REQUEST['visa56fechacrea'] = $iHoy;
}
if (isset($_REQUEST['visa56fechacierre']) == 0) {
	$_REQUEST['visa56fechacierre'] = '';
	//$_REQUEST['visa56fechacierre'] = $iHoy;
}
if (isset($_REQUEST['visa56observacion']) == 0) {
	$_REQUEST['visa56observacion'] = '';
}
if (isset($_REQUEST['visa56activo']) == 0) {
	$_REQUEST['visa56activo'] = 0;
}
$_REQUEST['visa56consec'] = numeros_validar($_REQUEST['visa56consec']);
$_REQUEST['visa56id'] = numeros_validar($_REQUEST['visa56id']);
$_REQUEST['visa56fechaini'] = numeros_validar($_REQUEST['visa56fechaini']);
$_REQUEST['visa56fechafin'] = numeros_validar($_REQUEST['visa56fechafin']);
$_REQUEST['visa56estado'] = numeros_validar($_REQUEST['visa56estado']);
$_REQUEST['visa56fechacrea'] = numeros_validar($_REQUEST['visa56fechacrea']);
$_REQUEST['visa56fechacierre'] = numeros_validar($_REQUEST['visa56fechacierre']);
$_REQUEST['visa56observacion'] = cadena_Validar($_REQUEST['visa56observacion']);
$_REQUEST['visa56activo'] = numeros_validar($_REQUEST['visa56activo']);
...
if (($_REQUEST['paso'] == 1) || ($_REQUEST['paso'] == 3)) {
	if ($_REQUEST['paso'] == 1) {
		$sSQLcondi = 'visa56consec=' . $_REQUEST['visa56consec'] . '';
	} else {
		$sSQLcondi = 'visa56id=' . $_REQUEST['visa56id'] . '';
	}
	$sNomTabla2956 = f2956_NombreTabla();
	$sSQL = 'SELECT * FROM ' . $sNomTabla2956 . ' WHERE ' . $sSQLcondi;
	$tabla = $objDB->ejecutasql($sSQL);
	if ($objDB->nf($tabla) > 0) {
		$fila = $objDB->sf($tabla);
		$_REQUEST['visa56consec'] = $fila['visa56consec'];
		$_REQUEST['visa56id'] = $fila['visa56id'];
		$_REQUEST['visa56fechaini'] = $fila['visa56fechaini'];
		$_REQUEST['visa56fechafin'] = $fila['visa56fechafin'];
		$_REQUEST['visa56estado'] = $fila['visa56estado'];
		$_REQUEST['visa56fechacrea'] = $fila['visa56fechacrea'];
		$_REQUEST['visa56fechacierre'] = $fila['visa56fechacierre'];
		$_REQUEST['visa56observacion'] = $fila['visa56observacion'];
		$_REQUEST['visa56activo'] = $fila['visa56activo'];
		$bcargo = true;
		$_REQUEST['paso'] = 2;
		$_REQUEST['boculta2956'] = 0;
		$bLimpiaHijos = true;
...
if (($_REQUEST['paso'] == 10) || ($_REQUEST['paso'] == 12)) {
	$bMueveScroll = true;
	list($_REQUEST, $sError, $iTipoError, $sDebugGuardar) = f2956_db_GuardarV2b($_REQUEST, $objDB, $bDebug, $idTercero);
...
if ($_REQUEST['paso'] == 93) {
	$_REQUEST['paso'] = 2;
	$_REQUEST['visa56consec_nuevo'] = numeros_validar($_REQUEST['visa56consec_nuevo']);
	if ($_REQUEST['visa56consec_nuevo'] == '') {
		$sError = $ERR['visa56consec'];
	}
	if ($sError == '') {
	...
	if ($sError == '') {
		...
		$sSQL = 'SELECT visa56id FROM visa56persemanal WHERE visa56consec=' . $_REQUEST['visa56consec_nuevo'] . '';
		$tabla = $objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla) > 0) {
			$sError = 'El consecutivo ' . $_REQUEST['visa56consec_nuevo'] . ' ya existe';
		}
	}
	if ($sError == '') {
		//Aplicar el cambio.
		$sSQL = 'UPDATE visa56persemanal SET visa56consec=' . $_REQUEST['visa56consec_nuevo'] . ' WHERE visa56id=' . $_REQUEST['visa56id'] . '';
		$tabla = $objDB->ejecutasql($sSQL);
		$sDetalle = 'Cambia el consecutivo de ' . $_REQUEST['visa56consec'] . ' a ' . $_REQUEST['visa56consec_nuevo'] . '';
		$_REQUEST['visa56consec'] = $_REQUEST['visa56consec_nuevo'];
		$_REQUEST['visa56consec_nuevo'] = '';
		seg_auditar($iCodModulo, $_SESSION['unad_id_tercero'], 8, $_REQUEST['visa56id'], $sDetalle, $objDB);
...
if ($_REQUEST['paso'] == 13) {
	$_REQUEST['paso'] = 2;
	if ($sError == '') {
		list($sError, $iTipoError, $sDebugElimina) = f2956_db_Eliminar($_REQUEST['visa56id'], $objDB, $bDebug);
...
if ($_REQUEST['paso'] == -1) {
	...
	$_REQUEST['visa56fechaini'] = $iHoy;
	$_REQUEST['visa56fechafin'] = $iHoy;
	$_REQUEST['visa56estado'] = 1;
	$_REQUEST['visa56fechacrea'] = $iHoy;
	$_REQUEST['visa56fechacierre'] = $iHoy;
	$_REQUEST['visa56observacion'] = '';
	$_REQUEST['visa56activo'] = 1;
	$_REQUEST['paso'] = 0;
}
if ($bLimpiaHijos) {
}
...
$objCombos->nuevo('visa56estado', $_REQUEST['visa56estado'], true, $ETI['no'], 0);
//$objCombos->bEsCombobox = true;
$objCombos->addItem(1, $ETI['si']);
//$objCombos->addArreglo($avisa56estado, $ivisa56estado);
$sSQL = '';
$html_visa56estado = $objCombos->html($sSQL, $objDB);
$objCombos->nuevo('visa56activo', $_REQUEST['visa56activo'], true, $ETI['no'], 0);
//$objCombos->bEsCombobox = true;
$objCombos->addItem(1, $ETI['si']);
//$objCombos->addArreglo($avisa56activo, $ivisa56activo);
$sSQL = '';
$html_visa56activo = $objCombos->html($sSQL, $objDB);
if ((int)$_REQUEST['paso'] == 0) {
} else {
}
...
$objCombos->sAccion = 'paginarf2956()';
$sSQL = '';
$html_blistar = $objCombos->html($sSQL, $objDB);
//$html_blistar = $objCombos->comboSistema(2956, 1, $objDB, 'paginarf2956()');
*/
if (false) {
...
$aParametros[0] = ''; //$_REQUEST['p1_2956'];
$aParametros[100] = $idTercero;
$aParametros[101] = $_REQUEST['paginaf2956'];
$aParametros[102] = $_REQUEST['lppf2956'];
//$aParametros[103] = $_REQUEST['bnombre'];
//$aParametros[104] = $_REQUEST['blistar'];
list($sTabla2956, $sDebugTabla) = f2956_TablaDetalleV2($aParametros, $objDB, $bDebug);
...
?>
	...
	function imprimelista() {
		if (window.document.frmedita.seg_6.value == 1) {
			window.document.frmlista.consulta.value = window.document.frmedita.consulta_2956.value;
			window.document.frmlista.titulos.value = window.document.frmedita.titulos_2956.value;
			window.document.frmlista.nombrearchivo.value = 'Periodos semanales';
	...
	function imprimeexcel() {
		...
		if (sError == '') {
			asignarvariables();
			window.document.frmimpp.action = 'e2956_ss.php';
	...
	function imprimep() {
		if (window.document.frmedita.seg_5.value == 1) {
			asignarvariables();
			window.document.frmimpp.action = 'p2956.php';
			window.document.frmimpp.submit();
		} else {
	...
	function RevisaLlave() {
		let datos = new Array();
		datos[1] = window.document.frmedita.visa56consec.value;
		if ((datos[1] != '')) {
			xajax_f2956_ExisteDato(datos);
		}
	}

	function cargadato(llave1) {
		window.document.frmedita.visa56consec.value = String(llave1);
		window.document.frmedita.paso.value = 1;
		window.document.frmedita.submit();
	}

	function cargaridf2956(llave1) {
		window.document.frmedita.visa56id.value = String(llave1);
		window.document.frmedita.paso.value = 3;
		window.document.frmedita.submit();
	}

	function paginarf2956() {
		let params = new Array();
		params[99] = window.document.frmedita.debug.value;
		params[100] = <?php echo $idTercero; ?>;
		params[101] = window.document.frmedita.paginaf2956.value;
		params[102] = window.document.frmedita.lppf2956.value;
		//params[103] = window.document.frmedita.bnombre.value;
		//params[104] = window.document.frmedita.blistar.value;
		document.getElementById('div_f2956detalle').innerHTML = '<div class="GrupoCamposAyuda"><div class="MarquesinaMedia"><?php echo $ETI['msg_procesando_datos']; ?></div></div><input id="paginaf2956" name="paginaf2956" type="hidden" value="' + params[101] + '" /><input id="lppf2956" name="lppf2956" type="hidden" value="' + params[102] + '" />';
		xajax_f2956_HtmlTabla(params);
	}
	...
	function objinicial() {
		document.getElementById("visa56consec").focus();
	}
...
?>
<form id="frmimpp" name="frmimpp" method="post" action="p2956.php" target="_blank" style="display:none">
<input id="r" name="r" type="hidden" value="2956" />
<input id="id2956" name="id2956" type="hidden" value="<?php echo $_REQUEST['visa56id']; ?>" />
...
<?php
echo $objForma->htmlExpande(2956, $_REQUEST['boculta2956'], $ETI['bt_mostrar'], $ETI['bt_ocultar']);
$sEstiloDiv = '';
if ($_REQUEST['boculta2956'] != 0) {
	$sEstiloDiv = ' style="display:none;"';
}
?>
</div>
<div id="div_p2956"<?php echo $sEstiloDiv; ?>>
<?php
}
...
if ($_REQUEST['paso'] != 2) {
?>
<input id="visa56consec" name="visa56consec" type="text" value="<?php echo $_REQUEST['visa56consec']; ?>" onchange="RevisaLlave()" class="cuatro" />
<?php
} else {
	echo html_oculto('visa56consec', $_REQUEST['visa56consec'], formato_numero($_REQUEST['visa56consec']));
}
?>
</label>
<?php
...
<label class="Label60"<?php echo $sOcultaId; ?>>
<?php
	echo html_oculto('visa56id', $_REQUEST['visa56id'], formato_numero($_REQUEST['visa56id']));
?>
</label>
<label class="Label130">
<?php
echo $ETI['visa56fechaini'];
?>
</label>
<div class="Campo220">
<?php
echo html_FechaEnNumero('visa56fechaini', $_REQUEST['visa56fechaini']); //, false, '', $iAgnoIni, $iAgnoFin); //$bvacio, $accion
?>
</div>
<?php
if (false) {
	echo $objForma->htmlBotonSolo('bvisa56fechaini_hoy', 'btMiniHoy', "fecha_AsignarNum('visa56fechaini', " . $iHoy . ")", $ETI['bt_hoy']);
}
?>
<label class="Label130">
<?php
echo $ETI['visa56fechafin'];
?>
</label>
<div class="Campo220">
<?php
echo html_FechaEnNumero('visa56fechafin', $_REQUEST['visa56fechafin']); //, false, '', $iAgnoIni, $iAgnoFin); //$bvacio, $accion
?>
</div>
<?php
if (false) {
	echo $objForma->htmlBotonSolo('bvisa56fechafin_hoy', 'btMiniHoy', "fecha_AsignarNum('visa56fechafin', " . $iHoy . ")", $ETI['bt_hoy']);
}
?>
<label class="Label130">
<?php
echo $ETI['visa56estado'];
?>
</label>
<label>
<div id="div_visa56estado" class="field">
<?php
echo $html_visa56estado;
?>
</div>
</label>
<label class="Label130">
<?php
echo $ETI['visa56fechacrea'];
?>
</label>
<div class="Campo220">
<?php
echo html_FechaEnNumero('visa56fechacrea', $_REQUEST['visa56fechacrea']); //, false, '', $iAgnoIni, $iAgnoFin); //$bvacio, $accion
?>
</div>
<?php
if (false) {
	echo $objForma->htmlBotonSolo('bvisa56fechacrea_hoy', 'btMiniHoy', "fecha_AsignarNum('visa56fechacrea', " . $iHoy . ")", $ETI['bt_hoy']);
}
?>
<label class="Label130">
<?php
echo $ETI['visa56fechacierre'];
?>
</label>
<div class="Campo220">
<?php
echo html_FechaEnNumero('visa56fechacierre', $_REQUEST['visa56fechacierre']); //, false, '', $iAgnoIni, $iAgnoFin); //$bvacio, $accion
?>
</div>
<?php
if (false) {
	echo $objForma->htmlBotonSolo('bvisa56fechacierre_hoy', 'btMiniHoy', "fecha_AsignarNum('visa56fechacierre', " . $iHoy . ")", $ETI['bt_hoy']);
}
?>
<label class="txtAreaS">
<?php
echo $ETI['visa56observacion'];
?>
<textarea id="visa56observacion" name="visa56observacion" placeholder="<?php echo $ETI['ing_campo'] . $ETI['visa56observacion']; ?>"><?php echo $_REQUEST['visa56observacion']; ?></textarea>
</label>
<label class="Label130">
<?php
echo $ETI['visa56activo'];
?>
</label>
<label>
<div id="div_visa56activo" class="field">
<?php
echo $html_visa56activo;
?>
</div>
</label>
<?php
...
if ($bConExpande) {
	//Este es el cierre del div_p2956
?>
<div class="salto1px"></div>
</div>
<?php
...
<label>
<input id="bnombre" name="bnombre" type="text" value="<?php echo $_REQUEST['bnombre']; ?>" onchange="paginarf2956()" autocomplete="off" />
</label>
<label class="Label90">
<?php
...
<label class="Label160">
<?php
echo $ETI['msg_visa56consec'];
?>
</label>
<label class="Label90">
<?php
echo '<b>' . $_REQUEST['visa56consec'] . '</b>';
?>
</label>
<div class="salto1px"></div>
<label class="Label160">
<?php
echo $ETI['msg_visa56consec_nuevo'];
// onchange="RevisaConsec()"
?>
</label>
<label class="Label90">
<input id="visa56consec_nuevo" name="visa56consec_nuevo" type="text" value="<?php echo $_REQUEST['visa56consec_nuevo']; ?>" class="cuatro" />
</label>
<div class="salto1px"></div>
...
<input id="titulo_2956" name="titulo_2956" type="hidden" value="<?php echo $sTituloModulo; ?>" />
<?php
if ($bBloqueTitulo) {
?>
<div class="titulos">
