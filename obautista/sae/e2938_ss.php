...
if ($sError == '') {
	...
	list($bEntra, $sDebugP) = seg_revisa_permisoV3(2938, 6, $idTercero, $objDB);
	if (!$bEntra) {
		$sError = 'No tiene permiso para consultar este reporte [Mod 2938 : 6]';
	}
}
if ($sError == '') {
...
if ($sError == '') {
	...
	$objExcel->getProperties()->setDescription('Reporte 2938 del SII 5.0 en ' . $sServerRpt . ' creado en ' . fecha_hoy() . ' ' . formato_horaminuto(fecha_hora(), fecha_minuto()));
	...
	$iFila = 12;
	PHPEXCEL_Escribir($objHoja, 0, $iFila, 'Pruebas de convocatoria');
	...
	while ($fila = $objDB->sf($tabla)) {
		$et_visa38idtipo = '';
		if ($fila['visa38idtipo'] != 0) {
			if (isset($avisa38idtipo[$fila['visa38idtipo']]) == 0) {
				$sDato = '{' . $fila['visa38idtipo'] . '}';
				...
				$avisa38idtipo[$fila['visa38idtipo']] = $sDato;
			}
			$et_visa38idtipo = $avisa38idtipo[$fila['visa38idtipo']];
		}
		PHPEXCEL_Escribir($objHoja, 0, $iFila, $et_visa38idtipo);
		PHPEXCEL_Escribir($objHoja, 1, $iFila, $fila['visa38consec']);
		PHPEXCEL_Escribir($objHoja, 2, $iFila, $fila['visa38nombre']);
		PHPEXCEL_Escribir($objHoja, 3, $iFila, $fila['visa38tipoprueba']);
		PHPEXCEL_Escribir($objHoja, 4, $iFila, $fila['visa38puntajemaximo']);
		PHPEXCEL_Escribir($objHoja, 5, $iFila, $fila['visa38puntajeaproba']);
		$et_visa38activo = 'Si';
		if ($fila['visa38activo'] == 0) {
			$et_visa38activo = 'No';
		}
		PHPEXCEL_Escribir($objHoja, 6, $iFila, $et_visa38activo);
		$et_visa38idnav = '';
		if ($fila['visa38idnav'] != 0) {
			if (isset($avisa38idnav[$fila['visa38idnav']]) == 0) {
				$sDato = '{' . $fila['visa38idnav'] . '}';
				$sSQL = 'SELECT unad39nombre FROM unad39nav WHERE unad39id=' . $fila['visa38idnav'] . '';
				$tablad = $objDB->ejecutasql($sSQL);
				if ($objDB->nf($tablad) > 0) {
					$filad = $objDB->sf($tablad);
					$sDato = $filad['unad39nombre'];
				}
				$avisa38idnav[$fila['visa38idnav']] = $sDato;
			}
			$et_visa38idnav = $avisa38idnav[$fila['visa38idnav']];
		}
		PHPEXCEL_Escribir($objHoja, 7, $iFila, $et_visa38idnav);
		PHPEXCEL_Escribir($objHoja, 8, $iFila, $fila['visa38idmoodle']);
		$iFila++;
	}
