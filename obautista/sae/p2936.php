...
class clsPDF extends FPDF
{
	...
	var $sRefRpt = '';
	var $sNumCopia = '';
	var $sTituloReporte = 'Tipologias de convocatoria';
	...
	function ArmarReporte2936($PARAMS, $objDB)
	{
		$this->SetTextColor(0, 0, 0);
		...
		$sSQL = 'SELECT * FROM visa36convtipologia WHERE visa36id=' . $PARAMS['id2936'] . '';
		if ($this->bDebug) {
		...
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			$svisa36idgrupo = $fila['visa36idgrupo'];
			$sSQL = 'SELECT visa46nombre FROM visa46grupotipologia WHERE visa46id=' . $fila['visa36idgrupo'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa36idgrupo = $filat['visa46nombre'];
				if ($this->bCodificar) {
					$svisa36idgrupo = cadena_codificar($filat['visa46nombre']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Grupo: ' . cadena_codificar($svisa36idgrupo));
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Consec: ' . $fila['visa36consec']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Nombre: ' . $fila['visa36nombre']);
			$this->Ln();
			$svisa36activo = $fila['visa36activo'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['visa36activo'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa36activo = $filat[''];
				if ($this->bCodificar) {
					$svisa36activo = cadena_codificar($filat['']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Activo: ' . cadena_codificar($svisa36activo));
			$this->Ln();
			$sTitulo = 'Subtipologias de convocatorias';
			//$sNumera=p_AddIndice($sTitulo, $this->PageNo(), 1, $this);
			p_TituloBloque($sTitulo, $this);
			p_FuenteNormalV2($this);
			$sSQL = 'SELECT * FROM visa37convsubtipo WHERE visa37idtipologia=' . $fila['visa36id'] . '';
			if ($this->bDebug) {
...
function pdfReporteV2($iReporte, $PARAMS, $iFormato, $sNumCopiaReporte, $bCodificarUTF8, $objDB, $bDebug = false)
{
	...
	if ($sError == '') {
		...
		$objpdf->sTituloReporte = 'Tipologias de convocatoria';
		$sDetalle = '';
		/*
		if ($PARAMS['v7'] != '') {
		...
		*/
		...
		$objpdf->AddPage();
		$objpdf->ArmarReporte2936($PARAMS, $objDB);
		//$objpdf->AddPage();
...
if ($sError == '') {
	...
	$sTituloRpt = 'Tipologias de convocatoria';
