...
if ($sError == '') {
	...
	list($bEntra, $sDebugP) = seg_revisa_permisoV3(2946, 6, $idTercero, $objDB);
	if (!$bEntra) {
		$sError = 'No tiene permiso para consultar este reporte [Mod 2946 : 6]';
	}
}
if ($sError == '') {
...
if ($sError == '') {
	...
	// ------------------------------------------------
	$sCampos = 'SELECT TB.visa46consec, TB.visa46id';
...
if ($sError == '') {
	...
	$objExcel->getProperties()->setDescription('Reporte 2946 del SII 5.0 en ' . $sServerRpt . ' creado en ' . fecha_hoy() . ' ' . formato_horaminuto(fecha_hora(), fecha_minuto()));
	...
	$iFila = 12;
	PHPEXCEL_Escribir($objHoja, 0, $iFila, 'Grupos de tipologias');
	...
	while ($fila = $objDB->sf($tabla)) {
		PHPEXCEL_Escribir($objHoja, 0, $iFila, $fila['visa46consec']);
		PHPEXCEL_Escribir($objHoja, 1, $iFila, $fila['visa46nombre']);
		$iFila++;
	}
