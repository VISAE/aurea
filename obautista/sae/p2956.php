...
class clsPDF extends FPDF
{
	...
	var $sRefRpt = '';
	var $sNumCopia = '';
	var $sTituloReporte = 'Periodos semanales';
	...
	function ArmarReporte2956($PARAMS, $objDB)
	{
		$this->SetTextColor(0, 0, 0);
		...
		$sSQL = 'SELECT * FROM visa56persemanal WHERE visa56id=' . $PARAMS['id2956'] . '';
		if ($this->bDebug) {
		...
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			$this->Cell($this->iAnchoLibre, 5, 'Consec: ' . $fila['visa56consec']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fechaini: ' . $fila['visa56fechaini']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fechafin: ' . $fila['visa56fechafin']);
			$this->Ln();
			$svisa56estado = $fila['visa56estado'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['visa56estado'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa56estado = $filat[''];
				if ($this->bCodificar) {
					$svisa56estado = cadena_codificar($filat['']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Estado: ' . cadena_codificar($svisa56estado));
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fechacrea: ' . $fila['visa56fechacrea']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fechacierre: ' . $fila['visa56fechacierre']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Observacion: ' . $fila['visa56observacion']);
			$this->Ln();
			$svisa56activo = $fila['visa56activo'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['visa56activo'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa56activo = $filat[''];
				if ($this->bCodificar) {
					$svisa56activo = cadena_codificar($filat['']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Activo: ' . cadena_codificar($svisa56activo));
			$this->Ln();
		}
		// Fin de ArmarReporte2956
	}
	// Fin de clsPDF
}
function pdfReporteV2($iReporte, $PARAMS, $iFormato, $sNumCopiaReporte, $bCodificarUTF8, $objDB, $bDebug = false)
{
	...
	if ($sError == '') {
		...
		$objpdf->sTituloReporte = 'Periodos semanales';
		$sDetalle = '';
		/*
		if ($PARAMS['v7'] != '') {
		...
		*/
		...
		$objpdf->AddPage();
		$objpdf->ArmarReporte2956($PARAMS, $objDB);
		//$objpdf->AddPage();
