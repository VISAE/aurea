...
if ($sError == '') {
	...
	list($bEntra, $sDebugP) = seg_revisa_permisoV3(2956, 6, $idTercero, $objDB);
	if (!$bEntra) {
		$sError = 'No tiene permiso para consultar este reporte [Mod 2956 : 6]';
	}
}
if ($sError == '') {
...
if ($sError == '') {
	...
	$sConsulta = 'FROM visa56persemanal AS TB 
...
if ($sError == '') {
	...
	$objExcel->getProperties()->setDescription('Reporte 2956 del SII 5.0 en ' . $sServerRpt . ' creado en ' . fecha_hoy() . ' ' . formato_horaminuto(fecha_hora(), fecha_minuto()));
	...
	$iFila = 12;
	PHPEXCEL_Escribir($objHoja, 0, $iFila, 'Periodos semanales');
	...
	while ($fila = $objDB->sf($tabla)) {
		PHPEXCEL_Escribir($objHoja, 0, $iFila, $fila['visa56consec']);
		PHPEXCEL_Escribir($objHoja, 1, $iFila, $fila['visa56fechaini']);
		PHPEXCEL_Escribir($objHoja, 2, $iFila, $fila['visa56fechafin']);
		$et_visa56estado = 'Si';
		if ($fila['visa56estado'] == 0) {
			$et_visa56estado = 'No';
		}
		PHPEXCEL_Escribir($objHoja, 3, $iFila, $et_visa56estado);
		PHPEXCEL_Escribir($objHoja, 4, $iFila, $fila['visa56fechacrea']);
		PHPEXCEL_Escribir($objHoja, 5, $iFila, $fila['visa56fechacierre']);
		PHPEXCEL_Escribir($objHoja, 6, $iFila, $fila['visa56observacion']);
		$et_visa56activo = 'Si';
		if ($fila['visa56activo'] == 0) {
			$et_visa56activo = 'No';
		}
		PHPEXCEL_Escribir($objHoja, 7, $iFila, $et_visa56activo);
		$iFila++;
	}
