...
class clsPDF extends FPDF
{
	...
	var $sRefRpt = '';
	var $sNumCopia = '';
	var $sTituloReporte = 'Subtipologias de convocatorias';
	...
	function ArmarReporte2937($PARAMS, $objDB)
	{
		$this->SetTextColor(0, 0, 0);
		...
		$sSQL = 'SELECT * FROM visa37convsubtipo WHERE visa37id=' . $PARAMS['id2937'] . '';
		if ($this->bDebug) {
		...
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			$this->Cell($this->iAnchoLibre, 5, 'Tipologia: ' . $fila['visa37idtipologia']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Nombre: ' . $fila['visa37nombre']);
			$this->Ln();
			$svisa37activo = $fila['visa37activo'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['visa37activo'];
			...
			$this->Cell($this->iAnchoLibre, 5, 'Activo: ' . cadena_codificar($svisa37activo));
			$this->Ln();
		}
		// Fin de ArmarReporte2937
	}
	// Fin de clsPDF
}
function pdfReporteV2($iReporte, $PARAMS, $iFormato, $sNumCopiaReporte, $bCodificarUTF8, $objDB, $bDebug = false)
{
	...
	if ($sError == '') {
		...
		$objpdf->sTituloReporte = 'Subtipologias de convocatorias';
		$sDetalle = '';
		/*
		if ($PARAMS['v7'] != '') {
		...
		*/
		...
		$objpdf->AddPage();
		$objpdf->ArmarReporte2937($PARAMS, $objDB);
		//$objpdf->AddPage();
...
if ($sError == '') {
	...
	$sTituloRpt = 'Subtipologias de convocatorias';
