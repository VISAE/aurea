...
switch ($datos[0]) {
	...
	if ($bDebug) {echo 'Enviando visa38convpruebas a ' . $idEntidad . ' ' . $sIdTercero . ' ' . $sIdMovil . ' ' . $sListaIds . '<br>';
	}
	...
	if (($sError == '') && ($sListaIds!=-99)) {
		...
		if ($sIds == '') {
			...
			$sCondicion = ' WHERE visa38id IN (' . $sIds . ')';
		}
	}
	if ($sError == '') {
		$data1 = array();
		...
		$sSQL = 'SELECT visa38idtipo, visa38consec, visa38id, visa38nombre, visa38tipoprueba, visa38puntajemaximo, visa38puntajeaproba, visa38activo, visa38idnav, visa38idmoodle FROM visa38convpruebas' . $sCondicion;
		if ($bDebug) {
		...
		while ($fila = $objDB->sf($tabla)) {
			$data1[] = $fila;
			$sIdsPadre = $sIdsPadre . ', ' . $fila['visa38id'];
			$iTotal++;
		}
		$res = array();
		$res[] = $iTotal;
