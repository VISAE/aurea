...
switch ($datos[0]) {
	...
	if ($bDebug) {echo 'Enviando visa56persemanal a ' . $idEntidad . ' ' . $sIdTercero . ' ' . $sIdMovil . ' ' . $sListaIds . '<br>';
	}
	...
	if (($sError == '') && ($sListaIds!=-99)) {
		...
		if ($sIds == '') {
			...
			$sCondicion = ' WHERE visa56id IN (' . $sIds . ')';
		}
	}
	if ($sError == '') {
		$data1 = array();
		...
		$sSQL = 'SELECT visa56consec, visa56id, visa56fechaini, visa56fechafin, visa56estado, visa56fechacrea, visa56fechacierre, visa56observacion, visa56activo FROM visa56persemanal' . $sCondicion;
		if ($bDebug) {
		...
		while ($fila = $objDB->sf($tabla)) {
			$data1[] = $fila;
			$sIdsPadre = $sIdsPadre . ', ' . $fila['visa56id'];
			$iTotal++;
		}
		$res = array();
		$res[] = $iTotal;
