...
*/
/** Archivo visaesubtipologia.php.
 * Modulo 2937 visa37convsubtipo.
...
if (!$bPeticionXAJAX) {
	if (noticias_pendientes($objDB)) {
		$objDB->CerrarConexion();
		header('Location:noticia.php?ret=visaesubtipologia.php');
		die();
	}
}
$seg_1707 = 0;
...
$xajax->register(XAJAX_FUNCTION, 'f2937_ExisteDato');
$xajax->processRequest();
if ($bPeticionXAJAX) {
...
if (($_REQUEST['paso'] == 1) || ($_REQUEST['paso'] == 3)) {
	if ($_REQUEST['paso'] == 1) {
		$sSQLcondi = 'visa37idtipologia=' . $_REQUEST['visa37idtipologia'] . '';
	} else {
		$sSQLcondi = 'visa37id=' . $_REQUEST['visa37id'] . '';
	}
	$sSQL = 'SELECT * FROM visa37convsubtipo WHERE ' . $sSQLcondi;
	$tabla = $objDB->ejecutasql($sSQL);
	if ($objDB->nf($tabla) > 0) {
		$fila = $objDB->sf($tabla);
		$_REQUEST['visa37idtipologia'] = $fila['visa37idtipologia'];
		$_REQUEST['visa37id'] = $fila['visa37id'];
		$_REQUEST['visa37nombre'] = $fila['visa37nombre'];
		$_REQUEST['visa37activo'] = $fila['visa37activo'];
		$bcargo = true;
		$_REQUEST['paso'] = 2;
		$_REQUEST['boculta2937'] = 0;
		$bLimpiaHijos = true;
...
if (($_REQUEST['paso'] == 10) || ($_REQUEST['paso'] == 12)) {
	$bMueveScroll = true;
	list($_REQUEST, $sError, $iTipoError, $sDebugGuardar) = f2937_db_GuardarV2b($_REQUEST, $objDB, $bDebug, $idTercero);
...
if ($_REQUEST['paso'] == 13) {
	$_REQUEST['paso'] = 2;
	if ($sError == '') {
		list($sError, $iTipoError, $sDebugElimina) = f2937_db_Eliminar($_REQUEST['visa37id'], $objDB, $bDebug);
...
$objCombos->sAccion = 'paginarf2937()';
$sSQL = '';
$html_blistar = $objCombos->html($sSQL, $objDB);
//$html_blistar = $objCombos->comboSistema(2937, 1, $objDB, 'paginarf2937()');
*/
if (false) {
...
$aParametros[0] = ''; //$_REQUEST['p1_2937'];
$aParametros[100] = $idTercero;
$aParametros[101] = $_REQUEST['paginaf2937'];
$aParametros[102] = $_REQUEST['lppf2937'];
//$aParametros[103] = $_REQUEST['bnombre'];
//$aParametros[104] = $_REQUEST['blistar'];
list($sTabla2937, $sDebugTabla) = f2937_TablaDetalleV2($aParametros, $objDB, $bDebug);
...
?>
	...
	function imprimeexcel() {
		...
		if (sError == '') {
			asignarvariables();
			window.document.frmimpp.action = 'e2937_ss.php';
	...
	function imprimep() {
		if (window.document.frmedita.seg_5.value == 1) {
			asignarvariables();
			window.document.frmimpp.action = 'p2937.php';
			window.document.frmimpp.submit();
		} else {
	...
	function RevisaLlave() {
		let datos = new Array();
		datos[1] = window.document.frmedita.visa37idtipologia.value;
		if ((datos[1] != '')) {
			xajax_f2937_ExisteDato(datos);
		}
	}

	function cargadato(llave1) {
		window.document.frmedita.visa37idtipologia.value = String(llave1);
		window.document.frmedita.paso.value = 1;
		window.document.frmedita.submit();
	}

	function cargaridf2937(llave1) {
		window.document.frmedita.visa37id.value = String(llave1);
	...
	function objinicial() {
		document.getElementById("visa37idtipologia").focus();
	}
...
?>
<form id="frmimpp" name="frmimpp" method="post" action="p2937.php" target="_blank" style="display:none">
<input id="r" name="r" type="hidden" value="2937" />
<input id="id2937" name="id2937" type="hidden" value="<?php echo $_REQUEST['visa37id']; ?>" />
...
if ($_REQUEST['paso'] != 2) {
?>
<input id="visa37idtipologia" name="visa37idtipologia" type="text" value="<?php echo $_REQUEST['visa37idtipologia']; ?>" onchange="RevisaLlave()" class="diez" maxlength="10" placeholder="<?php echo $ETI['ing_vr']; ?>" />
<?php
} else {
	echo html_oculto('visa37idtipologia', $_REQUEST['visa37idtipologia']);
}
?>
</label>
...
<label>
<input id="bnombre" name="bnombre" type="text" value="<?php echo $_REQUEST['bnombre']; ?>" onchange="paginarf2937()" autocomplete="off" />
</label>
<label class="Label90">
<?php
...
<input id="titulo_2937" name="titulo_2937" type="hidden" value="<?php echo $sTituloModulo; ?>" />
<?php
if ($bBloqueTitulo) {
?>
<div class="titulos">
