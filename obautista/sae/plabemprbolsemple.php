...
if (isset($_REQUEST['plab08nombre'])==0){$_REQUEST['plab08nombre']='';}
...
if (($_REQUEST['paso']==1)||($_REQUEST['paso']==3)){
	if ($_REQUEST['paso']==1){
		$sSQLcondi='plab08consecutivo='.$_REQUEST['plab08consecutivo'].'';
		}else{
	...
	if ($objDB->nf($tabla)>0){
		...
		$_REQUEST['plab08nombre']=$fila['plab08nombre'];
...
if ($_REQUEST['paso']==93){
	...
	if ($sError==''){
		...
		$sSQL='SELECT plab08id FROM plab08emprbolsempleo WHERE plab08consecutivo='.$_REQUEST['plab08consecutivo_nuevo'].'';
		$tabla=$objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla)>0){
...
if ($_REQUEST['paso']!=0){
	//rango salarial
	$aParametros2903[0]=$_REQUEST['plab08id'];
...
function RevisaLlave(){
	var datos= new Array();
	datos[1]=window.document.frmedita.plab08consecutivo.value;
	if ((datos[1]!='')){
		xajax_f2908_ExisteDato(datos);
		}
	}
function cargadato(llave1){
	window.document.frmedita.plab08consecutivo.value=String(llave1);
	window.document.frmedita.paso.value=1;
	window.document.frmedita.submit();
	}
...
function objinicial(){
	document.getElementById("plab08consecutivo").focus();
	}
...
<label class="Label130">
<?php
echo $ETI['plab08nombre'];
?>
</label>
<label>
<input id="plab08nombre" name="plab08nombre" type="text" value="<?php echo $_REQUEST['plab08nombre']; ?>" maxlength="50" placeholder="<?php echo $ETI['ing_campo'].$ETI['plab08nombre']; ?>"/>
</label>
<label class="Label130">
<?php
