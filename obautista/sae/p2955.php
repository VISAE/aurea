...
class clsPDF extends FPDF
{
	...
	var $sRefRpt = '';
	var $sNumCopia = '';
	var $sTituloReporte = 'Sistemas VISAE';
	...
	function ArmarReporte2955($PARAMS, $objDB)
	{
		$this->SetTextColor(0, 0, 0);
		...
		$sSQL = 'SELECT * FROM visa55sistema WHERE visa55id=' . $PARAMS['id2955'] . '';
		if ($this->bDebug) {
		...
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			$svisa55idproyecto = $fila['visa55idproyecto'];
			$sSQL = 'SELECT plan04numero FROM plan04proyecto WHERE plan04id=' . $fila['visa55idproyecto'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa55idproyecto = $filat['plan04numero'];
				if ($this->bCodificar) {
					$svisa55idproyecto = cadena_codificar($filat['plan04numero']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Proyecto: ' . cadena_codificar($svisa55idproyecto));
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Codigo: ' . $fila['visa55codigo']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Nombre: ' . $fila['visa55nombre']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Descripcion: ' . $fila['visa55descripcion']);
			$this->Ln();
			$et_visa55idresponsable = p_DatosTercero($fila['visa55idresponsable'], $objDB);
			if ($this->bCodificar) {
				$et_visa55idresponsable = cadena_codificar($et_visa55idresponsable);
			}
			$this->Cell($this->iAnchoLibre, 5, 'Responsable: ' . cadena_codificar($et_visa55idresponsable));
			$this->Ln();
			$svisa55vigente = $fila['visa55vigente'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['visa55vigente'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa55vigente = $filat[''];
				if ($this->bCodificar) {
					$svisa55vigente = cadena_codificar($filat['']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Vigente: ' . cadena_codificar($svisa55vigente));
			$this->Ln();
			$svisa55color = $fila['visa55color'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['visa55color'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa55color = $filat[''];
				if ($this->bCodificar) {
					$svisa55color = cadena_codificar($filat['']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Color: ' . cadena_codificar($svisa55color));
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fechacreacion: ' . $fila['visa55fechacreacion']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fechaactualiza: ' . $fila['visa55fechaactualiza']);
			$this->Ln();
		}
		// Fin de ArmarReporte2955
	}
	// Fin de clsPDF
}
function pdfReporteV2($iReporte, $PARAMS, $iFormato, $sNumCopiaReporte, $bCodificarUTF8, $objDB, $bDebug = false)
{
	...
	if ($sError == '') {
		...
		$objpdf->sTituloReporte = 'Sistemas VISAE';
		$sDetalle = '';
		/*
		if ($PARAMS['v7'] != '') {
		...
		*/
		...
		$objpdf->AddPage();
		$objpdf->ArmarReporte2955($PARAMS, $objDB);
		//$objpdf->AddPage();
