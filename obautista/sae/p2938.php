...
class clsPDF extends FPDF
{
	...
	var $sRefRpt = '';
	var $sNumCopia = '';
	var $sTituloReporte = 'Pruebas de convocatoria';
	...
	function ArmarReporte2938($PARAMS, $objDB)
	{
		$this->SetTextColor(0, 0, 0);
		...
		$sSQL = 'SELECT * FROM visa38convpruebas WHERE visa38id=' . $PARAMS['id2938'] . '';
		if ($this->bDebug) {
		...
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			$svisa38idtipo = $fila['visa38idtipo'];
			$sSQL = 'SELECT visa34nombre FROM visa34convtipo WHERE visa34id=' . $fila['visa38idtipo'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa38idtipo = $filat['visa34nombre'];
				if ($this->bCodificar) {
					$svisa38idtipo = cadena_codificar($filat['visa34nombre']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Tipo: ' . cadena_codificar($svisa38idtipo));
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Consec: ' . $fila['visa38consec']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Nombre: ' . $fila['visa38nombre']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Tipoprueba: ' . $fila['visa38tipoprueba']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Puntajemaximo: ' . $fila['visa38puntajemaximo']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Puntajeaproba: ' . $fila['visa38puntajeaproba']);
			$this->Ln();
			$svisa38activo = $fila['visa38activo'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['visa38activo'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa38activo = $filat[''];
				if ($this->bCodificar) {
					$svisa38activo = cadena_codificar($filat['']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Activo: ' . cadena_codificar($svisa38activo));
			$this->Ln();
			$svisa38idnav = $fila['visa38idnav'];
			$sSQL = 'SELECT unad39nombre FROM unad39nav WHERE unad39id=' . $fila['visa38idnav'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa38idnav = $filat['unad39nombre'];
				if ($this->bCodificar) {
					$svisa38idnav = cadena_codificar($filat['unad39nombre']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Nav: ' . cadena_codificar($svisa38idnav));
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Moodle: ' . $fila['visa38idmoodle']);
			$this->Ln();
		}
		// Fin de ArmarReporte2938
	}
	// Fin de clsPDF
}
function pdfReporteV2($iReporte, $PARAMS, $iFormato, $sNumCopiaReporte, $bCodificarUTF8, $objDB, $bDebug = false)
{
	...
	if ($sError == '') {
		...
		$objpdf->sTituloReporte = 'Pruebas de convocatoria';
		$sDetalle = '';
		/*
		if ($PARAMS['v7'] != '') {
		...
		*/
		...
		$objpdf->AddPage();
		$objpdf->ArmarReporte2938($PARAMS, $objDB);
		//$objpdf->AddPage();
...
if ($sError == '') {
	...
	$sTituloRpt = 'Pruebas de convocatoria';
