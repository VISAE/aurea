# Ficha técnica · Componente APP· Módulo 2955 "Familias presupuestales"

## 1. Identificación
- Componente: NomAPP (APP 000).
- Módulo: 2955 – 'ppto60familiappto'.
- Ruta de entrada: '_comp_/pptofamilia.php'.
- Lógica de negocio reutilizable: 'presupuesto/lib2955.php' ('f2955_*').
- Internacionalización: 'presupuesto/lg/lg_2955_{es|en|pt}.php'.
...
- Versión del módulo: v1.0.0 (baseline 15/9/2026, Juan David Avellaneda Molina).
...
5. Acciones de usuario:
   - Guardar/actualizar llaman a 'f2955_db_GuardarV2b'.
   - Eliminar llama a 'f2955_db_Eliminar'.
   - Tabla inferior se carga con 'f2955_TablaDetalleV2' vía XAJAX.
...
| 'seg_2/3/4' | Crear, editar, borrar (validado dentro de 'f2955_db_GuardarV2b'/'f2955_db_Eliminar'). |

## 5. Formulario y campos
| Campo | Descripción | Fuente/Regla | Validaciones |
| --- | --- | --- | --- |
| 'visa55idproyecto' | Proyecto. | Numerico. | Requerido en creación. |
| 'visa55codigo' | Código (<=10 chars). | Texto. | Requerido, sanitizado con 'cadena_Validar'. |
| 'visa55id' | Ref. | Numérico opcional. | Entero positivo. |
| 'visa55nombre' | Nombre (<=100 chars). | Texto. | Requerido, sanitizado con 'cadena_Validar'. |
| 'visa55descripcion' | Descripcion. | Numerico. | Requerido en creación. |
| 'visa55idresponsable' | Responsable. | Numerico. | Requerido en creación. |
| 'visa55vigente' | Vigente. | Numerico. | Requerido en creación. |
| 'visa55color' | Color. | Numerico. | Requerido en creación. |
| 'visa55fechacreacion' | Fechacreacion. | Numerico. | Requerido en creación. |
| 'visa55fechaactualiza' | Fecha Actualización. | Numerico. | Requerido en creación. |
| 'deb_tipodoc', 'deb_doc' | Acceso delegado. | Sólo visibles con 'seg_1707'. | Validan existencia en 'unad11terceros'. |
| Ocultos ('paso', 'iscroll', 'boculta2955', 'debug', etc.) | Control de estado. | – | – |

## 6. Servicios XAJAX destacados
| Función | Archivo | Descripción |
| --- | --- | --- |
| 'f2955_TablaDetalleV2' | 'lib2955.php' | Construye tabla HTML filtrada/paginada, retorna 'div_f2955detalle'. |
| 'f2955_db_GuardarV2b' | 'lib2955.php' | Alta/edición con validaciones de permisos, largos y auditoría ('seg_auditar'). |
| 'f2955_db_Eliminar' | 'lib2955.php' | Elimina tras revisar bloqueos ('unad70bloqueoelimina'). |
| 'f2955_ExisteDato' | 'lib2955.php' | Verifica existencia por consecutivo y redirige formulario. |
...
2. Exportar listado con usuario con/ sin 'seg_6'.
