# Ficha técnica · Componente APP· Módulo 2937 "Familias presupuestales"

## 1. Identificación
- Componente: NomAPP (APP 000).
- Módulo: 2937 – 'ppto60familiappto'.
- Ruta de entrada: '_comp_/pptofamilia.php'.
- Lógica de negocio reutilizable: 'presupuesto/lib2937.php' ('f2937_*').
- Internacionalización: 'presupuesto/lg/lg_2937_{es|en|pt}.php'.
...
5. Acciones de usuario:
   - Guardar/actualizar llaman a 'f2937_db_GuardarV2b'.
   - Eliminar llama a 'f2937_db_Eliminar'.
   - Tabla inferior se carga con 'f2937_TablaDetalleV2' vía XAJAX.
...
| 'seg_2/3/4' | Crear, editar, borrar (validado dentro de 'f2937_db_GuardarV2b'/'f2937_db_Eliminar'). |

## 5. Formulario y campos
| Campo | Descripción | Fuente/Regla | Validaciones |
| --- | --- | --- | --- |
| 'visa37idtipologia' | Idtipologia. | Numerico. | Requerido en creación. |
| 'visa37id' | Ref. | Numérico opcional. | Entero positivo. |
| 'visa37nombre' | Nombre (<=50 chars). | Texto. | Requerido, sanitizado con 'cadena_Validar'. |
| 'visa37activo' | Activo. | Numerico. | Requerido en creación. |
| 'deb_tipodoc', 'deb_doc' | Acceso delegado. | Sólo visibles con 'seg_1707'. | Validan existencia en 'unad11terceros'. |
| Ocultos ('paso', 'iscroll', 'boculta2937', 'debug', etc.) | Control de estado. | – | – |

## 6. Servicios XAJAX destacados
| Función | Archivo | Descripción |
| --- | --- | --- |
| 'f2937_TablaDetalleV2' | 'lib2937.php' | Construye tabla HTML filtrada/paginada, retorna 'div_f2937detalle'. |
| 'f2937_db_GuardarV2b' | 'lib2937.php' | Alta/edición con validaciones de permisos, largos y auditoría ('seg_auditar'). |
| 'f2937_db_Eliminar' | 'lib2937.php' | Elimina tras revisar bloqueos ('unad70bloqueoelimina'). |
| 'f2937_ExisteDato' | 'lib2937.php' | Verifica existencia por consecutivo y redirige formulario. |
