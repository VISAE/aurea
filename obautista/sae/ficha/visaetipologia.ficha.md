# Ficha técnica · Componente APP· Módulo 2936 "Familias presupuestales"

## 1. Identificación
- Componente: NomAPP (APP 000).
- Módulo: 2936 – 'ppto60familiappto'.
- Ruta de entrada: '_comp_/pptofamilia.php'.
- Lógica de negocio reutilizable: 'presupuesto/lib2936.php' ('f2936_*').
- Internacionalización: 'presupuesto/lg/lg_2936_{es|en|pt}.php'.
...
5. Acciones de usuario:
   - Guardar/actualizar llaman a 'f2936_db_GuardarV2b'.
   - Eliminar llama a 'f2936_db_Eliminar'.
   - Tabla inferior se carga con 'f2936_TablaDetalleV2' vía XAJAX.
...
| 'seg_2/3/4' | Crear, editar, borrar (validado dentro de 'f2936_db_GuardarV2b'/'f2936_db_Eliminar'). |

## 5. Formulario y campos
| Campo | Descripción | Fuente/Regla | Validaciones |
| --- | --- | --- | --- |
| 'visa36idgrupo' | Grupo. | Numerico. | Requerido en creación. |
| 'visa36consec' | Consecutivo. | Numérico gestionado por el sistema; modificable con permiso 8. | Entero positivo. |
| 'visa36id' | Ref. | Numérico opcional; si es 0 toma 'visa36consec'. | Entero positivo. |
| 'visa36nombre' | Nombre (<=50 chars). | Texto. | Requerido, sanitizado con 'cadena_Validar'. |
| 'visa36activo' | Activo. | Numerico. | Requerido en creación. |
| 'deb_tipodoc', 'deb_doc' | Acceso delegado. | Sólo visibles con 'seg_1707'. | Validan existencia en 'unad11terceros'. |
| Ocultos ('paso', 'iscroll', 'boculta2936', 'debug', etc.) | Control de estado. | – | – |

## 6. Servicios XAJAX destacados
| Función | Archivo | Descripción |
| --- | --- | --- |
| 'f2936_TablaDetalleV2' | 'lib2936.php' | Construye tabla HTML filtrada/paginada, retorna 'div_f2936detalle'. |
| 'f2936_db_GuardarV2b' | 'lib2936.php' | Alta/edición con validaciones de permisos, largos y auditoría ('seg_auditar'). |
| 'f2936_db_Eliminar' | 'lib2936.php' | Elimina tras revisar bloqueos ('unad70bloqueoelimina'). |
| 'f2936_ExisteDato' | 'lib2936.php' | Verifica existencia por consecutivo y redirige formulario. |
...
2. Modificar 'visa36consec' sin permiso 8 (debe fallar).
