# Ficha técnica · Componente APP· Módulo 2946 "Familias presupuestales"

## 1. Identificación
- Componente: NomAPP (APP 000).
- Módulo: 2946 – 'ppto60familiappto'.
- Ruta de entrada: '_comp_/pptofamilia.php'.
- Lógica de negocio reutilizable: 'presupuesto/lib2946.php' ('f2946_*').
- Internacionalización: 'presupuesto/lg/lg_2946_{es|en|pt}.php'.
...
5. Acciones de usuario:
   - Guardar/actualizar llaman a 'f2946_db_GuardarV2b'.
   - Eliminar llama a 'f2946_db_Eliminar'.
   - Tabla inferior se carga con 'f2946_TablaDetalleV2' vía XAJAX.
...
| 'seg_2/3/4' | Crear, editar, borrar (validado dentro de 'f2946_db_GuardarV2b'/'f2946_db_Eliminar'). |

## 5. Formulario y campos
| Campo | Descripción | Fuente/Regla | Validaciones |
| --- | --- | --- | --- |
| 'visa46consec' | Consecutivo. | Numérico gestionado por el sistema; modificable con permiso 8. | Entero positivo. |
| 'visa46id' | Ref. | Numérico opcional; si es 0 toma 'visa46consec'. | Entero positivo. |
| 'visa46nombre' | Nombre Grupo (<=50 chars). | Texto. | Requerido, sanitizado con 'cadena_Validar'. |
| 'deb_tipodoc', 'deb_doc' | Acceso delegado. | Sólo visibles con 'seg_1707'. | Validan existencia en 'unad11terceros'. |
| Ocultos ('paso', 'iscroll', 'boculta2946', 'debug', etc.) | Control de estado. | – | – |

## 6. Servicios XAJAX destacados
| Función | Archivo | Descripción |
| --- | --- | --- |
| 'f2946_TablaDetalleV2' | 'lib2946.php' | Construye tabla HTML filtrada/paginada, retorna 'div_f2946detalle'. |
| 'f2946_db_GuardarV2b' | 'lib2946.php' | Alta/edición con validaciones de permisos, largos y auditoría ('seg_auditar'). |
| 'f2946_db_Eliminar' | 'lib2946.php' | Elimina tras revisar bloqueos ('unad70bloqueoelimina'). |
| 'f2946_ExisteDato' | 'lib2946.php' | Verifica existencia por consecutivo y redirige formulario. |
...
2. Modificar 'visa46consec' sin permiso 8 (debe fallar).
