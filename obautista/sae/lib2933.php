...
function f2933_db_Guardar($valores, $objDB, $bDebug = false, $idTercero = 0)
{
	...
	$sSepara = ', ';
	if ($plab33activo == '') {
		$sError = $ERR['plab33activo'] . $sSepara . $sError;
	}
	if ((int)$plab33idmonitor == 0) {
		$sError = $ERR['plab33idmonitor'] . $sSepara . $sError;
	}
	if ((int)$plab33idperiodo == 0) {
		$sError = $ERR['plab33idperiodo'] . $sSepara . $sError;
	}
	if ((int)$plab33idcurso == 0) {
		$sError = $ERR['plab33idcurso'] . $sSepara . $sError;
	}
	...
	if ($sError == '') {
		if ($bInserta) {
			...
			if ($bDebug) {
				$sDebug = $sDebug . fecha_microtiempo() . ' Guardar 2933 ' . $sSQL . '<br>';
			}
			$result = $objDB->ejecutasql($sSQL);
			if ($result == false) {
...
function f2933_TablaDetalleV2($aParametros, $objDB, $bDebug = false)
{
	...
	$sTabla2933 = 'plab33emoncurso';
	$sLeyenda = '';
	$sBotones = '<input id="paginaf2933" name="paginaf2933" type="hidden" value="' . $pagina . '"/>';
	$sBotones = $sBotones . '<input id="lppf2933" name="lppf2933" type="hidden" value="' . $lineastabla . '"/>';
	if ($sLeyenda != '') {
	...
	*/
	...
	$sConsulta = 'FROM ' . $sTabla2933 . ' AS TB, unad11terceros AS T2, unad40curso AS T40 
	...
	if ($bGigante) {
		...
		if ($bDebug) {
			$sDebug = $sDebug . log_debug(' Totalizando consulta 2933: ' . $sSQLContador . '');
		}
	...
	$sErrConsulta = $sErrConsulta . '<input id="titulos_2933" name="titulos_2933" type="hidden" value="' . $sTitulos . '"/>';
	if ($bDebug) {
		$sDebug = $sDebug . log_debug('Consulta 2933: ' . $sSQL . '');
	}
	...
	if ($bDebug) {
		$sDebug = $sDebug . log_debug('Termina la consulta 2933');
	}
	...
	$res = $res . '<th colspan="2"><b>' . $ETI['plab33idmonitor'] . '</b></th>';
	$res = $res . '<th><b>' . $ETI['plab33idperiodo'] . '</b></th>';
	$res = $res . '<th colspan="2"><b>' . $ETI['plab33idcurso'] . '</b></th>';
	$res = $res . '<th><b>' . $ETI['plab33activo'] . '</b></th>';
	$res = $res . '<th class="flex gap-1 justify-end">';
	$res = $res . html_paginador('paginaf2933', $registros, $lineastabla, $pagina, 'paginarf2933()');
	$res = $res . html_lpp('lppf2933', $lineastabla, 'paginarf2933()');
	$res = $res . '</th>';
	...
	while ($filadet = $objDB->sf($tabladetalle)) {
		...
		$res = $res . '<tr' . $sClass . '>';
		$res = $res . '<td>' . $et_plab33idmonitor_doc . '</td>';
		$res = $res . '<td>' . $et_plab33idmonitor_nombre . '</td>';
		$res = $res . '<td>' . $et_plab33idperiodo . '</td>';
		$res = $res . '<td>' . $et_plab33idcurso . '</td>';
		$res = $res . '<td>' . $et_plab33idcurso_nombre . '</td>';
		$res = $res . '<td>' . $et_plab33activo . '</td>';
...
function f2933_Traer($aParametros)
{
	...
	if ($paso == 1) {
		...
		$plab33idperiodo = numeros_validar($aParametros[3]);
		$plab33idcurso = numeros_validar($aParametros[4]);
		if (($plab33idciclo != '') && ($plab33idmonitor != '') && ($plab33idperiodo != '') && ($plab33idcurso != '')) {
			$besta = true;
		}
	} else {
