...
class clsPDF extends FPDF
{
	...
	var $sRefRpt = '';
	var $sNumCopia = '';
	var $sTituloReporte = 'Grupos de tipologias';
	...
	function ArmarReporte2946($PARAMS, $objDB)
	{
		$this->SetTextColor(0, 0, 0);
		...
		$sSQL = 'SELECT * FROM visa46grupotipologia WHERE visa46id=' . $PARAMS['id2946'] . '';
		if ($this->bDebug) {
		...
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			$this->Cell($this->iAnchoLibre, 5, 'Consec: ' . $fila['visa46consec']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Nombre: ' . $fila['visa46nombre']);
			$this->Ln();
		}
		// Fin de ArmarReporte2946
	}
	// Fin de clsPDF
}
function pdfReporteV2($iReporte, $PARAMS, $iFormato, $sNumCopiaReporte, $bCodificarUTF8, $objDB, $bDebug = false)
{
	...
	if ($sError == '') {
		...
		$objpdf->sTituloReporte = 'Grupos de tipologias';
		$sDetalle = '';
		/*
		if ($PARAMS['v7'] != '') {
		...
		*/
		...
		$objpdf->AddPage();
		$objpdf->ArmarReporte2946($PARAMS, $objDB);
		//$objpdf->AddPage();
...
if ($sError == '') {
	...
	$sTituloRpt = 'Grupos de tipologias';
