...
if ($sError == '') {
	...
	list($bEntra, $sDebugP) = seg_revisa_permisoV3(2945, 6, $idTercero, $objDB);
	if (!$bEntra) {
		$sError = 'No tiene permiso para consultar este reporte [Mod 2945 : 6]';
	}
}
if ($sError == '') {
...
if ($sError == '') {
	...
	$objExcel->getProperties()->setDescription('Reporte 2945 del SII 5.0 en ' . $sServerRpt . ' creado en ' . fecha_hoy() . ' ' . formato_horaminuto(fecha_hora(), fecha_minuto()));
	...
	$iFila = 12;
	PHPEXCEL_Escribir($objHoja, 0, $iFila, 'Resultados pruebas');
	...
	while ($fila = $objDB->sf($tabla)) {
		PHPEXCEL_Escribir($objHoja, 0, $iFila, $fila['visa45idinscripcion']);
		$et_visa45idprueba = '';
		if ($fila['visa45idprueba'] != 0) {
			if (isset($avisa45idprueba[$fila['visa45idprueba']]) == 0) {
				$sDato = '{' . $fila['visa45idprueba'] . '}';
				...
				if ($objDB->nf($tablad) > 0) {
					...
					$sDato = $filad['visa38nombre'];
				}
				$avisa45idprueba[$fila['visa45idprueba']] = $sDato;
			}
			$et_visa45idprueba = $avisa45idprueba[$fila['visa45idprueba']];
		}
		PHPEXCEL_Escribir($objHoja, 1, $iFila, $et_visa45idprueba);
		PHPEXCEL_Escribir($objHoja, 2, $iFila, $fila['visa45puntaje']);
		$iFila++;
	}
