...
class clsPDF extends FPDF
{
	...
	var $sRefRpt = '';
	var $sNumCopia = '';
	var $sTituloReporte = 'Convocatorias';
	...
	function ArmarReporte2935($PARAMS, $objDB)
	{
		$this->SetTextColor(0, 0, 0);
		...
		$sSQL = 'SELECT * FROM visa35convocatoria WHERE visa35id=' . $PARAMS['id2935'] . '';
		if ($this->bDebug) {
		...
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			$this->Cell($this->iAnchoLibre, 5, 'Consec: ' . $fila['visa35consec']);
			$this->Ln();
			$svisa35idtipo = $fila['visa35idtipo'];
			...
			if ($objDB->nf($tablat) > 0) {
				...
				$svisa35idtipo = $filat['visa34nombre'];
				if ($this->bCodificar) {
					$svisa35idtipo = cadena_codificar($filat['visa34nombre']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Tipo: ' . cadena_codificar($svisa35idtipo));
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Nombre: ' . $fila['visa35nombre']);
			$this->Ln();
			$svisa35idzona = $fila['visa35idzona'];
			$sSQL = 'SELECT unad23nombre FROM unad23zona WHERE unad23id=' . $fila['visa35idzona'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa35idzona = $filat['unad23nombre'];
				if ($this->bCodificar) {
					$svisa35idzona = cadena_codificar($filat['unad23nombre']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Zona: ' . cadena_codificar($svisa35idzona));
			$this->Ln();
			$svisa35idcentro = $fila['visa35idcentro'];
			$sSQL = 'SELECT unad24nombre FROM unad24sede WHERE unad24id=' . $fila['visa35idcentro'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa35idcentro = $filat['unad24nombre'];
				if ($this->bCodificar) {
					$svisa35idcentro = cadena_codificar($filat['unad24nombre']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Centro: ' . cadena_codificar($svisa35idcentro));
			$this->Ln();
			$svisa35idescuela = $fila['visa35idescuela'];
			$sSQL = 'SELECT core12nombre FROM core12escuela WHERE core12id=' . $fila['visa35idescuela'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa35idescuela = $filat['core12nombre'];
				if ($this->bCodificar) {
					$svisa35idescuela = cadena_codificar($filat['core12nombre']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Escuela: ' . cadena_codificar($svisa35idescuela));
			$this->Ln();
			$svisa35idprograma = $fila['visa35idprograma'];
			$sSQL = 'SELECT core09nombre FROM core09programa WHERE core09id=' . $fila['visa35idprograma'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa35idprograma = $filat['core09nombre'];
				if ($this->bCodificar) {
					$svisa35idprograma = cadena_codificar($filat['core09nombre']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Programa: ' . cadena_codificar($svisa35idprograma));
			$this->Ln();
			$svisa35gruponivel = $fila['visa35gruponivel'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['visa35gruponivel'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa35gruponivel = $filat[''];
				if ($this->bCodificar) {
					$svisa35gruponivel = cadena_codificar($filat['']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Gruponivel: ' . cadena_codificar($svisa35gruponivel));
			$this->Ln();
			$svisa35nivelforma = $fila['visa35nivelforma'];
			$sSQL = 'SELECT core22nombre FROM core22nivelprograma WHERE core22id=' . $fila['visa35nivelforma'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa35nivelforma = $filat['core22nombre'];
				if ($this->bCodificar) {
					$svisa35nivelforma = cadena_codificar($filat['core22nombre']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Nivelforma: ' . cadena_codificar($svisa35nivelforma));
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Estado: ' . $fila['visa35estado']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Numcupos: ' . $fila['visa35numcupos']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fecha_apertura: ' . $fila['visa35fecha_apertura']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fecha_liminscrip: ' . $fila['visa35fecha_liminscrip']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fecha_limrevdoc: ' . $fila['visa35fecha_limrevdoc']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fecha_examenes: ' . $fila['visa35fecha_examenes']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fecha_seleccion: ' . $fila['visa35fecha_seleccion']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fecha_ratificacion: ' . $fila['visa35fecha_ratificacion']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fecha_cierra: ' . $fila['visa35fecha_cierra']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Presentacion: ' . $fila['visa35presentacion']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Total_inscritos: ' . $fila['visa35total_inscritos']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Total_autorizados: ' . $fila['visa35total_autorizados']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Total_presentaex: ' . $fila['visa35total_presentaex']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Total_aprobados: ' . $fila['visa35total_aprobados']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Total_admitidos: ' . $fila['visa35total_admitidos']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Convenio: ' . $fila['visa35idconvenio']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Resolucion: ' . $fila['visa35idresolucion']);
			$this->Ln();
			$svisa35idproducto = $fila['visa35idproducto'];
			$sSQL = 'SELECT cart01nombre FROM cart01productos WHERE cart01id=' . $fila['visa35idproducto'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$svisa35idproducto = $filat['cart01nombre'];
				if ($this->bCodificar) {
					$svisa35idproducto = cadena_codificar($filat['cart01nombre']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Producto: ' . cadena_codificar($svisa35idproducto));
			$this->Ln();
...
function pdfReporteV2($iReporte, $PARAMS, $iFormato, $sNumCopiaReporte, $bCodificarUTF8, $objDB, $bDebug = false)
{
	...
	if ($sError == '') {
		...
		$objpdf->sTituloReporte = 'Convocatorias';
		$sDetalle = '';
		/*
		if ($PARAMS['v7'] != '') {
		...
		*/
		...
		$objpdf->AddPage();
		$objpdf->ArmarReporte2935($PARAMS, $objDB);
		//$objpdf->AddPage();
