...
$aNombres = array(
	'', '', '', 'Sistema', 'Titulo',
	'Estado', 'Prioridad', 'Fechaini', 'Fechafin'
);
$aTipos = array(
	0, 0, 0, 0, 1,
	0, 0, 0, 0
...
if ($sError == '') {
	...
	list($bEntra, $sDebugP) = seg_revisa_permisoV3(2957, 6, $idTercero, $objDB);
	if (!$bEntra) {
		$sError = 'No tiene permiso para consultar este reporte [Mod 2957 : 6]';
	}
}
if ($sError == '') {
...
if ($sError == '') {
	...
	// ------------------------------------------------
	$bsistema = numeros_validar($_REQUEST['v3']);
	$btitulo = cadena_Validar(trim($_REQUEST['v4']));
	$bestado = numeros_validar($_REQUEST['v5']);
	$bprioridad = numeros_validar($_REQUEST['v6']);
	$bfechaini = numeros_validar($_REQUEST['v7']);
	$bfechafin = numeros_validar($_REQUEST['v8']);
	$sSubtitulo = '';
	$sDetalle = '';
	$sSQLadd = '';
	$sSQLadd1 = '';
	...
	$sConsulta = 'FROM visa57actividad AS TB, visa55sistema AS T2 
...
if ($sError == '') {
	...
	$objExcel->getProperties()->setDescription('Reporte 2957 del SII 5.0 en ' . $sServerRpt . ' creado en ' . fecha_hoy() . ' ' . formato_horaminuto(fecha_hora(), fecha_minuto()));
	...
	$iFila = 12;
	PHPEXCEL_Escribir($objHoja, 0, $iFila, 'Actividades VISAE');
	...
	while ($fila = $objDB->sf($tabla)) {
		$et_visa57idpersemanal = '';
		if ($fila['visa57idpersemanal'] != 0) {
			if (isset($avisa57idpersemanal[$fila['visa57idpersemanal']]) == 0) {
				$sDato = '{' . $fila['visa57idpersemanal'] . '}';
				...
				if ($objDB->nf($tablad) > 0) {
					...
					$sDato = $filad['visa56fechaini'];
				}
				$avisa57idpersemanal[$fila['visa57idpersemanal']] = $sDato;
			}
			$et_visa57idpersemanal = $avisa57idpersemanal[$fila['visa57idpersemanal']];
		}
		PHPEXCEL_Escribir($objHoja, 0, $iFila, $et_visa57idpersemanal);
		$et_visa57idsistema = '';
		if ($fila['visa57idsistema'] != 0) {
			if (isset($avisa57idsistema[$fila['visa57idsistema']]) == 0) {
				$sDato = '{' . $fila['visa57idsistema'] . '}';
				$sSQL = 'SELECT visa55nombre FROM visa55sistema WHERE visa55id=' . $fila['visa57idsistema'] . '';
				$tablad = $objDB->ejecutasql($sSQL);
				if ($objDB->nf($tablad) > 0) {
					$filad = $objDB->sf($tablad);
					$sDato = $filad['visa55nombre'];
				}
				$avisa57idsistema[$fila['visa57idsistema']] = $sDato;
			}
			$et_visa57idsistema = $avisa57idsistema[$fila['visa57idsistema']];
		}
		PHPEXCEL_Escribir($objHoja, 1, $iFila, $et_visa57idsistema);
		PHPEXCEL_Escribir($objHoja, 2, $iFila, $fila['visa57consec']);
		PHPEXCEL_Escribir($objHoja, 3, $iFila, $fila['visa57titulo']);
		PHPEXCEL_Escribir($objHoja, 4, $iFila, $fila['visa57descripcion']);
		$et_visa57tipoactividad = 'Si';
		if ($fila['visa57tipoactividad'] == 0) {
			$et_visa57tipoactividad = 'No';
		}
		PHPEXCEL_Escribir($objHoja, 5, $iFila, $et_visa57tipoactividad);
		PHPEXCEL_Escribir($objHoja, 6, $iFila, $fila['visa57estado']);
		$et_visa57prioridad = 'Si';
		if ($fila['visa57prioridad'] == 0) {
			$et_visa57prioridad = 'No';
		}
		PHPEXCEL_Escribir($objHoja, 7, $iFila, $et_visa57prioridad);
		PHPEXCEL_Escribir($objHoja, 8, $iFila, $fila['visa57fechaprogini']);
		PHPEXCEL_Escribir($objHoja, 9, $iFila, $fila['visa57fechaprogfin']);
		PHPEXCEL_Escribir($objHoja, 10, $iFila, $fila['visa57fechaejecini']);
		PHPEXCEL_Escribir($objHoja, 11, $iFila, $fila['visa57fechaejecfin']);
		PHPEXCEL_Escribir($objHoja, 12, $iFila, $fila['visa57porcavance']);
		PHPEXCEL_Escribir($objHoja, 13, $iFila, $fila['visa57fechacrea']);
		PHPEXCEL_Escribir($objHoja, 14, $iFila, $fila['visa57fechaactualiza']);
		$iFila++;
	}
