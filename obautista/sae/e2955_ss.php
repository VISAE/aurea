...
if ($sError == '') {
	...
	list($bEntra, $sDebugP) = seg_revisa_permisoV3(2955, 6, $idTercero, $objDB);
	if (!$bEntra) {
		$sError = 'No tiene permiso para consultar este reporte [Mod 2955 : 6]';
	}
}
if ($sError == '') {
...
if ($sError == '') {
	...
	$sConsulta = 'FROM visa55sistema AS TB, plan04proyecto AS T2, unad11terceros AS T4 
...
if ($sError == '') {
	...
	$objExcel->getProperties()->setDescription('Reporte 2955 del SII 5.0 en ' . $sServerRpt . ' creado en ' . fecha_hoy() . ' ' . formato_horaminuto(fecha_hora(), fecha_minuto()));
	...
	$iFila = 12;
	PHPEXCEL_Escribir($objHoja, 0, $iFila, 'Sistemas VISAE');
	...
	while ($fila = $objDB->sf($tabla)) {
		$et_visa55idproyecto = '';
		if ($fila['visa55idproyecto'] != 0) {
			if (isset($avisa55idproyecto[$fila['visa55idproyecto']]) == 0) {
				$sDato = '{' . $fila['visa55idproyecto'] . '}';
				...
				if ($objDB->nf($tablad) > 0) {
					...
					$sDato = $filad['plan04numero'];
				}
				$avisa55idproyecto[$fila['visa55idproyecto']] = $sDato;
			}
			$et_visa55idproyecto = $avisa55idproyecto[$fila['visa55idproyecto']];
		}
		PHPEXCEL_Escribir($objHoja, 0, $iFila, $et_visa55idproyecto);
		PHPEXCEL_Escribir($objHoja, 1, $iFila, $fila['visa55codigo']);
		PHPEXCEL_Escribir($objHoja, 2, $iFila, $fila['visa55nombre']);
		PHPEXCEL_Escribir($objHoja, 3, $iFila, $fila['visa55descripcion']);
		PHPEXCEL_Escribir($objHoja, 4, $iFila, $fila['visa55idresponsable']);
		$et_visa55vigente = 'Si';
		if ($fila['visa55vigente'] == 0) {
			$et_visa55vigente = 'No';
		}
		PHPEXCEL_Escribir($objHoja, 5, $iFila, $et_visa55vigente);
		$et_visa55color = 'Si';
		if ($fila['visa55color'] == 0) {
			$et_visa55color = 'No';
		}
		PHPEXCEL_Escribir($objHoja, 6, $iFila, $et_visa55color);
		PHPEXCEL_Escribir($objHoja, 7, $iFila, $fila['visa55fechacreacion']);
		PHPEXCEL_Escribir($objHoja, 8, $iFila, $fila['visa55fechaactualiza']);
		$iFila++;
	}
