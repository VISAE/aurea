...
switch ($datos[0]) {
	...
	if ($bDebug) {echo 'Enviando visa37convsubtipo a ' . $idEntidad . ' ' . $sIdTercero . ' ' . $sIdMovil . ' ' . $sListaIds . '<br>';
	}
	...
	if (($sError == '') && ($sListaIds!=-99)) {
		...
		if ($sIds == '') {
			...
			$sCondicion = ' WHERE visa37id IN (' . $sIds . ')';
		}
	}
	if ($sError == '') {
		$data1 = array();
		...
		$sSQL = 'SELECT visa37idtipologia, visa37id, visa37nombre, visa37activo FROM visa37convsubtipo' . $sCondicion;
		if ($bDebug) {
		...
		while ($fila = $objDB->sf($tabla)) {
			$data1[] = $fila;
			$sIdsPadre = $sIdsPadre . ', ' . $fila['visa37id'];
			$iTotal++;
		}
		$res = array();
		$res[] = $iTotal;
