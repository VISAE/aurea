...
function f2908_ExisteDato($datos){
	...
	$bHayLlave=true;
	$plab08consecutivo=numeros_validar($datos[1]);
	...
	if ($bHayLlave){
		...
		$objDB->xajax();
		$sSQL='SELECT plab08consecutivo FROM plab08emprbolsempleo WHERE plab08consecutivo='.$plab08consecutivo.'';
...
<div class="salto1px"></div>
	...
	*/
	$sTitulos='Consecutivo, Id, Nombre, Activo, Fechainicontr, Fechafincontr';
	$sSQL='SELECT TB.plab08consecutivo, TB.plab08id, TB.plab08nombre, TB.plab08activo, TB.plab08fechainicontr, TB.plab08fechafincontr 
FROM plab08emprbolsempleo AS TB 
WHERE '.$sSQLadd1.'  '.$sSQLadd.'
ORDER BY TB.plab08consecutivo';
...
function f2908_db_CargarPadre($DATA, $objDB, $bDebug=false){
	...
	require './app.php';
	if ($DATA['paso']==1){
		$sSQLcondi='plab08consecutivo='.$DATA['plab08consecutivo'].'';
		}else{
	...
	if ($objDB->nf($tabla)>0){
		...
		$DATA['plab08nombre']=$fila['plab08nombre'];
...
function f2908_db_GuardarV2($DATA, $objDB, $bDebug=false){
	...
	if (isset($DATA['plab08nombre'])==0){$DATA['plab08nombre']='';}
	...
	*/
	...
	$DATA['plab08nombre']=htmlspecialchars(trim($DATA['plab08nombre']));
	...
	if (true){
		...
		if ($DATA['plab08nombre']==''){$sError=$ERR['plab08nombre'].$sSepara.$sError;}
	...
	if ($sError==''){
		if ($DATA['paso']==10){
			if ($DATA['plab08consecutivo']==''){
				$DATA['plab08consecutivo']=tabla_consecutivo('plab08emprbolsempleo', 'plab08consecutivo', '', $objDB);
			...
			if ($sError==''){
				$sSQL='SELECT 1 FROM plab08emprbolsempleo WHERE plab08consecutivo='.$DATA['plab08consecutivo'].'';
	...
	if ($sError==''){
		...
		if ($DATA['paso']==10){
			...
			$sCampos2908='plab08consecutivo, plab08id, plab08nombre, plab08activo, plab08fechainicontr, plab08fechafincontr';
			$sValores2908=''.$DATA['plab08consecutivo'].', '.$DATA['plab08id'].', "'.$DATA['plab08nombre'].'", "'.$DATA['plab08activo'].'", "'.$DATA['plab08fechainicontr'].'", "'.$DATA['plab08fechafincontr'].'"';
			if ($APP->utf8==1){
			...
			$scampo[1]='plab08nombre';
			$scampo[2]='plab08activo';
			$scampo[3]='plab08fechainicontr';
			$scampo[4]='plab08fechafincontr';
			$sdato[1]=$DATA['plab08nombre'];
			$sdato[2]=$DATA['plab08activo'];
			$sdato[3]=$DATA['plab08fechainicontr'];
			$sdato[4]=$DATA['plab08fechafincontr'];
			$numcmod=4;
...
function f2908_db_Eliminar($plab08id, $objDB, $bDebug=false){
	...
	if ($sError==''){
		$sSQL='SELECT plab03idbolsa FROM plab03rangsala WHERE plab03idbolsa='.$filabase['plab08id'].'';
		$tabla=$objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla)>0){
	...
	if ($sError==''){
		//$sSQL='DELETE FROM plab03rangsala WHERE plab03idbolsa='.$filabase['plab08id'].'';
		//$tabla=$objDB->ejecutasql($sSQL);
		$sWhere='plab08id='.$plab08id.'';
		//$sWhere='plab08consecutivo='.$filabase['plab08consecutivo'].'';
