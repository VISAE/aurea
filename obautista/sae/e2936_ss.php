...
if ($sError == '') {
	...
	list($bEntra, $sDebugP) = seg_revisa_permisoV3(2936, 6, $idTercero, $objDB);
	if (!$bEntra) {
		$sError = 'No tiene permiso para consultar este reporte [Mod 2936 : 6]';
	}
}
if ($sError == '') {
...
if ($sError == '') {
	...
	$objExcel->getProperties()->setDescription('Reporte 2936 del SII 5.0 en ' . $sServerRpt . ' creado en ' . fecha_hoy() . ' ' . formato_horaminuto(fecha_hora(), fecha_minuto()));
	...
	$iFila = 12;
	PHPEXCEL_Escribir($objHoja, 0, $iFila, 'Tipologias de convocatoria');
	...
	while ($fila = $objDB->sf($tabla)) {
		$et_visa36idgrupo = '';
		if ($fila['visa36idgrupo'] != 0) {
			if (isset($avisa36idgrupo[$fila['visa36idgrupo']]) == 0) {
				$sDato = '{' . $fila['visa36idgrupo'] . '}';
				...
				$avisa36idgrupo[$fila['visa36idgrupo']] = $sDato;
			}
			$et_visa36idgrupo = $avisa36idgrupo[$fila['visa36idgrupo']];
		}
		PHPEXCEL_Escribir($objHoja, 0, $iFila, $et_visa36idgrupo);
		PHPEXCEL_Escribir($objHoja, 1, $iFila, $fila['visa36consec']);
		PHPEXCEL_Escribir($objHoja, 2, $iFila, $fila['visa36nombre']);
		$et_visa36activo = 'Si';
		if ($fila['visa36activo'] == 0) {
			$et_visa36activo = 'No';
		}
		PHPEXCEL_Escribir($objHoja, 3, $iFila, $et_visa36activo);
		$iFila++;
	}
