...
switch ($datos[0]) {
	...
	if ($bDebug) {echo 'Enviando visa55sistema a ' . $idEntidad . ' ' . $sIdTercero . ' ' . $sIdMovil . ' ' . $sListaIds . '<br>';
	}
	...
	if (($sError == '') && ($sListaIds!=-99)) {
		...
		if ($sIds == '') {
			...
			$sCondicion = ' WHERE visa55id IN (' . $sIds . ')';
		}
	}
	if ($sError == '') {
		$data1 = array();
		...
		$sSQL = 'SELECT visa55idproyecto, visa55codigo, visa55id, visa55nombre, visa55descripcion, visa55idresponsable, visa55vigente, visa55color, visa55fechacreacion, visa55fechaactualiza FROM visa55sistema' . $sCondicion;
		if ($bDebug) {
		...
		while ($fila = $objDB->sf($tabla)) {
			$data1[] = $fila;
			$sIdsPadre = $sIdsPadre . ', ' . $fila['visa55id'];
			$iTotal++;
		}
		$res = array();
		$res[] = $iTotal;
