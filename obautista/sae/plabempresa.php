...
if (($_REQUEST['paso']==1)||($_REQUEST['paso']==3)){
	...
	if ($_REQUEST['paso']==1){
		$sSQLcondi='plab01idtercero="'.$_REQUEST['plab01idtercero'].'"';
		}else{
...
function RevisaLlave(){
	var datos= new Array();
	datos[1]=window.document.frmedita.plab01idtercero.value;
	if ((datos[1]!='')){
		xajax_f2909_ExisteDato(datos);
		}
	}
function cargadato(llave1){
	window.document.frmedita.plab01idtercero.value=String(llave1);
	window.document.frmedita.paso.value=1;
	window.document.frmedita.submit();
	}
...
function objinicial(){
	document.getElementById("plab01idtercero").focus();
	}
