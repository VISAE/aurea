...
*/
/** Archivo visaegrupotipologia.php.
 * Modulo 2946 visa46grupotipologia.
...
if (!$bPeticionXAJAX) {
	if (noticias_pendientes($objDB)) {
		$objDB->CerrarConexion();
		header('Location:noticia.php?ret=visaegrupotipologia.php');
		die();
	}
}
$seg_1707 = 0;
...
$xajax->register(XAJAX_FUNCTION, 'f2946_HtmlTabla');
$xajax->register(XAJAX_FUNCTION, 'f2946_ExisteDato');
$xajax->processRequest();
if ($bPeticionXAJAX) {
...
if (isset($_REQUEST['paginaf2946']) == 0) {
	$_REQUEST['paginaf2946'] = 1;
}
if (isset($_REQUEST['lppf2946']) == 0) {
	$_REQUEST['lppf2946'] = 20;
}
if (isset($_REQUEST['boculta2946']) == 0) {
	$_REQUEST['boculta2946'] = 0;
}
// -- Inicializar variables de datos.
if (isset($_REQUEST['visa46consec']) == 0) {
	$_REQUEST['visa46consec'] = '';
}
if (isset($_REQUEST['visa46consec_nuevo']) == 0) {
	$_REQUEST['visa46consec_nuevo'] = '';
}
if (isset($_REQUEST['visa46id']) == 0) {
	$_REQUEST['visa46id'] = '';
}
if (isset($_REQUEST['visa46nombre']) == 0) {
	$_REQUEST['visa46nombre'] = '';
}
$_REQUEST['visa46consec'] = numeros_validar($_REQUEST['visa46consec']);
$_REQUEST['visa46id'] = numeros_validar($_REQUEST['visa46id']);
$_REQUEST['visa46nombre'] = cadena_Validar($_REQUEST['visa46nombre']);
...
if (($_REQUEST['paso'] == 1) || ($_REQUEST['paso'] == 3)) {
	if ($_REQUEST['paso'] == 1) {
		$sSQLcondi = 'visa46consec=' . $_REQUEST['visa46consec'] . '';
	} else {
		$sSQLcondi = 'visa46id=' . $_REQUEST['visa46id'] . '';
	}
	$sSQL = 'SELECT * FROM visa46grupotipologia WHERE ' . $sSQLcondi;
	$tabla = $objDB->ejecutasql($sSQL);
	if ($objDB->nf($tabla) > 0) {
		$fila = $objDB->sf($tabla);
		$_REQUEST['visa46consec'] = $fila['visa46consec'];
		$_REQUEST['visa46id'] = $fila['visa46id'];
		$_REQUEST['visa46nombre'] = $fila['visa46nombre'];
		$bcargo = true;
		$_REQUEST['paso'] = 2;
		$_REQUEST['boculta2946'] = 0;
		$bLimpiaHijos = true;
...
if (($_REQUEST['paso'] == 10) || ($_REQUEST['paso'] == 12)) {
	$bMueveScroll = true;
	list($_REQUEST, $sError, $iTipoError, $sDebugGuardar) = f2946_db_GuardarV2b($_REQUEST, $objDB, $bDebug, $idTercero);
...
if ($_REQUEST['paso'] == 93) {
	$_REQUEST['paso'] = 2;
	$_REQUEST['visa46consec_nuevo'] = numeros_validar($_REQUEST['visa46consec_nuevo']);
	if ($_REQUEST['visa46consec_nuevo'] == '') {
		$sError = $ERR['visa46consec'];
	}
	if ($sError == '') {
	...
	if ($sError == '') {
		...
		$sSQL = 'SELECT visa46id FROM visa46grupotipologia WHERE visa46consec=' . $_REQUEST['visa46consec_nuevo'] . '';
		$tabla = $objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla) > 0) {
			$sError = 'El consecutivo ' . $_REQUEST['visa46consec_nuevo'] . ' ya existe';
		}
	}
	if ($sError == '') {
		//Aplicar el cambio.
		$sSQL = 'UPDATE visa46grupotipologia SET visa46consec=' . $_REQUEST['visa46consec_nuevo'] . ' WHERE visa46id=' . $_REQUEST['visa46id'] . '';
		$tabla = $objDB->ejecutasql($sSQL);
		$sDetalle = 'Cambia el consecutivo de ' . $_REQUEST['visa46consec'] . ' a ' . $_REQUEST['visa46consec_nuevo'] . '';
		$_REQUEST['visa46consec'] = $_REQUEST['visa46consec_nuevo'];
		$_REQUEST['visa46consec_nuevo'] = '';
		seg_auditar($iCodModulo, $_SESSION['unad_id_tercero'], 8, $_REQUEST['visa46id'], $sDetalle, $objDB);
...
if ($_REQUEST['paso'] == 13) {
	$_REQUEST['paso'] = 2;
	if ($sError == '') {
		list($sError, $iTipoError, $sDebugElimina) = f2946_db_Eliminar($_REQUEST['visa46id'], $objDB, $bDebug);
...
$objCombos->sAccion = 'paginarf2946()';
$sSQL = '';
$html_blistar = $objCombos->html($sSQL, $objDB);
//$html_blistar = $objCombos->comboSistema(2946, 1, $objDB, 'paginarf2946()');
*/
if (false) {
...
$aParametros[0] = ''; //$_REQUEST['p1_2946'];
$aParametros[100] = $idTercero;
$aParametros[101] = $_REQUEST['paginaf2946'];
$aParametros[102] = $_REQUEST['lppf2946'];
//$aParametros[103] = $_REQUEST['bnombre'];
//$aParametros[104] = $_REQUEST['blistar'];
list($sTabla2946, $sDebugTabla) = f2946_TablaDetalleV2($aParametros, $objDB, $bDebug);
...
?>
	...
	function imprimelista() {
		if (window.document.frmedita.seg_6.value == 1) {
			window.document.frmlista.consulta.value = window.document.frmedita.consulta_2946.value;
			window.document.frmlista.titulos.value = window.document.frmedita.titulos_2946.value;
			window.document.frmlista.nombrearchivo.value = 'Grupos de tipologias';
	...
	function imprimeexcel() {
		...
		if (sError == '') {
			asignarvariables();
			window.document.frmimpp.action = 'e2946_ss.php';
	...
	function imprimep() {
		if (window.document.frmedita.seg_5.value == 1) {
			asignarvariables();
			window.document.frmimpp.action = 'p2946.php';
			window.document.frmimpp.submit();
		} else {
	...
	function RevisaLlave() {
		let datos = new Array();
		datos[1] = window.document.frmedita.visa46consec.value;
		if ((datos[1] != '')) {
			xajax_f2946_ExisteDato(datos);
		}
	}

	function cargadato(llave1) {
		window.document.frmedita.visa46consec.value = String(llave1);
		window.document.frmedita.paso.value = 1;
		window.document.frmedita.submit();
	}

	function cargaridf2946(llave1) {
		window.document.frmedita.visa46id.value = String(llave1);
		window.document.frmedita.paso.value = 3;
		window.document.frmedita.submit();
	}

	function paginarf2946() {
		let params = new Array();
		params[99] = window.document.frmedita.debug.value;
		params[100] = <?php echo $idTercero; ?>;
		params[101] = window.document.frmedita.paginaf2946.value;
		params[102] = window.document.frmedita.lppf2946.value;
		//params[103] = window.document.frmedita.bnombre.value;
		//params[104] = window.document.frmedita.blistar.value;
		document.getElementById('div_f2946detalle').innerHTML = '<div class="GrupoCamposAyuda"><div class="MarquesinaMedia">Procesando datos, por favor espere.</div></div><input id="paginaf2946" name="paginaf2946" type="hidden" value="' + params[101] + '" /><input id="lppf2946" name="lppf2946" type="hidden" value="' + params[102] + '" />';
		xajax_f2946_HtmlTabla(params);
	}
	...
	function objinicial() {
		document.getElementById("visa46consec").focus();
	}
...
?>
<form id="frmimpp" name="frmimpp" method="post" action="p2946.php" target="_blank" style="display:none">
<input id="r" name="r" type="hidden" value="2946" />
<input id="id2946" name="id2946" type="hidden" value="<?php echo $_REQUEST['visa46id']; ?>" />
...
<?php
echo $objForma->htmlExpande(2946, $_REQUEST['boculta2946'], $ETI['bt_mostrar'], $ETI['bt_ocultar']);
$sEstiloDiv = '';
if ($_REQUEST['boculta2946'] != 0) {
	$sEstiloDiv = ' style="display:none;"';
}
?>
</div>
<div id="div_p2946"<?php echo $sEstiloDiv; ?>>
<?php
}
...
if ($_REQUEST['paso'] != 2) {
?>
<input id="visa46consec" name="visa46consec" type="text" value="<?php echo $_REQUEST['visa46consec']; ?>" onchange="RevisaLlave()" class="cuatro" />
<?php
} else {
	echo html_oculto('visa46consec', $_REQUEST['visa46consec'], formato_numero($_REQUEST['visa46consec']));
}
?>
</label>
<?php
...
<label class="Label60"<?php echo $sOcultaId; ?>>
<?php
	echo html_oculto('visa46id', $_REQUEST['visa46id'], formato_numero($_REQUEST['visa46id']));
?>
</label>
<label class="Label130">
<?php
echo $ETI['visa46nombre'];
?>
</label>
<label>

<input id="visa46nombre" name="visa46nombre" type="text" value="<?php echo $_REQUEST['visa46nombre']; ?>" maxlength="50" placeholder="<?php echo $ETI['ing_campo'] . $ETI['visa46nombre']; ?>" />
</label>
<?php
if (false) {
...
if ($bConExpande) {
	//Este es el cierre del div_p2946
?>
<div class="salto1px"></div>
</div>
<?php
...
<label>
<input id="bnombre" name="bnombre" type="text" value="<?php echo $_REQUEST['bnombre']; ?>" onchange="paginarf2946()" autocomplete="off" />
</label>
<label class="Label90">
<?php
...
<label class="Label160">
<?php
echo $ETI['msg_visa46consec'];
?>
</label>
<label class="Label90">
<?php
echo '<b>' . $_REQUEST['visa46consec'] . '</b>';
?>
</label>
<div class="salto1px"></div>
<label class="Label160">
<?php
echo $ETI['msg_visa46consec_nuevo'];
// onchange="RevisaConsec()"
?>
</label>
<label class="Label90">
<input id="visa46consec_nuevo" name="visa46consec_nuevo" type="text" value="<?php echo $_REQUEST['visa46consec_nuevo']; ?>" class="cuatro" />
</label>
<div class="salto1px"></div>
...
<input id="titulo_2946" name="titulo_2946" type="hidden" value="<?php echo $sTituloModulo; ?>" />
<?php
if ($bBloqueTitulo) {
?>
<div class="titulos">
