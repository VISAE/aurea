...
function f2903_db_Guardar($valores, $objDB, $bDebug=false){
	...
	$plab03idbolsa=numeros_validar($valores[1]);
	...
	if ($plab03idbolsa==''){$sError=$ERR['plab03idbolsa'].$sSepara.$sError;}
	if ($sError==''){
		if ((int)$plab03id==0){
			if ((int)$plab03consecutivo==0){
				$plab03consecutivo=tabla_consecutivo('plab03rangsala', 'plab03consecutivo', 'plab03idbolsa='.$plab03idbolsa.'', $objDB);
				if ($plab03consecutivo==-1){$sError=$objDB->serror;}
				}else{
				if (!seg_revisa_permiso($iCodModulo, 8, $objDB)){$sError=$ERR['8'];}
				}
			if ($sError==''){
				$sSQL='SELECT plab03idbolsa FROM plab03rangsala WHERE plab03idbolsa='.$plab03idbolsa.' AND plab03consecutivo='.$plab03consecutivo.'';
	...
	if ($sError==''){
		if ($binserta){
			$scampos='plab03idbolsa, plab03consecutivo, plab03id, plab03activo, plab03nombre';
			$svalores=''.$plab03idbolsa.', '.$plab03consecutivo.', '.$plab03id.', "'.$plab03activo.'", "'.$plab03nombre.'"';
			if ($APP->utf8==1){
			...
			//$sWhere='plab03idbolsa='.$plab03idbolsa.' AND plab03consecutivo='.$plab03consecutivo.'';
...
function f2903_db_Eliminar($aParametros, $objDB, $bDebug=false){
	...
	$plab03idbolsa=numeros_validar($aParametros[1]);
...
function f2903_TablaDetalleV2($aParametros, $objDB, $bDebug=false){
	...
	$sDebug='';
	$plab08id=$aParametros[0];
	$pagina=$aParametros[101];
	$lineastabla=$aParametros[102];
	$babierta=true;
	//$sSQL='SELECT Campo FROM plab08emprbolsempleo WHERE plab08id='.$plab08id;
	//$tabla=$objDB->ejecutasql($sSQL);
	//if ($objDB->nf($tabla)>0){
		//$fila=$objDB->sf($tabla);
...
<div class="salto1px"></div>
	...
	*/
	$sTitulos='Bolsa, Consecutivo, Id, Activo, Nombre';
	$sSQL='SELECT TB.plab03idbolsa, TB.plab03consecutivo, TB.plab03id, TB.plab03activo, TB.plab03nombre 
FROM plab03rangsala AS TB 
WHERE '.$sSQLadd1.' TB.plab03idbolsa='.$plab08id.' '.$sSQLadd.'
...
function f2903_Clonar($plab03idbolsa, $plab03idbolsaPadre, $objDB){
	$sError='';
	$plab03consecutivo=tabla_consecutivo('plab03rangsala', 'plab03consecutivo', 'plab03idbolsa='.$plab03idbolsa.'', $objDB);
	if ($plab03consecutivo==-1){$sError=$objDB->serror;}
	$plab03id=tabla_consecutivo('plab03rangsala', 'plab03id', '', $objDB);
	if ($plab03id==-1){$sError=$objDB->serror;}
	if ($sError==''){
		$sCampos2903='plab03idbolsa, plab03consecutivo, plab03id, plab03activo, plab03nombre';
		$sValores2903='';
		$sSQL='SELECT * FROM plab03rangsala WHERE plab03idbolsa='.$plab03idbolsaPadre.'';
		$tabla=$objDB->ejecutasql($sSQL);
		while($fila=$objDB->sf($tabla)){
			if ($sValores2903!=''){$sValores2903=$sValores2903.', ';}
			$sValores2903=$sValores2903.'('.$plab03idbolsa.', '.$plab03consecutivo.', '.$plab03id.', "'.$fila['plab03activo'].'", "'.$fila['plab03nombre'].'")';
			$plab03consecutivo++;
			$plab03id++;
			}
		if ($sValores2903!=''){
...
function f2903_Traer($aParametros){
	...
	if ($paso==1){
		$plab03idbolsa=numeros_validar($aParametros[1]);
		$plab03consecutivo=numeros_validar($aParametros[2]);
		if (($plab03idbolsa!='')&&($plab03consecutivo!='')){$besta=true;}
		}else{
	...
	if ($besta){
		...
		$sSQLcondi='';
		if ($paso==1){
			$sSQLcondi=$sSQLcondi.'plab03idbolsa='.$plab03idbolsa.' AND plab03consecutivo='.$plab03consecutivo.'';
			}else{
