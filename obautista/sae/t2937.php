...
*/
/*
/** Archivo para reportes tipo csv 2937.
...
if ($sError == '') {
	...
	*/
	...
	$visa37idtipologia_lg = 'Idtipologia';
	$visa37id_lg = 'Ref';
	$visa37nombre_lg = 'Nombre';
	$visa37activo_lg = 'Activo';
	$objDB = new clsdbadmin($APP->dbhost, $APP->dbuser, $APP->dbpass, $APP->dbname);
	if ($APP->dbpuerto != '') {
		$objDB->dbPuerto = $APP->dbpuerto;
	}
	// ----------- Espacio para los parametros.
	$sCondi = 'WHERE visa37idtipologia=' . $DATA['visa37idtipologia'] . '';
	// ----------- Fin del bloque de parametros.
	$sPath = dirname(__FILE__);
	$sSeparador = archivos_separador($sPath);
	$sPath = archivos_rutaservidor($sPath, $sSeparador);
	$sNombrePlano = 't2937.csv';
	$sTituloRpt = 'visa37convsubtipo';
	...
	$sDato = cadena_codificar('visa37convsubtipo');
	$objplano->AdicionarLinea($sDato);
	$sDato = '';
	$objplano->AdicionarLinea($sDato);
	...
	$sBloque1 = '' . $visa37idtipologia_lg . $cSepara . $visa37nombre_lg . $cSepara . $visa37activo_lg;
	//$objplano->AdicionarLinea($sTitulo1);
	$objplano->AdicionarLinea($sBloque1);
	$sCampos = 'SELECT TB.*
	$sConsulta = 'FROM visa37convsubtipo 
	' . $sCondi . '';
	$sOrden = '';
	...
	while ($fila = $objDB->sf($tabla)) {
		$lin_visa37idtipologia = '';
		$lin_visa37nombre = $cSepara;
		$lin_visa37activo = $cSepara;
		$lin_visa37idtipologia = $fila['visa37idtipologia'];
		$lin_visa37nombre = $cSepara . str_replace($cSepara, $cComplementa, cadena_codificar($fila['visa37nombre']));
		$lin_visa37activo = $cSepara . '[' . $fila['visa37activo'] . ']';
		if (isset($avisa37activo[$fila['visa37activo']]) != 0) {
			$lin_visa37activo = $cSepara . cadena_codificar($avisa37activo[$fila['visa37activo']]);
		}
		$sBloque1 = '' . $lin_visa37idtipologia . $lin_visa37nombre . $lin_visa37activo;
		$objplano->AdicionarLinea($sBloque1);
	}
