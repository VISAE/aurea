...
--- Modelo Version 2.23.2 lunes, 10 de junio de 2019
*/
...
class clsPDF extends FPDF{
	...
	function Header(){
		...
		if ($this->iReporte==214){
			//Ubique aqui los componentes adicionales del encabezado
			$this->Cell($this->iAnchoLibre,5,'Registro de actividades recurrentes '.$this->sRefRpt, 0, 0, 'C');
			$this->Ln();
			}
		$yPos=$this->GetY();
	...
	function ArmarReporte214($PARAMS, $objDB){
		...
		$sSQL='SELECT * FROM unae14logcron WHERE unae14id='.$PARAMS['id214'].'';
		if ($this->bDebug){p_AddDebug('Consulta para el reporte '.$sSQL, $this);}
		$tabla=$objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla)>0){
			$fila=$objDB->sf($tabla);
			$this->Cell($this->iAnchoLibre, 5, 'Fecha: '.$fila['unae14fecha']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Consec: '.$fila['unae14consec']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Minuto: '.$fila['unae14minuto']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Minutofin: '.$fila['unae14minutofin']);
			$this->Ln();
			$sTitulo='Actividades ejecutadas';
			//$sNumera=p_AddIndice($sTitulo, $this->PageNo(), 1, $this);
			p_TituloBloque($sTitulo, $this);
			p_FuenteNormalV2($this);
			$sSQL='SELECT * FROM unae15cronregistro WHERE unae15idlogcron='.$fila['unae14id'].'';
			...
			if ($objDB->nf($tabla1)>0){
				...
				$this->Cell($c4, 5, 'Accion');
				$this->Cell($c5, 5, 'Detalle');
				$this->Cell($c6, 5, 'Minuto');
				$this->Ln();
				}
			while($fila1=$objDB->sf($tabla1)){
				$this->Cell($c1, 5, $fila1['unae15consec']);
				$sunae15idaccion=$fila1['unae15idaccion'];
				$sSQL='SELECT unae16accion FROM unae16cronaccion WHERE unae16id='.$fila1['unae15idaccion'];
				$tablat=$objDB->ejecutasql($sSQL);
				if ($objDB->nf($tablat)>0){
					$filat=$objDB->sf($tablat);
					$sunae15idaccion=$filat['unae16accion'];
					if ($this->bCodificar){$sunae15idaccion=utf8_encode($filat['unae16accion']);}
					}
				$this->Cell($c2, 5, utf8_decode($sunae15idaccion));
				$this->Cell($c3, 5, $fila1['unae15detalle']);
				$this->Cell($c4, 5, $fila1['unae15minuto']);
				$this->Ln();
				}
			p_Separador($this);
			}
		// Fin de ArmarReporte214
		}
	// Fin de clsPDF
	}
...
WHERE TB.sys94id='.$fila['che73idformato'].' AND TB.sys94formatopag=T1.sys57id';
		...
		if ($bConCFG){
			require './app.php';
			$sRutaConfig=$APP->rutacomun.'pcfg.php';
...
WHERE T1.sys57id='.$idFormatoPag.'';
		...
		$objpdf->AddPage();
		$objpdf->ArmarReporte214($PARAMS, $objDB);
		//$objpdf->AddPage();
...
if ($bEntra){
	...
	$bEntra=false;
	$sTituloRpt='Reporte';
	if ($iReporte==214){$sTituloRpt='Registro de actividades recurrentes';}
