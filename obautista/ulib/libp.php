...
function p_TituloEntidad($objpdf, $bConSeparador=true){
	$objpdf->SetFont('Arial','B',14);
	$objpdf->Cell($objpdf->iAnchoLibre,5,utf8_decode('Universidad Nacional Abierta y a Distancia - UNAD'),0,0,'C');
	$objpdf->Ln();
	$objpdf->Cell($objpdf->iAnchoLibre,5,utf8_decode('Nit: 860.512.780'),0,0,'C');
	$objpdf->Ln();
	$objpdf->SetFont('Arial','',12);
	$objpdf->Cell($objpdf->iAnchoLibre,5,utf8_decode('Calle 14 sur No. 14 - 23 - Bogotá'),0,0,'C');
	$objpdf->Ln();
	if ($bConSeparador){
		p_Separador($objpdf);
		}
