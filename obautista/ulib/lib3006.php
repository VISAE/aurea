...
--- 3006 Anotaciones
*/
function f3006_db_Guardar($valores, $objDB, $bDebug=false){
	$iCodModulo=3006;
	$bAudita[2]=false;
	$bAudita[3]=false;
	require './app.php';
	...
	if (!file_exists($mensajes_3006)){$mensajes_3006=$APP->rutacomun.'lg/lg_3006_es.php';}
	...
	$sSepara=', ';
	if ($saiu06minuto==''){$sError=$ERR['saiu06minuto'].$sSepara.$sError;}
	if ($saiu06hora==''){$sError=$ERR['saiu06hora'].$sSepara.$sError;}
	if ($saiu06fecha==0){
		//$saiu06fecha=fecha_DiaMod();
		$sError=$ERR['saiu06fecha'].$sSepara.$sError;
		}
	if ($saiu06idusuario==0){$sError=$ERR['saiu06idusuario'].$sSepara.$sError;}
	if ($saiu06descartada==''){$sError=$ERR['saiu06descartada'].$sSepara.$sError;}
	if ($saiu06visible==''){$sError=$ERR['saiu06visible'].$sSepara.$sError;}
	if ($saiu06anotacion==''){$sError=$ERR['saiu06anotacion'].$sSepara.$sError;}
	...
	if ($saiu06idsolicitud==''){$sError=$ERR['saiu06idsolicitud'].$sSepara.$sError;}
	if ($sError==''){
	...
	if ($sError==''){
		if ((int)$saiu06id==0){
			if ((int)$saiu06consec==0){
				$saiu06consec=tabla_consecutivo('saiu06solanotacion', 'saiu06consec', 'saiu06idsolicitud='.$saiu06idsolicitud.'', $objDB);
				if ($saiu06consec==-1){$sError=$objDB->serror;}
				}else{
				if (!seg_revisa_permiso($iCodModulo, 8, $objDB)){$sError=$ERR['8'];}
				}
			if ($sError==''){
				$sSQL='SELECT saiu06idsolicitud FROM saiu06solanotacion WHERE saiu06idsolicitud='.$saiu06idsolicitud.' AND saiu06consec='.$saiu06consec.'';
			...
			if ($sError==''){
				$saiu06id=tabla_consecutivo('saiu06solanotacion', 'saiu06id', '', $objDB);
				if ($saiu06id==-1){$sError=$objDB->serror;}
				$bInserta=true;
				$iAccion=2;
				}
			}else{
...
'.$saiu06hora.', '.$saiu06minuto.'';
			if ($APP->utf8==1){
				$sSQL='INSERT INTO saiu06solanotacion ('.$sCampos3005.') VALUES ('.utf8_encode($sValores3005).');';
				}else{
				$sSQL='INSERT INTO saiu06solanotacion ('.$sCampos3005.') VALUES ('.$sValores3005.');';
				}
			$result=$objDB->ejecutasql($sSQL);
			if ($result==false){
			...
			$sSQL='SELECT * FROM saiu06solanotacion WHERE '.$sWhere;
			$sdatos='';
			$bpasa=false;
			$result=$objDB->ejecutasql($sSQL);
			if ($objDB->nf($result)>0){
			...
			if ($bpasa){
				if ($APP->utf8==1){
					$sSQL='UPDATE saiu06solanotacion SET '.utf8_encode($sdatos).' WHERE '.$sWhere.';';
					}else{
					$sSQL='UPDATE saiu06solanotacion SET '.$sdatos.' WHERE '.$sWhere.';';
					}
				$result=$objDB->ejecutasql($sSQL);
				if ($result==false){
					$sError=$ERR['falla_guardar'].' {Anotaciones}. <!-- '.$sSQL.' -->';
...
function f3006_db_Eliminar($aParametros, $objDB, $bDebug=false){
	...
	if ($sError==''){
		...
		$sWhere='saiu06id='.$saiu06id.'';
		//$sWhere='saiu06idsolicitud='.$saiu06idsolicitud.' AND saiu06consec='.$saiu06consec.'';
		$sSQL='DELETE FROM saiu06solanotacion WHERE '.$sWhere.';';
		$result=$objDB->ejecutasql($sSQL);
		if ($result==false){
...
<input id="titulos_3006" name="titulos_3006" type="hidden" value="'.$sTitulos.'"/>';
	...
	if ($tabladetalle==false){
		if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Consulta 3006: '.$sSQL.'<br>';}
		$registros=0;
...
'.html_lpp('lppf3006', $lineastabla, 'paginarf3006()').'
	...
	while($filadet=$objDB->sf($tabladetalle)){
		...
		if ($filadet['saiu06visible']=='S'){$et_saiu06visible=$sPrefijo.$ETI['si'].$sSufijo;}
		$et_saiu06descartada=$sPrefijo.$ETI['no'].$sSufijo;
		if ($filadet['saiu06descartada']=='S'){$et_saiu06descartada=$sPrefijo.$ETI['si'].$sSufijo;}
		$et_saiu06idarchivo='';
		...
		$et_saiu06fecha='';
		if ($filadet['saiu06fecha']!=0){$et_saiu06fecha=$sPrefijo.fecha_desdenumero($filadet['saiu06fecha']).$sSufijo;}
...
function f3006_Clonar($saiu06idsolicitud, $saiu06idsolicitudPadre, $objDB){
	$sError='';
	$saiu06consec=tabla_consecutivo('saiu06solanotacion', 'saiu06consec', 'saiu06idsolicitud='.$saiu06idsolicitud.'', $objDB);
	if ($saiu06consec==-1){$sError=$objDB->serror;}
	$saiu06id=tabla_consecutivo('saiu06solanotacion', 'saiu06id', '', $objDB);
	...
	if ($sError==''){
		...
		$sValores3006='';
		$sSQL='SELECT * FROM saiu06solanotacion WHERE saiu06idsolicitud='.$saiu06idsolicitudPadre.'';
		$tabla=$objDB->ejecutasql($sSQL);
		while($fila=$objDB->sf($tabla)){
			if ($sValores3006!=''){$sValores3006=$sValores3006.', ';}
		...
		if ($sValores3006!=''){
			$sSQL='INSERT INTO saiu06solanotacion('.$sCampos3006.') VALUES '.$sValores3006.'';
			$result=$objDB->ejecutasql($sSQL);
			}
		}
	return $sError;
...
function elimina_archivo_saiu06idarchivo($idpadre){
	$_SESSION['u_ultimominuto']=iminutoavance();
	require './app.php';
	$objDB=new clsdbadmin($APP->dbhost, $APP->dbuser, $APP->dbpass, $APP->dbname);
	if ($APP->dbpuerto!=''){$objDB->dbPuerto=$APP->dbpuerto;}
	$objDB->xajax();
	archivo_eliminar('saiu06solanotacion', 'saiu06id', 'saiu06idorigen', 'saiu06idarchivo', $idpadre, $objDB);
...
function f3006_Guardar($valores, $aParametros){
	...
	if ($sError==''){
		...
		$objDB->xajax();
		$bHayDb=true;
		list($sError, $iAccion, $saiu06id, $sDebugGuardar)=f3006_db_Guardar($valores, $objDB, $bDebug);
...
function f3006_Traer($aParametros){
	...
	if ($paso==1){
		$saiu06idsolicitud=numeros_validar($aParametros[1]);
		$saiu06consec=numeros_validar($aParametros[2]);
		if (($saiu06idsolicitud!='')&&($saiu06consec!='')){$besta=true;}
		}else{
		$saiu06id=$aParametros[103];
		if ((int)$saiu06id!=0){$besta=true;}
		}
	if ($besta){
		$besta=false;
		$objDB=new clsdbadmin($APP->dbhost, $APP->dbuser, $APP->dbpass, $APP->dbname);
		...
		$sSQL='SELECT * FROM saiu06solanotacion WHERE '.$sSQLcondi;
		$tabla=$objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla)>0){
			$fila=$objDB->sf($tabla);
			$besta=true;
	...
	if ($besta){
		...
		if ((int)$fila['saiu06idarchivo']!=0){$stemp='block';}
