...
function f3048_db_Guardar($valores, $objDB, $bDebug = false)
{
	...
	if (!$objDB->bexistetabla($sTabla47)) {
		...
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			if ($fila['saiu47idsolicitante'] == $_SESSION['unad_id_tercero']) {
				$bEsElPropietario = true;
			}
		}
	}
	if ($saiu48visiblealinteresado == '') {
		$saiu48visiblealinteresado = 1;
	}
	...
	*/
	if ($saiu48anotacion == '') {
		$sError = $ERR['saiu48anotacion'] . $sSepara . $sError;
	}
	...
	if ($saiu48idtramite == '') {
		$sError = $ERR['saiu48idtramite'] . $sSepara . $sError;
	}
	/*
	if ($sError==''){
	...
	*/
	...
	if ($sError == '') {
		...
		if ($bInserta) {
			...
			"' . $saiu48idusuario . '", "' . $saiu48fecha . '", ' . $saiu48hora . ', ' . $saiu48minuto . '';
			if ($APP->utf8 == 1) {
				$sSQL = 'INSERT INTO ' . $sTabla48 . ' (' . $sCampos3048 . ') VALUES (' . cadena_codificar($sValores3048) . ');';
			} else {
				$sSQL = 'INSERT INTO ' . $sTabla48 . ' (' . $sCampos3048 . ') VALUES (' . $sValores3048 . ');';
			}
			if ($bDebug) {
				$sDebug = $sDebug . fecha_microtiempo() . ' Guardar 3048 ' . $sSQL . '<br>';
			}
			$result = $objDB->ejecutasql($sSQL);
			if ($result == false) {
			...
			if ($bpasa) {
				if ($APP->utf8 == 1) {
					$sSQL = 'UPDATE ' . $sTabla48 . ' SET ' . cadena_codificar($sdatos) . ' WHERE ' . $sWhere . ';';
				} else {
...
function f3048_TablaDetalleV2($aParametros, $objDB, $bDebug = false)
{
	...
	*/
	...
	if ($bDebug) {
		$sDebug = $sDebug . fecha_microtiempo() . ' Consulta 3048: ' . $sSQL . '<br>';
	}
	...
	if ($tabladetalle == false) {
		...
		if ($registros == 0) {
			return array(cadena_codificar($sErrConsulta . '<input id="paginaf3048" name="paginaf3048" type="hidden" value="' . $pagina . '"/><input id="lppf3048" name="lppf3048" type="hidden" value="' . $lineastabla . '"/>'), $sDebug);
		}
...
function f3048_Traer($aParametros)
{
	...
	if ($sError == '') {
		if ($paso == 1) {
			$saiu48idtramite = numeros_validar($aParametros[1]);
			$saiu48consec = numeros_validar($aParametros[2]);
			if (($saiu48idtramite != '') && ($saiu48consec != '')) {
				$besta = true;
			}
		} else {
