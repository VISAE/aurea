...
--- Modelo Versión 2.23.5 Wednesday, August 28, 2019
--- 2216 core16actamatricula
*/
/** Archivo lib2216.php.
* Libreria 2216 core16actamatricula.
...
function f2216_HTMLComboV2_core16idprograma($objDB, $objCombos, $valor, $idEscuela){
	require './app.php';
	$mensajes_todas=$APP->rutacomun.'lg/lg_todas_'.$_SESSION['unad_idioma'].'.php';
	if (!file_exists($mensajes_todas)){$mensajes_todas=$APP->rutacomun.'lg/lg_todas_es.php';}
	require $mensajes_todas;
	$sCondi=' WHERE core09idescuela="'.$idEscuela.'" AND core09id>0';
...
function f2216_HTMLComboV2_core16idcead($objDB, $objCombos, $valor, $idZona){
	require './app.php';
	$mensajes_todas=$APP->rutacomun.'lg/lg_todas_'.$_SESSION['unad_idioma'].'.php';
	if (!file_exists($mensajes_todas)){$mensajes_todas=$APP->rutacomun.'lg/lg_todas_es.php';}
	require $mensajes_todas;
	$sCondi=' WHERE unad24idzona="'.$idZona.'" AND unad24id>0';
	$objCombos->nuevo('core16idcead', $valor, true, '{'.$ETI['msg_seleccione'].'}');
	$sSQL='SELECT unad24id AS id, unad24nombre AS nombre FROM unad24sede'.$sCondi;
	$res=$objCombos->html($sSQL, $objDB);
	return $res;
	}
function f2216_HTMLComboV2_core16origen($objDB, $objCombos, $valor){
	require './app.php';
	$mensajes_todas=$APP->rutacomun.'lg/lg_todas_'.$_SESSION['unad_idioma'].'.php';
	if (!file_exists($mensajes_todas)){$mensajes_todas=$APP->rutacomun.'lg/lg_todas_es.php';}
	require $mensajes_todas;
	$objCombos->nuevo('core16origen', $valor, true, '{'.$ETI['msg_seleccione'].'}');
	$sSQL='SELECT core17id AS id, core17nombre AS nombre FROM core17origenmatricula';
	$res=$objCombos->html($sSQL, $objDB);
	return $res;
	}
function f2216_Combocore16idprograma($aParametros){
	...
	$html_core16idprograma=f2216_HTMLComboV2_core16idprograma($objDB, $objCombos, '', $aParametros[0]);
	$objDB->CerrarConexion();
	$objResponse=new xajaxResponse();
	$objResponse->assign('div_core16idprograma', 'innerHTML', $html_core16idprograma);
	$objResponse->call('$("#core16idprograma").chosen()');
	return $objResponse;
	}
function f2216_Combocore16idcead($aParametros){
	$_SESSION['u_ultimominuto']=iminutoavance();
	if(!is_array($aParametros)){$aParametros=json_decode(str_replace('\"','"',$aParametros),true);}
	require './app.php';
	$objDB=new clsdbadmin($APP->dbhost, $APP->dbuser, $APP->dbpass, $APP->dbname);
	...
	$objDB->xajax();
	$objCombos=new clsHtmlCombos('n');
	$html_core16idcead=f2216_HTMLComboV2_core16idcead($objDB, $objCombos, '', $aParametros[0]);
	$objDB->CerrarConexion();
	$objResponse=new xajaxResponse();
	$objResponse->assign('div_core16idcead', 'innerHTML', $html_core16idcead);
	return $objResponse;
	}
...
function f2216_Busquedas($aParametros){
	...
	switch($sCampo){
		...
		if ($bExiste){
			...
			$sJavaBusqueda=f140_JavaScriptBusqueda(2216);
			}else{
			$sTitulo='Busquedas';
...
<div class="salto1px"></div>
	...
	$sSQL='SELECT T2.unad11doc AS C2_doc, T2.unad11razonsocial AS C2_nombre, T9.core09nombre, TB.core16nuevo, TB.core16numcursos, T2.unad11fechaultingreso, 
TB.core16id, TB.core16idcead, TB.core16parametros, TB.core16fecharecibido, TB.core16minrecibido, TB.core16procesado, TB.core16numaprobados, TB.core16promedio, TB.core16origen, TB.core16proccarac, TB.core16procagenda, TB.core16peraca, TB.core16tercero, T2.unad11tipodoc AS C2_td, TB.core16idprograma, TB.core16idescuela, TB.core16idzona 
...
function f2216_db_CargarPadre($DATA, $objDB, $bDebug=false){
	...
	if ($objDB->nf($tabla)>0){
		...
		$DATA['core16errormatricula']=$fila['core16errormatricula'];
		$DATA['core16estado']=$fila['core16estado'];
		$DATA['core16erroragenda']=$fila['core16erroragenda'];
		$DATA['core16aplazados']=$fila['core16aplazados'];
		$DATA['core16cancelados']=$fila['core16cancelados'];
		$DATA['core16fechamatricula']=$fila['core16fechamatricula'];
		$DATA['core16paraseguimiento']=$fila['core16paraseguimiento'];
		$DATA['core16tipomatricula']=$fila['core16tipomatricula'];
		$DATA['core16idconvenio']=$fila['core16idconvenio'];
		$bcargo=true;
		$DATA['paso']=2;
		$DATA['boculta2216']=0;
		$bLimpiaHijos=true;
...
function f2216_db_GuardarV2($DATA, $objDB, $bDebug=false){
	...
	if (isset($DATA['core16origen'])==0){$DATA['core16origen']='';}
	if (isset($DATA['core16nuevo'])==0){$DATA['core16nuevo']='';}
	if (isset($DATA['core16aplazados'])==0){$DATA['core16aplazados']='';}
	if (isset($DATA['core16cancelados'])==0){$DATA['core16cancelados']='';}
	if (isset($DATA['core16fechamatricula'])==0){$DATA['core16fechamatricula']='';}
	if (isset($DATA['core16paraseguimiento'])==0){$DATA['core16paraseguimiento']='';}
	if (isset($DATA['core16idconvenio'])==0){$DATA['core16idconvenio']='';}
	*/
	...
	$DATA['core16origen']=numeros_validar($DATA['core16origen']);
	$DATA['core16nuevo']=numeros_validar($DATA['core16nuevo']);
	$DATA['core16aplazados']=htmlspecialchars(trim($DATA['core16aplazados']));
	$DATA['core16cancelados']=htmlspecialchars(trim($DATA['core16cancelados']));
	$DATA['core16paraseguimiento']=numeros_validar($DATA['core16paraseguimiento']);
	$DATA['core16idconvenio']=numeros_validar($DATA['core16idconvenio']);
	...
	if ($DATA['core16aplazados']==''){$DATA['core16aplazados']=0;}
	if ($DATA['core16cancelados']==''){$DATA['core16cancelados']=0;}
	...
	if ($DATA['core16errormatricula']==''){$DATA['core16errormatricula']=0;}
	if ($DATA['core16estado']==''){$DATA['core16estado']=0;}
	if ($DATA['core16erroragenda']==''){$DATA['core16erroragenda']=0;}
	if ($DATA['core16paraseguimiento']==''){$DATA['core16paraseguimiento']=0;}
	if ($DATA['core16tipomatricula']==''){$DATA['core16tipomatricula']=0;}
	if ($DATA['core16idconvenio']==''){$DATA['core16idconvenio']=0;}
	// -- Seccion para validar los posibles causales de error.
	$sSepara=', ';
	if (true){
		//if ($DATA['core16paraseguimiento']==''){$sError=$ERR['core16paraseguimiento'].$sSepara.$sError;}
		if ($DATA['core16fechamatricula']==0){
			//$DATA['core16fechamatricula']=fecha_DiaMod();
			if ($DATA['core16tipomatricula']==0){
				$sError=$ERR['core16fechamatricula'].$sSepara.$sError;
				}else{
				$DATA['core16fechamatricula']=$ERR['core16fecharecibido'];
				}
			}
		//if ($DATA['core16cancelados']==''){$sError=$ERR['core16cancelados'].$sSepara.$sError;}
		//if ($DATA['core16aplazados']==''){$sError=$ERR['core16aplazados'].$sSepara.$sError;}
		...
		if ($DATA['core16origen']==''){$sError=$ERR['core16origen'].$sSepara.$sError;}
		if ($DATA['core16fecharecibido']==0){
			//$DATA['core16fecharecibido']=fecha_DiaMod();
			if ($DATA['core16tipomatricula']==0){
				$DATA['core16fecharecibido']=fecha_DiaMod();
				$DATA['core16minrecibido']=fecha_MinutoMod();
				}else{
	...
	if ($sError==''){
		...
		if ($DATA['paso']==10){
			...
			$DATA['core16errormatricula']=0;
			$DATA['core16estado']=0;
			$DATA['core16erroragenda']=0;
...
core16procesado, core16numcursos, core16numaprobados, core16promedio, core16origen, core16nuevo, core16proccarac, core16procagenda, core16idconsejero, core16errormatricula, 
core16estado, core16erroragenda, core16aplazados, core16cancelados, core16fechamatricula, core16paraseguimiento, core16tipomatricula, core16idconvenio';
...
'.$DATA['core16procesado'].', '.$DATA['core16numcursos'].', '.$DATA['core16numaprobados'].', '.$DATA['core16promedio'].', '.$DATA['core16origen'].', '.$DATA['core16nuevo'].', '.$DATA['core16proccarac'].', '.$DATA['core16procagenda'].', '.$DATA['core16idconsejero'].', '.$DATA['core16errormatricula'].', 
'.$DATA['core16estado'].', '.$DATA['core16erroragenda'].', "'.$DATA['core16aplazados'].'", "'.$DATA['core16cancelados'].'", "'.$DATA['core16fechamatricula'].'", '.$DATA['core16paraseguimiento'].', '.$DATA['core16tipomatricula'].', '.$DATA['core16idconvenio'].'';
			if ($APP->utf8==1){
			...
			$scampo[8]='core16fechamatricula';
			$scampo[9]='core16tipomatricula';
			$scampo[10]='core16idconvenio';
			...
			$sdato[8]=$DATA['core16fechamatricula'];
			$sdato[9]=$DATA['core16tipomatricula'];
			$sdato[10]=$DATA['core16idconvenio'];
			$numcmod=10;
...
function f2216_db_Eliminar($core16id, $objDB, $bDebug=false){
	...
	if ($sError==''){
		// Toca resolver esto porque los datos estan en un contenedor.
		$sSQL='SELECT core04idmatricula FROM core04matricula WHERE core04idmatricula='.$filabase['core16id'].'';
		$tabla=$objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla)>0){
			$sError='Existen Matricula creados, no es posible eliminar';
			}
		}
	if ($sError==''){
...
<div class="salto1px"></div>
	...
	*/
	$sTitulos='Peraca, Tercero, Programa, Id, Cead, Parametros, Escuela, Zona, Fecharecibido, Minrecibido, Procesado, Numcursos, Numaprobados, Promedio, Origen, Nuevo, Proccarac, Procagenda, Consejero, Errormatricula, Estado, Erroragenda, Aplazados, Cancelados, Fechamatricula, Paraseguimiento, Tipomatricula, Convenio';
	$sSQL='SELECT T1.exte02nombre, T2.unad11razonsocial AS C2_nombre, T3.core09nombre, TB.core16id, T5.unad24nombre, TB.core16parametros, T7.core12nombre, T8.unad23nombre, TB.core16fecharecibido, TB.core16minrecibido, TB.core16procesado, TB.core16numcursos, TB.core16numaprobados, TB.core16promedio, T15.core17nombre, TB.core16nuevo, TB.core16proccarac, TB.core16procagenda, T19.unad11razonsocial AS C19_nombre, TB.core16errormatricula, T21.core30nombre, TB.core16erroragenda, TB.core16aplazados, TB.core16cancelados, TB.core16fechamatricula, TB.core16paraseguimiento, T27.core08nombre, T28.core50nombre, TB.core16peraca, TB.core16tercero, T2.unad11tipodoc AS C2_td, T2.unad11doc AS C2_doc, TB.core16idprograma, TB.core16idcead, TB.core16idescuela, TB.core16idzona, TB.core16origen, TB.core16idconsejero, T19.unad11tipodoc AS C19_td, T19.unad11doc AS C19_doc, TB.core16estado, TB.core16tipomatricula, TB.core16idconvenio 
FROM core16actamatricula AS TB, exte02per_aca AS T1, unad11terceros AS T2, core09programa AS T3, unad24sede AS T5, core12escuela AS T7, unad23zona AS T8, core17origenmatricula AS T15, unad11terceros AS T19, core30estadomatricula AS T21, core08tipomatricula AS T27, core50convenios AS T28 
WHERE '.$sSQLadd1.' TB.core16peraca=T1.exte02id AND TB.core16tercero=T2.unad11id AND TB.core16idprograma=T3.core09id AND TB.core16idcead=T5.unad24id AND TB.core16idescuela=T7.core12id AND TB.core16idzona=T8.unad23id AND TB.core16origen=T15.core17id AND TB.core16idconsejero=T19.unad11id AND TB.core16estado=T21.core30id AND TB.core16tipomatricula=T27.core08id AND TB.core16idconvenio=T28.core50id '.$sSQLadd.'
...
function f2216_RegistrarMatricula($aParametros, $objDB, $bDebug=false){
	...
	//--- Modelo Versión 2.24.1 martes, 4 de febrero de 2020 - Se agrega aplazamientos y cancelaciones.
	$sDebug='';
	$sError='';
	...
	if (isset($aParametros['aplazados'])==0){$aParametros['aplazados']='';}
	if (isset($aParametros['cancelados'])==0){$aParametros['cancelados']='';}
	$core16tercero=0;
	$core16idescuela=0;
	$core16idzona=0;
	$core16idcead=0;
	...
	if ($sError==''){
		...
		$sSQL='SELECT core16id, core16procesado, core16parametros, core16idcead, core16idzona, core16nuevo, core16aplazados, core16cancelados 
...
WHERE core16tercero='.$core16tercero.' AND core16idprograma='.$aParametros['programa'].' AND core16peraca='.$aParametros['peraca'].'';
		...
		if ($objDB->nf($tabla)==0){
			...
			$sCampos2216='core16peraca, core16tercero, core16idprograma, core16id, core16idcead, 
core16parametros, core16idescuela, core16idzona, core16fecharecibido, core16minrecibido, 
core16procesado, core16numcursos, core16numaprobados, core16promedio, core16origen, 
core16nuevo, core16aplazados, core16cancelados';
			$sValores2216=''.$aParametros['peraca'].', '.$core16tercero.', '.$aParametros['programa'].', '.$core16id.', '.$core16idcead.', 
"'.$aParametros['cursos'].'", '.$core16idescuela.', '.$core16idzona.', '.$core16fecharecibido.', '.$core16minrecibido.', 
0, 0, 0, 0, 1, 
'.$aParametros['nuevo'].', "'.$aParametros['aplazados'].'", "'.$aParametros['cancelados'].'"';
			$sSQL='INSERT INTO core16actamatricula ('.$sCampos2216.') VALUES ('.$sValores2216.');';
			//$sdetalle=$sCampos2216.'['.$sValores2216.']';
			$result=$objDB->ejecutasql($sSQL);
			}else{
			...
			if ($fila['core16procesado']==0){
				$sSQL='UPDATE core16actamatricula SET core16parametros="'.$aParametros['cursos'].'", core16aplazados="'.$aParametros['aplazados'].'", core16cancelados="'.$aParametros['cancelados'].'", core16idcead='.$core16idcead.', core16idzona='.$core16idzona.', core16fecharecibido='.$core16fecharecibido.', core16minrecibido='.$core16minrecibido.', core16nuevo='.$aParametros['nuevo'].' WHERE core16id='.$fila['core16id'].'';
				$result=$objDB->ejecutasql($sSQL);
				//$sError=$sSQL;
				}else{
				...
				//inicia la edicion.
				$sdatos='';
				$bDevolverProceso=false;
				if ($fila['core16idcead']!=$core16idcead){
					$sdatos='core16idcead='.$core16idcead.'';
					}
				if  ($fila['core16parametros']!=$aParametros['cursos']){
					if ($sdatos!=''){$sdatos=$sdatos.', ';}
					$sdatos=$sdatos.'core16parametros="'.$aParametros['cursos'].'"';
					$bDevolverProceso=true;
					}
				if  ($fila['core16aplazados']!=$aParametros['aplazados']){
					if ($sdatos!=''){$sdatos=$sdatos.', ';}
					$sdatos=$sdatos.'core16aplazados="'.$aParametros['aplazados'].'"';
					$bDevolverProceso=true;
					}
				if  ($fila['core16cancelados']!=$aParametros['cancelados']){
					if ($sdatos!=''){$sdatos=$sdatos.', ';}
					$sdatos=$sdatos.'core16cancelados="'.$aParametros['cancelados'].'"';
					$bDevolverProceso=true;
					}
				...
				if ($bDevolverProceso){
					$sdatos=$sdatos.', core16procesado=0, core16errormatricula=0';
					}
				if ($sdatos!=''){
...
function f2216_ProcesarMatricula($core16id, $objDB, $objNoConformidad, $bDebug=false){
	//14 de Febrero de 2020 - Se incluye la consideración de aplazamientos y cancelaciones.
	$sDebug='';
	$sError='';
	$iHoy=fecha_DiaMod();
	$core16procesado=0;
	...
	$sSQL='SELECT core16peraca, core16tercero, core16idprograma, core16procesado, core16numcursos, core16nuevo, core16idescuela, core16idzona, core16idcead, core16parametros, core16fecharecibido, core16idcead, core16aplazados, core16cancelados 
FROM core16actamatricula WHERE core16id='.$core16id.'';
	...
	if ($objDB->nf($tabla)>0){
		...
		if ($sError==''){
			//Actualizar la tabla de terceros con el ultimo dato de matricula.
			list($iNumProcesados, $sError, $sDebugP)=f111_ProcesarMatricula($idTercero, $objDB, $bDebug);
			...
			for ($k=1;$k<=$iCursos;$k++){
				...
				if ($sErrorCurso==''){
					...
					if ($objDB->nf($result)==0){
						...
						$sSQL='INSERT INTO '.$sTabla04.' (core04peraca, core04tercero, core04idcurso, core04id, core04idrol, 
core04idaula, core04fechamatricula, core04idcead, core04tienenota, core04idagenda, 
core04idnav, core04idprograma, core04nuevo, core04cursoequivalente, core04idmoodle, 
core04idmatricula) 
VALUES ('.$core16peraca.', '.$idTercero.', '.$sCodCurso.', '.$core04id.', 5, 
0, '.$core16fecharecibido.', '.$core16idcead.', 1, '.$ofer08idagenda.', 
'.$ofer08idnav.', '.$idPrograma.', '.$iNuevo.', '.$core04cursoequivalente.', '.$core04idmoodle.', 
'.$core16id.')';
						//$core04id++;
						}else{
			...
			$sSQL='UPDATE core16actamatricula SET core16procesado='.$core16procesado.', core16numcursos='.$core16numcursos.', core16errormatricula='.$core16errormatricula.', core16estado=7 WHERE core16id='.$core16id.'';
			$result=$objDB->ejecutasql($sSQL);
			}
		}
...
function f2216_ArmarGrupo($idTercero, $idPeraca, $objDB, $bDebug=false, $idContTercero=0, $idContPeraca=0, $objDBRyC=NULL){
	$idGrupo=0;
	$sError='';
	$sDebug='';
	$bTraeRyC=false;
	...
	if ($sError==''){
		...
		if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' ARMAR GRUPOS - Listando cursos sin grupo '.$sSQL.'<br>';}
		...
		while($fila04=$objDB->sf($tabla04)){
			...
			if ($objDBRyC==NULL){
				...
				$sError='No fue posible conectar con la base de datos de RyC';
				}else{
...
function f2216_ProcesarAgenda($core16id, $objDB, $bDebug=false, $objDBRyC=NULL){
	$sDebug='';
	$sError='';
	$iHoy=fecha_DiaMod();
	$core16procagenda=0;
...
FROM core16actamatricula WHERE core16id='.$core16id.'';
	...
	if ($objDB->nf($tabla)>0){
		...
		if ($sError==''){
			list($idContTercero, $sError)=f1011_BloqueTercero($idTercero, $objDB);
			if ($idContTercero==0){$sError='No se ha definido el contenedor para el tercero {Ref: '.$idTercero.'}, por favor informe al administrador del sistema.';}
			if ($sError==''){
				$idContPeraca=f146_Contenedor($core16peraca, $objDB);
				if ($idContPeraca==0){$sError='No se ha definido el contenedor para el periodo '.$core16peraca.', por favor informe al administrador del sistema.';}
				}
			}
		if ($sError==''){
			...
			$iFormaGrupo=0;
			if ($iFormaGrupo==0){
				//Analisis del problema del cron 29 de Agosto de 2019
				//Al parecer qui no se esta logrando crear el grupo cuando viene por consola.
				list($idGrupo, $sErrCurso, $sDebugG)=f2216_ArmarGrupo($idTercero, $core16peraca, $objDB, $bDebug, $idContTercero, $idContPeraca, $objDBRyC);
				//$sError=$sDebugG;
				if ($sErrCurso!=''){
					$sError=$sErrCurso;
					}
...
WHERE TB.core04tercero='.$idTercero.' AND TB.core04peraca='.$core16peraca.' AND TB.core04aplicoagenda=0';
			if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' ARMAR AGENDA - Listando cursos '.$sSQL.'<br>';}
			$tabla04=$objDB->ejecutasql($sSQL);
			while($fila04=$objDB->sf($tabla04)){
		...
		$sError='No se encuentra la matricula {Ref:'.$core16id.'}';
		}
...
SELECT '.$idPeraca.', core97idprograma, core97idcurso, core97idcead, 0, 0, SUM(core97numestudiantes), SUM(core97numnuevos) FROM core97matriculaest_t WHERE core97idperaca='.$idPeraca.' GROUP BY core97idprograma, core97idcurso, core97idcead';
	...
	$sSQL='UPDATE core07matriculaest AS TB, unad24sede AS T2 SET TB.core07idzona=T2.unad24idzona WHERE TB.core07idperaca='.$idPeraca.' AND TB.core07idcead=T2.unad24id';
	$result=$objDB->ejecutasql($sSQL);
	//Actualizar el dato de escuela.
	$sSQL='UPDATE core07matriculaest AS TB, core09programa AS T2 SET TB.core07idescuela=T2.core09idescuela WHERE TB.core07idperaca='.$idPeraca.' AND TB.core07idprograma=T2.core09id';
	$result=$objDB->ejecutasql($sSQL);
	//Optimizar la tabla.
