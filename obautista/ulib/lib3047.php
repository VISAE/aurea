...
function f3047_HTMLComboV2_saiu47idgrupotrabajo($objDB, $objCombos, $valor, $vrsaiu47idunidad)
{
	...
	//$objCombos->iAncho=450;
	$objCombos->sAccion = 'carga_combo_saiu47idresponsable()';
	$sSQL = '';
	if ((int)$vrsaiu47idunidad != 0) {
		...
		WHERE TB.bita27idunidadfunc=' . $vrsaiu47idunidad . ' 
		ORDER BY TB.bita27activo DESC, TB.bita27nombre';
	}
	$res = $objCombos->html($sSQL, $objDB);
	return $res;
}
function f3047_HTMLComboV2_saiu47idresponsable($objDB, $objCombos, $valor, $vrsaiu47idgrupotrabajo)
{
	require './app.php';
	$mensajes_todas = $APP->rutacomun . 'lg/lg_todas_' . $_SESSION['unad_idioma'] . '.php';
	if (!file_exists($mensajes_todas)) {
	...
	require $mensajes_todas;
	$objCombos->nuevo('saiu47idresponsable', $valor, true, '{' . $ETI['msg_ninguno'] . '}', 0);
	//$objCombos->iAncho = 450;
	$sSQL = '';
	if ((int)$vrsaiu47idgrupotrabajo != 0) {
		//$objCombos->addItem('0', '[Sin Dato]');
		$sSQL = 'SELECT TB.bita28id AS id, T2.unad11razonsocial AS nombre
		FROM bita28eqipoparte AS TB, unad11terceros AS T2 
		WHERE  TB.bita28idequipotrab=' . $vrsaiu47idgrupotrabajo . ' AND TB.bita28idtercero=T2.unad11id AND TB.bita28activo="S"
...
function f3047_Combosaiu47idgrupotrabajo($aParametros)
{
	...
	$objDB->xajax();
	$objCombos = new clsHtmlCombos();
	$html_saiu47idgrupotrabajo = f3047_HTMLComboV2_saiu47idgrupotrabajo($objDB, $objCombos, 0, $aParametros[0]);
	$html_saiu47idresponsable = f3047_HTMLComboV2_saiu47idresponsable($objDB, $objCombos, 0, 0);
	$objDB->CerrarConexion();
	$objResponse = new xajaxResponse();
	$objResponse->assign('div_saiu47idgrupotrabajo', 'innerHTML', $html_saiu47idgrupotrabajo);
	$objResponse->assign('div_saiu47idresponsable', 'innerHTML', $html_saiu47idresponsable);
	//$objResponse->call('$("#saiu47idgrupotrabajo").chosen()');
	return $objResponse;
}
function f3047_Combosaiu47idresponsable($aParametros)
{
	$_SESSION['u_ultimominuto'] = iminutoavance();
	if (!is_array($aParametros)) {
		$aParametros = json_decode(str_replace('\"', '"', $aParametros), true);
	...
	$objDB->xajax();
	$objCombos = new clsHtmlCombos();
	$html_saiu47idresponsable = f3047_HTMLComboV2_saiu47idresponsable($objDB, $objCombos, '', $aParametros[0]);
	$objDB->CerrarConexion();
	$objResponse = new xajaxResponse();
	$objResponse->assign('div_saiu47idresponsable', 'innerHTML', $html_saiu47idresponsable);
	//$objResponse->call('$("#saiu47idresponsable").chosen()');
	return $objResponse;
}
...
function f3047_TablaDetalleV2($aParametros, $objDB, $bDebug = false)
{
	...
	*/
	...
	while ($filadet = $objDB->sf($tabladetalle)) {
		...
		$tlinea++;
		$et_saiu47consec = $sPrefijo . $filadet['saiu47consec'] . $sSufijo;
		...
		$res = $res . '<tr' . $sClass . '>';
		$res = $res . '<td>' . $sPrefijo . $et_fecha . $sSufijo . '</td>';
		$res = $res . '<td>' . $et_saiu47consec . '</td>';
...
function f3047_db_GuardarV2($DATA, $objDB, $bDebug = false, $idTercero = 0)
{
	...
	*/
	...
	if ($sError == '') {
		...
		if ($DATA['paso'] == 10) {
			$sCondiEstado = ' AND saiu47estado=0';
	...
	if ($sError == '') {
		...
		if ($DATA['paso'] == 10) {
			...
			if ($APP->utf8 == 1) {
				$sSQL = 'INSERT INTO ' . $sTabla47 . ' (' . $sCampos3047 . ') VALUES (' . cadena_codificar($sValores3047) . ');';
				$sdetalle = $sCampos3047 . '[' . cadena_codificar($sValores3047) . ']';
			} else {
			...
			$scampo[13] = 'saiu47t1vraprobado';
			$scampo[14] = 'saiu47t1vrsaldoafavor';
			$scampo[15] = 'saiu47t1vrdevolucion';
			$scampo[16] = 'saiu47idbenefdevol';
			...
			$sdato[13] = $DATA['saiu47t1vraprobado'];
			$sdato[14] = $DATA['saiu47t1vrsaldoafavor'];
			$sdato[15] = $DATA['saiu47t1vrdevolucion'];
			$sdato[16] = $DATA['saiu47idbenefdevol'];
			$numcmod = 16;
			...
			if ($bPasa) {
				if ($APP->utf8 == 1) {
					...
					$sSQL = 'UPDATE ' . $sTabla47 . ' SET ' . cadena_codificar($sdatos) . ' WHERE ' . $sWhere . ';';
				} else {
...
function f3047_NotificarEvento($saiu47agno, $saiu47id, $objDB, $bDebug = false, $bNotificaFuncionario = false)
{
	...
	if ($sError == '') {
		...
		if ($objDB->nf($tabla) > 0) {
			...
			switch ($saiu47tipotramite) {
				case 1:
					$sNombreTipoTramite = 'Solicitud de saldo a aplicar o devolución';
					break;
				case 707:
					$sNombreTipoTramite = 'Solicitud de confirmación de recaudo';
					break;
			}
			...
			$sCuerpo = $sCuerpo . 'Le informamos que su <b>' . cadena_notildes($sNombreTipoTramite) . ' N&deg; ' . $fila['saiu47consec'] . ' del ' . $saiu47agno . '</b>';
			switch ($saiu47estado) {
				case 0:
				...
				case 6: //En estudio
					...
					break;
					/*
				case 7: //Proyecccion Procedente  (debe tener resolucion.)
				...
				case 9: //No procedente
					...
					$bNotificaFinal = true;
					break;
				case 10: // Solicitud Resuelt
					$sCuerpo = $sCuerpo . ' ha sido <b>RESUELTA</b>.<br><br>';
					if ($iNumAnotaciones > 0) {
						$bConDetalle = true;
					}
					$bConActa = true;
			...
			if ($bConActa) {
				...
				$sSQL = 'SELECT TB.saiu59idorigen, TB.saiu59idarchivo, T12.saiu51nombre  
				FROM saiu59tramiteanexo_' . $saiu47agno . ' AS TB, saiu51tramitedoc AS T12 
				WHERE TB.saiu59idtramite=' . $saiu47id . ' AND TB.saiu59idtipodoc=T12.saiu51id 
				AND T12.saiu51proveedor<>0 AND T12.saiu51visible=1 
			...
			if ($sLinks != '') {
				if ($bNotificaFinal) {
					...
					$sCuerpo = $sCuerpo . 'impartida al momento de radicar su solicitud, mediante la presente nos permitmos notificar de forma electr&oacute;nica ';
					$sCuerpo = $sCuerpo . 'los siguientes documentos: <br>' . $sLinks . ', los cuales puede descargar haciendo clic sobre el nombre del mismo, ';
					$sCuerpo = $sCuerpo . 'la presente notificaci&oacute;n se hace de acuerdo con lo contemplado en el art&iacute;culo 56 de la Ley 1437 de 2011.<br><br>';
					...
					if (false) {
						...
						$sCuerpo = $sCuerpo . 'para interponer recurso en contra de la misma, en caso de no encontrarse de acuerdo con su contenido<br><br>';
					}
				} else {
	...
	if ($sError == '') {
		...
		$sCuerpo = $sCuerpo . 'Por favor no responder este mensaje, esta es una notificaci&oacute;n del Sistema de Atenci&oacute;n Integral<br>';
		$sCuerpo = $sCuerpo . 'Sistema Integrado de Informaci&oacute;n 4.0 - Plataforma AUREA';
...
function f3047_CambiaEstado($iAgno, $saiu47id, $idOrigen, $idDestino, $saiu49detalle, $saiu49usuario, $objDB, $bDebug = false)
{
	...
	if ($sError == '') {
		...
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' <b>Cambiando estado</b> Datos base: ' . $sSQL . '<br>';
		}
		$tabla = $objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			...
			if ($fila['saiu47estado'] != $idOrigen) {
				$sError = 'El estado de origen no coincide [' . $fila['saiu47estado'] . '].';
			}
		} else {
			$sError = 'No se ha encontrado el registro solicitado [Ref ' . $saiu47id . ' A&ntilde;o ' . $iAgno . ']';
		}
	...
	if ($sError == '') {
		...
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' <b>Cambiando estado</b> Guardar 3049 ' . $sSQL . '<br>';
		}
		$result = $objDB->ejecutasql($sSQL);
		if ($result == false) {
				...
				case 1: //Solicitudes de devolución
					switch ($idDestino) {
						...
						case 6: //En estudio
							...
							$sInfoComplemento = ', saiu47idunidad=' . $saiu47idunidad . ', saiu47idgrupotrabajo=' . $saiu47idgrupotrabajo . ', saiu47idresponsable=' . $saiu47idresponsable . '';
							break;
						case 7: // En proyección de resolución - Procedente  (debe tener resolucion.)
...
//Validaciones
function f3047_ValidarArchivosAnexos($sTabla59, $saiu47id, $objDB, $bDebug = false)
{
	$sError = '';
	$sDebug = '';
	$sSQL = 'SELECT 1 FROM ' . $sTabla59 . ' AS TB, saiu51tramitedoc AS T51 
	WHERE TB.saiu59idtramite=' . $saiu47id . ' AND TB.saiu59idarchivo=0 
	AND TB.saiu59idtipodoc=T51.saiu51id AND T51.saiu51opcional=0';
	$tabla = $objDB->ejecutasql($sSQL);
	if ($bDebug) {
		$sDebug = $sDebug . fecha_microtiempo() . ' Verificando documento ' . $sSQL . '<br>';
	}
	$iPendientes = $objDB->nf($tabla);
	if ($iPendientes > 0) {
		$sError = 'Existen ' . $iPendientes . ' anexos que no han sido cargados, no es posible continuar.';
	}
	return array($iPendientes, $sError, $sDebug);
}
