...
function f2937_db_Guardar($valores, $objDB, $bDebug = false, $idTercero = 0, $iCodModulo = 2937)
{
	...
	$mensajes_2937 = $APP->rutacomun . 'lg/lg_2937_' . $_SESSION['unad_idioma'] . '.php';
	...
	$visa37consec = numeros_validar($valores[2]);
	$visa37id = numeros_validar($valores[3], true);
	$visa37nombre = cadena_Validar(trim($valores[4]));
	$visa37activo = numeros_validar($valores[5]);
	/*
	if ($visa37activo == '') {
		$visa37activo = 0;
	}
	...
	*/
	...
	if ($sError == '') {
		if ((int)$visa37id == 0) {
			if ((int)$visa37consec == 0) {
				$visa37consec = tabla_consecutivo('visa37convsubtipo', 'visa37consec', 'visa37idtipologia=' . $visa37idtipologia . '', $objDB);
				if ($visa37consec == -1) {
					$sError = $objDB->serror;
				}
				$bQuitarCodigo = true;
				$sCampoCodigo = 'visa37consec';
			} else {
			...
			if ($sError == '') {
				$sSQL = 'SELECT 1 FROM visa37convsubtipo WHERE visa37idtipologia=' . $visa37idtipologia . ' AND visa37consec=' . $visa37consec . '';
	...
	if ($sError == '') {
		if ($bInserta) {
			$sCampos2937 = 'visa37idtipologia, visa37consec, visa37id, visa37nombre, visa37activo';
			$sValores2937 = '' . $visa37idtipologia . ', ' . $visa37consec . ', ' . $visa37id . ', "' . $visa37nombre . '", ' . $visa37activo . '';
			if ($APP->utf8 == 1) {
			...
			//$sWhere = 'visa37idtipologia=' . $visa37idtipologia . ' AND visa37consec=' . $visa37consec . '';
...
function f2937_db_Eliminar($aParametros, $objDB, $bDebug = false, $idTercero = 0)
{
	...
	$visa37consec = numeros_validar($aParametros[2]);
	$visa37id = numeros_validar($aParametros[3]);
	if ($sError == '') {
...
function f2937_TablaDetalleV2($aParametros, $objDB, $bDebug = false)
{
	...
	*/
	$sSQLadd1 = 'visa37idtipologia=' . $visa36id;
	$sTitulos = 'Consec, Id, Nombre, Activo';
	$registros = 0;
	$bGigante = false; //En caso de que la tabla sea muy grande pasarlo a true
	$sLimite = '';
	$sCampos = 'SELECT TB.visa37consec, TB.visa37id, TB.visa37nombre, TB.visa37activo';
	$sConsulta = 'FROM visa37convsubtipo AS TB 
	WHERE ' . $sSQLadd1 . '  ' . $sSQLadd . '';
	$sOrden = 'ORDER BY TB.visa37nombre';
	...
	$res = $res . '<th><b>' . $ETI['visa37consec'] . '</b></th>';
	...
	while ($filadet = $objDB->sf($tabladetalle)) {
		...
		$tlinea++;
		$et_visa37consec = $sPrefijo . $filadet['visa37consec'] . $sSufijo;
...
function f2937_Traer($aParametros)
{
	...
	*/
	...
	if ($paso == 1) {
		$visa37idtipologia = numeros_validar($aParametros[1]);
		$visa37consec = numeros_validar($aParametros[2]);
		if (($visa37idtipologia != '') && ($visa37consec != '')) {
			$besta = true;
		}
	} else {
	...
	if ($besta) {
		...
		$sSQLcondi = '';
		if ($paso == 1) {
			$sSQLcondi = $sSQLcondi . 'visa37idtipologia=' . $visa37idtipologia . ' AND visa37consec=' . $visa37consec . '';
		} else {
	...
	if ($besta) {
		$iPiel = iDefinirPiel($APP, 2);
		$visa37consec_nombre = '';
		$html_visa37consec = html_oculto('visa37consec', $fila['visa37consec'], $visa37consec_nombre);
		$objResponse->assign('div_visa37consec', 'innerHTML', $html_visa37consec);
		$visa37id_nombre = '';
		...
		if ($paso == 1) {
			$objResponse->assign('visa37consec', 'value', $visa37consec);
		} else {
...
function f2937_PintarLlaves($aParametros)
{
	...
	*/
	...
	//require $mensajes_2937;
	$iPiel = iDefinirPiel($APP, 2);
	$html_visa37consec = '<input id="visa37consec" name="visa37consec" type="text" value="" onchange="revisaf2937()" class="cuatro" />';
