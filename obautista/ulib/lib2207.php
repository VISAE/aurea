...
*/
/** Archivo lib2207.php.
* Libreria 2207 core07matriculaest.
...
function f2207_HTMLComboV2_core07idprograma($objDB, $objCombos, $valor, $idTercero=0, $idEscuela='', $seg_12=0){
	require './app.php';
	$mensajes_todas=$APP->rutacomun.'lg/lg_todas_'.$_SESSION['unad_idioma'].'.php';
	if (!file_exists($mensajes_todas)){$mensajes_todas=$APP->rutacomun.'lg/lg_todas_es.php';}
	require $mensajes_todas;
	if ($idTercero==0){$idTercero=$_SESSION['unad_id_tercero'];}
	$sCondiCombo=' WHERE core09iddirector='.$idTercero.' ';
	if ($seg_12==1){
		if ($idEscuela==''){
			$sCondiCombo='';
			}else{
			$sCondiCombo=' WHERE core09idescuela='.$idEscuela.'';
			}
		}else{
		if ($idEscuela!=''){
			$sCondiCombo=$sCondiCombo.' AND core09idescuela='.$idEscuela.'';
			}
		}
	$objCombos->nuevo('core07idprograma', $valor, true, '{'.$ETI['msg_todos'].'}');
	$objCombos->sAccion='paginarf2207();';
	$sSQL='SELECT core09id AS id, CONCAT(core09codigo, " - ", core09nombre) AS nombre FROM core09programa '.$sCondiCombo.' ORDER BY core09activo DESC, core09nombre';
	$res=$objCombos->html($sSQL, $objDB);
	return $res;
	}
...
function f2207_HTMLComboV2_core07idescuela($objDB, $objCombos, $valor){
	...
	$sSQL='SELECT core12id AS id, core12nombre AS nombre FROM core12escuela';
	$res=$objCombos->html($sSQL, $objDB);
	return $res;
	}
function f2207_Combocore07idprograma($aParametros){
	...
	$objResponse->call('$("#core07idprograma").chosen()');
	return $objResponse;
	}
...
function f2207_Combocore07idcead($aParametros){
	...
	//$objResponse->call('$("#core07idcead").chosen()');
	return $objResponse;
	}
...
<input id="lppf2207" name="lppf2207" type="hidden" value="'.$lineastabla.'"/>', $sDebug);
	...
	if ($aParametros[111]==1){
		//Solo los programas donde es lider.
		$sIds='-99';
		$sSQL='SELECT TB.core09id FROM core09programa AS TB WHERE TB.core09iddirector='.$idTercero.'';
		if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Consulta de programas donde es director: '.$sSQL.'<br>';}
		$tabla=$objDB->ejecutasql($sSQL);
		while($fila=$objDB->sf($tabla)){
			$sIds=$sIds.','.$fila['core09id'];
			}
		$sSQLadd1=$sSQLadd1.'TB.core07idprograma IN ('.$sIds.') AND ';
		}
	/*
	if ($aParametros[104]!=''){
