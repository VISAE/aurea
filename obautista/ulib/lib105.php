...
<input id="titulos_105" name="titulos_105" type="hidden" value="'.$sTitulos.'"/>';
	...
	$sTitulo='';
	if ($APP->idsistema==1){
		$sTitulo='<td><b>'.$ETI['unad05aplicativo'].'</b></td>';
		}
	$res=$sErrConsulta.$sLeyenda.'<table border="0" align="center" cellpadding="0" cellspacing="2" class="tablaapp">
<tr class="fondoazul">
<td><b>'.$ETI['unad05id'].'</b></td>
<td><b>'.$ETI['unad05nombre'].'</b></td>'.$sTitulo.'
...
'.html_lpp('lppf105', $lineastabla, 'paginarf105()').'
	...
	while($filadet=$objDB->sf($tabladetalle)){
		...
		$et_aplicativo='';
		if ($APP->idsistema==1){
			$et_aplicativo='<td>'.$sPrefijo.$filadet['unad05aplicativo'].$sSufijo.'</td>';
			}
...
<td>'.$sPrefijo.cadena_notildes($filadet['unad05nombre']).$sSufijo.'</td>'.$et_aplicativo.'
