...
function AUREA_ActualizarPerfilMoodle($idTercero, $objDB, $bDebug=false){
	...
	if ($APP->dbhostcampus!=''){
		...
		if ($objDBext->Conectar()){
			...
			$sSQL=utf8_decode('UPDATE mdl_user SET idnumber="'.$fila['unad11doc'].'", email="'.$sCorreoUsuario.'", firstname="'.trim($fila['unad11nombre1'].' '.$fila['unad11nombre2']).'", lastname="'.trim($fila['unad11apellido1'].' '.$fila['unad11apellido2']).'", country="'.$sPrefijoPais.'", city="'.$sNomCiudad.'", description="'.$unad11presentacion.'" WHERE username="'.$fila['unad11usuario'].'"');
	...
	if ($objDB->nf($tabla)>0){
		...
		if (!$bHayCorreo){
			...
			if ($fila['unad11rolunad']==0){
				if (correo_VerificarDireccion(trim($fila['unad11correo']))){
...
function AUREA_IniciarLogin($idTercero, $objDB, $sFrase='', $iUso=0, $bDebug=false){
	...
	switch($idEntidad){
		...
		$sURLCampus='https://aurea.unad.us/campus/';
		break;
		default: // UNAD Colombia
		$sNomEntidad='UNIVERSIDAD NACIONAL ABIERTA Y A DISTANCIA - UNAD';
		$sMailSeguridad='soporte.campus@unad.edu.co';
		$sURLCampus='https://campus0h.unad.edu.co/campus/';
		break;
		}
	$sCorreoUsuario='';
	$iCodigoRastro=0;
...
<h3 style="text-align: center; color: #22A7E0; font-size: 22px;">'.$sNomEntidad.'</h3>
...
function AUREA_RequiereDobleAutenticacion($idTercero, $objDB){
	...
	if ($bRes){
		...
		//Octubre 31 de 2019 - casos especiales.
		if ($idTercero==4){$bRes=true;}
		if ($idTercero==22731){$bRes=true;}
		}
	return $bRes;
	}
