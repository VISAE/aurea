...
WHERE unad47idtercero='.$idTercero.' AND unad47idcead=0 AND unad47origen=0';
	...
	while ($filaf=$objOrigen->sf($tablaf)){
		...
		if ($debug){
			$sInfoDebug=$sInfoDebug.'Procesando '.$filaf['shortname'].' ['.$filaf['role'].'] = Curso '.$idCurso.' Aula '.$idAula.' Peraca '.$idPeraca.'<br>';
			}
		...
		if ($idCurso!=''){
			switch($idRol){
				case 12:
				//Julio 16 de 2019...
				//Cancelamos la actualizacion al rol Administrador VIMEP.
				if ($debug){
					$sInfoDebug=$sInfoDebug.'<span class="rojo">El rol '.$idRol.' ha sido desactivado.</span><br>';
					}
				$idCurso='';
				break;
				}
		...
		if ($idCurso!=''){
			...
			if ($idPeraca!=''){
				...
				if ($bDebug2){$sDebug=$sDebug.fecha_microtiempo().' TABLERO: CURSO '.$idCurso.'_'.$idPeraca.' <br>';}
				$sql='SELECT 1 
FROM unad47tablero 
WHERE unad47idtercero='.$idTercero.' AND unad47idcurso='.$idCurso.' AND unad47peraca='.$idPeraca.' AND unad47idnav='.$idNav.' AND unad47idrol='.$idRol.' AND unad47numaula='.$idAula.'';
				$res=$objDB->ejecutasql($sql);
				if ($objDB->nf($res)==0){
...
WHERE unad47idtercero='.$idTercero.' AND unad47peraca='.$idPeraca.' AND unad47idnav='.$idNav.' AND unad47idrol='.$idRol.' AND unad47idcurso='.$idCurso.' AND unad47numaula='.$idAula.'';
	...
	if ($bDebug2){$sDebug=$sDebug.fecha_microtiempo().' TABLERO: Marcar los cursos como retirados..<br>';}
