...
function f2943_HTMLComboV2_visa43iddocumento($objDB, $objCombos, $valor)
{
	...
	$sSQL = 'SELECT TB.visa42id AS id, TB.visa42titulo AS nombre 
	FROM visa42convanexo AS TB
	WHERE TB.visa42id>0 AND TB.visa42activo=1
	ORDER BY TB.visa42titulo';
...
function f2943_db_Guardar($valores, $objDB, $bDebug = false, $idTercero = 0, $iCodModulo = 2943)
{
	...
	$mensajes_2943 = $APP->rutacomun . 'lg/lg_2943_' . $_SESSION['unad_idioma'] . '.php';
...
function f2943_TablaDetalleV2($aParametros, $objDB, $bDebug = false)
{
	...
	$mensajes_2942 = $APP->rutacomun . 'lg/lg_2942_' . $_SESSION['unad_idioma'] . '.php';
	...
	if (true) {
		...
		$bVistaUsr = numeros_validar($aParametros[103]);
	...
	if ($objDB->nf($tabla) > 0) {
		$fila = $objDB->sf($tabla);
		if ($fila['visa40estado'] <= 3) {
			$bConBotonesDoc = true;
			$bAbierta = true;
		}
		if ($fila['visa40estado'] == 1) {
			$bConRevision = true;
		}
	}
	$sLeyenda = '';
	...
	*/
	$sSQLadd = $sSQLadd . ' AND TB.visa43idinscripcion=' . $visa40id;
	$sTitulos = 'Id, Documento, Fechaaprob';
	$registros = 0;
	$bGigante = false; //En caso de que la tabla sea muy grande pasarlo a true
	$sLimite = '';
	$sCampos = 'SELECT TB.visa43id, T2.visa42id, T2.visa42titulo, T2.visa42descripcion, T2.visa42obligatorio, 
	TB.visa43idorigen, TB.visa43idarchivo, TB.visa43fechaaprob, TB.visa43iddocumento, TB.visa43usuarioaprueba';
	$sConsulta = 'FROM visa43inscripdocs AS TB, visa42convanexo AS T2 
	WHERE ' . $sSQLadd1 . ' TB.visa43iddocumento=T2.visa42id ' . $sSQLadd . '';
	$sOrden = 'ORDER BY T2.visa42orden';
	...
	$res = $res . '<th><b>' . $ETI['visa42descripcion'] . '</b></th>';
	$res = $res . '<th><b>' . $ETI['visa42obligatorio'] . '</b></th>';
	$res = $res . '<th colspan="2"><b>' . $ETI['visa43estado'] . '</b></th>';
	...
	while ($filadet = $objDB->sf($tabladetalle)) {
		...
		$sLinkRet = '';
		$sLinkDescribe = '';
		if ($filadet['visa43usuarioaprueba'] != 0) {
			$sPrefijo = '<b>';
			$sSufijo = '</b>';
		}
		if (($tlinea % 2) != 0) {
		...
		$tlinea++;
		$et_visa43iddocumento = $sPrefijo . cadena_notildes($filadet['visa42titulo']) . $sSufijo;
		$et_visa43fechaaprob = '';
		...
		if ($filadet['visa42descripcion'] != '') {
			$objForma = new clsHtmlForma($iPiel);
			$sLinkDescribe = $objForma->htmlBotonSolo('cmdDescribe' . $filadet['visa42id'], 'btMiniAyuda', "ModalConfirmV2('" . $filadet['visa42descripcion'] . "', null)", $ETI['visa42descripcion']);
		}
		if (($bConRevision) && ($filadet['visa43idarchivo'] != 0)) {
			if ($filadet['visa43usuarioaprueba'] == 0) {
				$sLinkApr='<a href="javascript:apruebaidf2943('.$filadet['visa43id'].')" class="lnkresalte">Aprobar</a>';
			} else {
				$sLinkRet='<a href="javascript:retiraidf2943('.$filadet['visa43id'].')" class="lnkresalte">Desaprobar</a>';
			}
		}
		$et_visa43estado = $sPrefijo . $ETI['visa43aprobado'] . $sSufijo;
		if ($filadet['visa43usuarioaprueba'] == 0) {
			$et_visa43estado = $sPrefijo . $ETI['No'] . ' ' . $ETI['visa43aprobado'] . $sSufijo;
		}
		...
		$res = $res . '<td>' . $sLinkDescribe . '</td>';
		$res = $res . '<td>' . $et_visa42obligatorio . '</td>';
		if ($bVistaUsr == 0) {
			$res = $res . '<td>' . $sLinkApr . '</td>';
			$res = $res . '<td>' . $sLinkRet . '</td>';
		} else {
			$res = $res . '<td colspan=2>' . $et_visa43estado . '</td>';
		}
	...
	*/
	...
	if ($besta) {
		...
		if ($fila['visa43iddocumento'] != 0) {
			list($visa43iddocumento_nombre, $serror_det) = tabla_campoxid('visa42convanexo', 'visa42titulo', 'visa42id', $fila['visa43iddocumento'], '{' . $ETI['msg_sindato'] . '}', $objDB);
...
// -----------------------------------
function f2943_CargarDocumentos($visa35id, $visa40id, $objDB, $bDebug = false, $bForzar = false)
{
	$sError = '';
	$sDebug = '';
	$visa35idtipo = 0;
	if ($sError == '') {
		$sSQL = 'SELECT visa43id, visa43idarchivo FROM visa43inscripdocs WHERE visa43idinscripcion=' . $visa40id . ';';
		$tabla = $objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla) > 0) {
			...
			while ($fila = $objDB->sf($tabla)) {
				if ($fila['visa43idarchivo'] != 0) {
					if ($sWhere==''){
						$sWhere=$sWhere.'WHERE visa43id IN (';
					} else {
						$sWhere=$sWhere.', ';
					}
					$sWhere=$sWhere.$fila['visa43id'];
				}
			}
			if ($sWhere != '') {
				$sWhere=$sWhere.')';
	...
	if ($sError == '') {
		$sSQL = 'SELECT visa35idtipo FROM visa35convocatoria WHERE visa35id=' . $visa35id . '';
		$tabla = $objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
		...
		$sDebug = $sDebug . fecha_microtiempo() . ' consulta tipo convocatoria: ' . $sSQL . '<br>';
	}
	if ($sError == '') {
		$sCampos2943 = ' visa43idinscripcion, visa43iddocumento, visa43id, visa43idorigen, visa43idarchivo, 
		visa43fechaaprob, visa43usuarioaprueba';
		$visa43id = tabla_consecutivo('visa43inscripdocs', 'visa43id', '', $objDB);
		$sSQL = 'SELECT visa42id, visa42obligatorio 
		FROM visa42convanexo 
		WHERE visa42idtipo=' . $visa35idtipo . ' AND visa42activo=1 
		ORDER BY visa42orden';
		$tabla = $objDB->ejecutasql($sSQL);
		while ($fila = $objDB->sf($tabla)) {
			$sValores2943 = '' . $visa40id . ', ' . $fila['visa42id'] . ', ' . $visa43id . ', 0, 0,
			0, 0';
			$sSQL = 'INSERT INTO visa43inscripdocs (' . $sCampos2943 . ') VALUES (' . $sValores2943 . ');';
			$result = $objDB->ejecutasql($sSQL);
			if ($bDebug) {
...
function f2943_AprobarDocumento($aParametros)
{
	$_SESSION['u_ultimominuto'] = iminutoavance();
	require './app.php';
	$sIdioma = AUREA_Idioma();
	...
	$visa43id = $aParametros[3];
	$visa35id = $aParametros[4];
	$iHoy = fecha_DiaMod();
	if ($idProceso == 0) {
		...
		$sSQL = 'SELECT T42.visa42obligatorio, T43.visa43idarchivo, T43.visa43fechaaprob
		FROM visa43inscripdocs AS T43, visa42convanexo AS T42 
		WHERE T43.visa43iddocumento=T42.visa42id AND T43.visa43idinscripcion=' . $visa40id . '';
		$tabla = $objDB->ejecutasql($sSQL);
		while ($fila = $objDB->sf($tabla)) {
		...
		if ($bCumpleArchivos) {
			$sSQL = 'SELECT T40.visa40estado, T40.visa40numcupo, T35.visa35total_inscritos FROM visa40inscripcion AS T40, visa35convocatoria AS T35 WHERE T40.visa40idconvocatoria=T35.visa35id AND T40.visa40id=' . $visa40id . '';
			$tabla = $objDB->ejecutasql($sSQL);
			if ($fila = $objDB->sf($tabla)) {
				list($sErrorC, $sDebugC, $sMensaje) = f2940_CambiaEstado($visa40id, $fila['visa40estado'], 5, '', 0, $objDB, $bDebug);
				$sDebug = $sDebug . $sDebugC;
				if ($fila['visa40numcupo'] == 0) {
					$sSQL = 'UPDATE visa35convocatoria SET visa35total_inscritos=visa35total_inscritos+1 WHERE visa35id=' . $visa35id;
					$result = $objDB->ejecutasql($sSQL);
					$sSQL = 'UPDATE visa40inscripcion SET visa40numcupo=' . ($fila['visa35total_inscritos'] + 1) . ' WHERE visa40id=' . $visa40id . ' AND visa40idconvocatoria=' . $visa35id . '';
					$result = $objDB->ejecutasql($sSQL);
				}
			}
		}
	...
	if ($bDebug) {
		$sDebug = $sDebug . fecha_microtiempo() . ' Actualizando dato: ' . $sSQL . '<br>';
	}
	$objResponse = new xajaxResponse();
	if ($sError == '') {
		list($sDetalle, $sDebugTabla) = f2943_TablaDetalleV2($aParametros, $objDB, $bDebug);
