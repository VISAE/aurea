...
function f269_ExisteDato($datos){
	...
	if ($aure69consec==''){$bHayLlave=false;}
	if ($bHayLlave){
		require './app.php';
		$objDB=new clsdbadmin($APP->dbhost, $APP->dbuser, $APP->dbpass, $APP->dbname);
		if ($APP->dbpuerto!=''){$objDB->dbPuerto=$APP->dbpuerto;}
...
function f269_TablaDetalleV2($aParametros, $objDB, $bDebug=false){
	...
	*/
	...
	if ($bGrupo!=''){$sSQLadd1=$sSQLadd1.'TB.aure69idgrupo='.$aParametros[104].' AND ';}
	if ($bNombre!=''){
	...
	if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Consulta 269: '.$sSQL.$sLimite.'<br>';}
	if ($tabladetalle==false){
		...
		if (!$bGigante){
			...
			if ($registros==0){
				//return array(utf8_encode($sErrConsulta.'<input id="paginaf269" name="paginaf269" type="hidden" value="'.$pagina.'"/><input id="lppf269" name="lppf269" type="hidden" value="'.$lineastabla.'"/>'), $sDebug);
				}
			if ((($registros-1)/$lineastabla)<($pagina-1)){$pagina=(int)(($registros-1)/$lineastabla)+1;}
			if ($registros>$lineastabla){
				$rbase=($pagina-1)*$lineastabla;
	...
	while($filadet=$objDB->sf($tabladetalle)){
		...
		$tlinea++;
		$et_aure69fecha='';
		if ($filadet['aure69fecha']!='00/00/0000'){$et_aure69fecha=$filadet['aure69fecha'];}
		$et_aure69publico=$sPrefijo.$ETI['no'].$sSufijo;
		if ($filadet['aure69publico']=='S'){$et_aure69publico=$sPrefijo.$ETI['si'].$sSufijo;}
		if ($bAbierta){
...
function f269_db_GuardarV2($DATA, $objDB, $bDebug=false){
	$iCodModulo=269;
	$bAudita[2]=true;
	$bAudita[3]=true;
	require './app.php';
	...
	*/
	...
	$sSepara=', ';
	if (true){
		if ($DATA['aure69idmodulo']==''){$sError=$ERR['aure69idmodulo'].$sSepara.$sError;}
		if ($DATA['aure69publico']==''){$sError=$ERR['aure69publico'].$sSepara.$sError;}
		//if ($DATA['aure69detalle']==''){$sError=$ERR['aure69detalle'].$sSepara.$sError;}
		if ($DATA['aure69idgrupo']==''){$sError=$ERR['aure69idgrupo'].$sSepara.$sError;}
		if ($DATA['aure69correccion']==''){$sError=$ERR['aure69correccion'].$sSepara.$sError;}
		if ($DATA['aure69menor']==''){$sError=$ERR['aure69menor'].$sSepara.$sError;}
		if ($DATA['aure69mayor']==''){$sError=$ERR['aure69mayor'].$sSepara.$sError;}
		if ($DATA['aure69verupd']==''){$sError=$ERR['aure69verupd'].$sSepara.$sError;}
		if (!fecha_esvalida($DATA['aure69fecha'])){
			//$DATA['aure69fecha']='00/00/0000';
			$sError=$ERR['aure69fecha'].$sSepara.$sError;
			}
		if ($DATA['aure69idsistema']==''){$sError=$ERR['aure69idsistema'].$sSepara.$sError;}
	...
	if ($sError==''){
		if ($DATA['paso']==10){
			if ($DATA['aure69consec']==''){
				...
				if ($DATA['aure69consec']==-1){$sError=$objDB->serror;}
				$bQuitarCodigo=true;
	...
	if ($sError==''){
		if ($DATA['paso']==10){
			...
			if ($DATA['aure69id']==-1){$sError=$objDB->serror;}
			}
		}
	if ($sError==''){
		...
		if ($DATA['paso']==10){
			...
			aure69publico, aure69idmodulo, aure69fechavigencia, aure69enlace';
			...
			"'.$DATA['aure69publico'].'", '.$DATA['aure69idmodulo'].', "'.$DATA['aure69fechavigencia'].'", "'.$DATA['aure69enlace'].'"';
			if ($APP->utf8==1){
				$sSQL='INSERT INTO aure69versionado ('.$sCampos269.') VALUES ('.utf8_encode($sValores269).');';
				$sdetalle=$sCampos269.'['.utf8_encode($sValores269).']';
				}else{
			...
			if ($bPasa){
				if ($APP->utf8==1){
					...
					$sSQL='UPDATE aure69versionado SET '.utf8_encode($sdatos).' WHERE '.$sWhere.';';
					}else{
		...
		if ($bPasa){
			if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Guardar 269 '.$sSQL.'<br>';}
			$result=$objDB->ejecutasql($sSQL);
			if ($result==false){
				...
				if ($bAudita[$idAccion]){seg_auditar($iCodModulo, $_SESSION['unad_id_tercero'], $idAccion, $DATA['aure69id'], $sdetalle, $objDB);}
				$DATA['paso']=2;
				}
			}else{
			$DATA['paso']=2;
...
function f269_db_Eliminar($aure69id, $objDB, $bDebug=false){
	...
	if ($sError==''){
		...
		if ($result==false){
			...
			if ($bAudita[4]){seg_auditar($iCodModulo, $_SESSION['unad_id_tercero'], 4, $aure69id, $sWhere, $objDB);}
			}
		}
	return array($sError, $iTipoError, $sDebug);
	}
function f269_TituloBusqueda(){
	return 'Busqueda de Versionado';
	}
...
function f269_TablaDetalleBusquedas($aParametros, $objDB){
	...
	if ($sLeyenda!=''){
		...
		return array($sLeyenda.'<input id="paginaf269" name="paginaf269" type="hidden" value="'.$pagina.'"/><input id="lppf269" name="lppf269" type="hidden" value="'.$lineastabla.'"/>', $sDebug);
		die();
		}
	$sSQLadd='';
	$sSQLadd1='';
...
function f269_TablaDetalleV2Sistema($aParametros, $objDB, $bDebug=false){
	...
	*/
	...
	<input id="titulos_269" name="titulos_269" type="hidden" value="'.$sTitulos.'"/>';
	$tabladetalle=$objDB->ejecutasql($sSQL.$sLimite);
	if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Mensajes de ayuda no vistos: '.$sSQL.$sLimite.'<br>';}
	if ($tabladetalle==false){
		$registros=0;
		$sErrConsulta=$sErrConsulta.'..<input id="err" name="err" type="hidden" value="'.$sSQL.' '.$objDB->serror.'"/>';
		//$sLeyenda=$sSQL;
