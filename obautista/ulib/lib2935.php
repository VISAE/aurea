...
function f2935_HTMLComboV2_visa35idcentro($objDB, $objCombos, $valor, $vrvisa35idzona)
{
	...
	$objCombos->nuevo('visa35idcentro', $valor, true, '{' . $ETI['msg_todos'] . '}', 0);
...
function f2935_HTMLComboV2_visa35idprograma($objDB, $objCombos, $valor, $vrvisa35idescuela)
{
	...
	require $mensajes_todas;
	$objCombos->nuevo('visa35idprograma', $valor, true, '{' . $ETI['msg_todos'] . '}', 0);
	//$objCombos->iAncho = 450;
	$sSQL = '';
...
function f2935_HTMLComboV2_visa35nivelforma($objDB, $objCombos, $valor, $vrvisa35gruponivel)
{
	...
	require $mensajes_todas;
	$objCombos->nuevo('visa35nivelforma', $valor, true, '{' . $ETI['msg_todos'] . '}', 0);
	//$objCombos->iAncho = 450;
	$sSQL = '';
...
function f2935_Combovisa35idcentro($aParametros)
{
	...
	$objResponse->call('$("#visa35idcentro").chosen({width:"100%"})');
	return $objResponse;
}
function f2935_Combovisa35idprograma($aParametros)
{
	...
	$objResponse->call('$("#visa35idprograma").chosen({width:"100%"})');
	return $objResponse;
}
function f2935_Combovisa35nivelforma($aParametros)
{
	...
	$objResponse->call('$("#visa35nivelforma").chosen({width:"100%"})');
	return $objResponse;
}
...
function f2935_db_GuardarV2b($DATA, $objDB, $bDebug = false, $idTercero = 0, $iCodModulo = 2935)
{
	...
	*/
	...
	if ($DATA['visa35estado'] == '') {
		$sError = $ERR['visa35estado'] . $sSepara . $sError;
	}
	if ($DATA['visa35fecha_cierra'] < $DATA['visa35fecha_apertura']) {
		$sError = $ERR['visa35fecha_cierra'] . $sSepara . $sError;
	}
	if ($DATA['visa35fecha_liminscrip'] > $DATA['visa35fecha_cierra']) {
		$sError = $ERR['visa35fecha_liminscrip'] . $sSepara . $sError;
	}
	...
	if ($sError == '') {
		...
		if ($DATA['paso'] == 10) {
			...
			$sDato[18] = $DATA['visa35idproducto'];
			$iNumCamposMod = 18;
