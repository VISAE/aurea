...
Config.prototype.controls = {
    ...
    about: {
        exec: function (editor) {
            ...
            dialog.setContent("<div class=\"jodit_about\">\n\t\t\t\t\t<div style=\"display: inline-block; width:200px;\"><div>" + i18n('Jodit Editor') + " v." + editor.getVersion() + "</div>\n\t\t\t\t\t<div>" + i18n('License: %s', !helpers_1.isLicense(editor.options.license)
                ? 'MIT'
                : helpers_1.normalizeLicense(editor.options.license)) + "</div>\n\t\t\t\t\t<div>\n\t\t\t\t\t\t<a href=\"https://xdsoft.net/jodit/\" target=\"_blank\">http://xdsoft.net/jodit/</a>\n\t\t\t\t\t</div>\n\t\t\t\t\t<div>\n\t\t\t\t\t\t<a href=\"https://xdsoft.net/jodit/doc/\" target=\"_blank\">" + i18n("Jodit User's Guide") + "</a>\n\t\t\t\t\t\t" + i18n('contains detailed help for using') + "\n\t\t\t\t\t</div>\n\t\t\t\t\t<div>" + i18n('Copyright © XDSoft.net - Chupurnov Valeriy. All rights reserved.') +"</div></div><div style=\"display: inline-block;\"><img src='template/imagenes/unad/unad_logo_color.png' height='120' alt='Logo UNAD-VISAE' /></div></div>");
            dialog.open();
        },
        tooltip: 'About Jodit',
        mode: consts.MODE_SOURCE + consts.MODE_WYSIWYG
                        ...
                        switch (_a.label) {
                            case 0:
                                ...
                                if(url.includes("https://drive.google.com/")) {
                                    var find = ["file/d/","/view?usp=sharing","/view?usp=share_link"];
                                    var replace = ["uc?id=","",""];
                                    url = url.replace(new RegExp("(" + find.map(function(i){return i.replace(/[.?*+^$[\]\\(){}|-]/g, "\\$&")}).join("|") + ")", "g"), function(s){ return replace[find.indexOf(s)]});
                                }
