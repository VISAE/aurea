...
function f3059_db_Guardar($valores, $objDB, $bDebug = false)
{
	...
	if ($saiu59opcional == '') {
		$sError = $ERR['saiu59opcional'] . $sSepara . $sError;
	}
	if ($saiu59idtipodoc == '') {
		$sError = $ERR['saiu59idtipodoc'] . $sSepara . $sError;
	}
	...
	if ($saiu59idtramite == '') {
		$sError = $ERR['saiu59idtramite'] . $sSepara . $sError;
	}
	if ($sError == '') {
	...
	if ($sError == '') {
		if ($bInserta) {
			...
			"' . $saiu59idrevisa . '", "' . $saiu59fecharevisa . '"';
			if ($APP->utf8 == 1) {
				$sSQL = 'INSERT INTO ' . $sTabla59 . ' (' . $sCampos3059 . ') VALUES (' . cadena_codificar($sValores3059) . ');';
			} else {
				$sSQL = 'INSERT INTO ' . $sTabla59 . ' (' . $sCampos3059 . ') VALUES (' . $sValores3059 . ');';
			}
			if ($bDebug) {
				$sDebug = $sDebug . fecha_microtiempo() . ' Guardar 3059 ' . $sSQL . '<br>';
			}
			$result = $objDB->ejecutasql($sSQL);
			if ($result == false) {
			...
			if ($bpasa) {
				if ($APP->utf8 == 1) {
					$sSQL = 'UPDATE ' . $sTabla59 . ' SET ' . cadena_codificar($sdatos) . ' WHERE ' . $sWhere . ';';
				} else {
...
function f3059_TablaDetalleV2($aParametros, $objDB, $bDebug = false)
{
		...
		if ($objDB->nf($tabla) > 0) {
			...
			switch ($fila['saiu47estado']) {
				...
				case 5: // Devuelta
					...
					$bAbierta = true;
					break;
				case 3: //Aprobación de docume
					$bConRevision = true;
					break;
				case 4: // En concepto Jurí
				case 6: // En concepto técnico
				case 7: // En proyección de
				case 8: // En aprobación de la GAF
				case 11: // En firma de la GAF
	...
	*/
	...
	TB.saiu59idrevisa, T4.saiu51proveedor 
	...
	$sErrConsulta = $sErrConsulta . '<input id="titulos_3059" name="titulos_3059" type="hidden" value="' . $sTitulos . '"/>';
	if ($bDebug) {
		$sDebug = $sDebug . fecha_microtiempo() . ' Consulta 3059: ' . $sSQL . '<br>';
	}
	...
	if ($tabladetalle == false) {
		...
		if ($registros == 0) {
			return array(cadena_codificar($sErrConsulta . '<input id="paginaf3059" name="paginaf3059" type="hidden" value="' . $pagina . '"/><input id="lppf3059" name="lppf3059" type="hidden" value="' . $lineastabla . '"/>'), $sDebug);
		}
	...
	while ($filadet = $objDB->sf($tabladetalle)) {
		...
		if ($filadet['saiu59fecharevisa'] != 0) {
			$et_saiu59fecharevisa = $sPrefijo . fecha_desdenumero($filadet['saiu59fecharevisa']) . $sSufijo;
		}
		$bRevisaFila = $bConRevision;
		/*
		if (!$bRevisaFila) {
			if ($filadet['saiu51proveedor'] != 0) {
				$bRevisaFila = $bConEditar;
			}
		}
		*/
		if ($bRevisaFila) {
		...
		$res = $res . '<tr' . $sClass . '>';
		$res = $res . '<td>' . $et_saiu59consec . '</td>';
		$res = $res . '<td>' . $et_saiu59idtipodoc . '</td>';
		$res = $res . '<td>' . $et_saiu59opcional . '</td>';
		$res = $res . '<td>' . $sBotonAnexa . '</td>';
		$res = $res . '<td>' . $et_saiu59idarchivo . '</td>';
		$res = $res . '<td>' . $et_saiu59fecharevisa . '</td>';
		$res = $res . '<td>' . $sLink . '</td>';
...
function f3059_Traer($aParametros)
{
	...
	if ($paso == 1) {
		$saiu59idtramite = numeros_validar($aParametros[1]);
		$saiu59consec = numeros_validar($aParametros[2]);
		if (($saiu59idtramite != '') && ($saiu59consec != '')) {
			$besta = true;
		}
		$sError = 'No se encontro el consecutivo: ' . $saiu59consec . '';
	} else {
		$saiu59id = $aParametros[103];
		if ((int)$saiu59id != 0) {
			$besta = true;
		}
		$sError = 'No se encontro el registro de referencia: ' . $saiu59id . '';
	}
	if ($besta) {
		$objDB = new clsdbadmin($APP->dbhost, $APP->dbuser, $APP->dbpass, $APP->dbname);
		if ($APP->dbpuerto != '') {
	...
	if ($besta) {
		...
		$sSQL = 'SELECT TB.*, T51.saiu51proveedor, T51.saiu51nombre 
		FROM ' . $sTabla59 . ' AS TB, saiu51tramitedoc AS T51 
		WHERE ' . $sSQLcondi .' AND TB.saiu59idtipodoc=T51.saiu51id  ';
		$tabla = $objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			switch($fila['saiu51proveedor']) {
				case 0: // Estudiante
					$sError = 'Los documentos aportados por estudiantes [' . cadena_notildes($fila['saiu51nombre']) . '] no son editables.';
					break;
				default:
					$besta = true;
					break;
...
function f3059_TablaDetalleV2Campus($aParametros, $objDB, $bDebug = false)
{
	...
	*/
	...
	$sSQL = 'SELECT TB.saiu59consec, TB.saiu59id, T4.saiu51nombre, TB.saiu59opcional, TB.saiu59idorigen, 
	TB.saiu59idarchivo, TB.saiu59fecha, TB.saiu59fecharevisa, TB.saiu59idtipodoc, TB.saiu59idestado, TB.saiu59idusuario, 
	TB.saiu59idrevisa, T4.saiu51opcional 
	FROM ' . $sTabla59 . ' AS TB, saiu51tramitedoc AS T4 
	WHERE ' . $sSQLadd1 . ' TB.saiu59idtramite=' . $saiu47id . ' AND TB.saiu59idtipodoc=T4.saiu51id 
	AND ((T4.saiu51proveedor=0) OR (T4.saiu51proveedor<>0 AND T4.saiu51visible=1)) ' . $sSQLadd . '
	ORDER BY TB.saiu59consec';
	$sSQLlista = str_replace("'", "|", $sSQL);
	$sSQLlista = str_replace('"', "|", $sSQLlista);
	$sErrConsulta = '<input id="consulta_3059" name="consulta_3059" type="hidden" value="' . $sSQLlista . '"/>';
