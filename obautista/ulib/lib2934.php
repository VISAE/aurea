...
function f2934_db_GuardarV2b($DATA, $objDB, $bDebug = false, $idTercero = 0, $iCodModulo = 2934)
{
	...
	*/
	...
	$bConRol = false;
	if ($DATA['visa34rolexterno'] == 1) {
		$bConRol = true;
	}
	if ($DATA['visa34rolegresado'] == 1) {
		$bConRol = true;
	}
	if ($DATA['visa34rolaspirante'] == 1) {
		$bConRol = true;
	}
	if ($DATA['visa34rolacademico'] == 1) {
		$bConRol = true;
	}
	if ($DATA['visa34roladministrativo'] == 1) {
		$bConRol = true;
	}
	if ($DATA['visa34rolestudiante'] == 1) {
		$bConRol = true;
	}
	if (!$bConRol) {
		$sError = $ERR['visa34sinrol'] . $sSepara . $sError;
	}
