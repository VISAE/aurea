...
--- Modelo Versión 2.19.7c viernes, 26 de enero de 2018
--- 1711 Actores
*/
...
<div class="salto1px"></div>
	...
	*/
	...
	if ($params[104]==1){
		$sql='SELECT T4.ofer10nombre, TB.ofer11id, T6.unad11razonsocial AS C6_nombre, TB.ofer11detalle, TB.ofer11per_aca, TB.ofer11idescuela, TB.ofer11idcurso, TB.ofer11idrol, TB.ofer11idtercero, T6.unad11tipodoc AS C6_td, T6.unad11doc AS C6_doc, T6.unad11telefono, T6.unad11correo, T6.unad11correoinstitucional, T6.unad11correonotifica, T6.unad11aceptanotificacion, TB.ofer11fecharegistro, T6.unad11mostrarcelular 
...
WHERE TB.ofer11per_aca="'.$_SESSION['oai_per_aca'].'" '.$sqladd1.' AND TB.ofer11idrol=T4.ofer10id AND TB.ofer11idtercero=T6.unad11id '.$sqladd.'';
		$tabladetalle=$objdb->ejecutasql($sql);
		$registros=$objdb->nf($tabladetalle);
		$sql='SELECT T2.exte01nombre, T3.unad40nombre, T4.ofer10nombre, TB.ofer11id, T6.unad11razonsocial AS C6_nombre, TB.ofer11detalle, TB.ofer11per_aca, TB.ofer11idescuela, TB.ofer11idcurso, TB.ofer11idrol, TB.ofer11idtercero, T6.unad11tipodoc AS C6_td, T6.unad11doc AS C6_doc, TB.ofer11idcurso, T6.unad11telefono, T6.unad11correo, T6.unad11correoinstitucional, T6.unad11correonotifica, T6.unad11aceptanotificacion, TB.ofer11fecharegistro, T6.unad11mostrarcelular
...
<td>'.$sprefijo.$filadet['ofer11idcurso'].' '.cadena_notildes($filadet['unad40nombre']).$ssufijo.'</td>';
		...
		if (cadena_contiene($et_correo, 'unad.edu.co')){
			}else{
		...
		$sCols='';
		if ($bMarcaGestor){
			$sCols='<td><a href="javascript:marcargestor(1, '.$filadet['ofer11id'].')" class="lnkresalte">Gestor</a></td>
<td><a href="javascript:marcargestor(2, '.$filadet['ofer11id'].')" class="lnkresalte">Director</a></td>';
			}
		if ($babierta){
...
function f1711_Guardar($valores, $params){
	...
	if ($sError==''){
		...
		$bHayDb=true;
		//$iAccion,
		list($sError, $ofer11id, $sDebugGuardar)=f1711_db_Guardar($valores, $objdb, $bDebug);
		$sDebug=$sDebug.$sDebugGuardar;
		}
	$objResponse=new xajaxResponse();
	if ($sError==''){
		list($sdetalle, $sDebugTabla)=f1711_TablaDetalleV2($params, $objdb, $bDebug);
		$sDebug=$sDebug.$sDebugTabla;
		$objResponse->assign('div_f1711detalle', 'innerHTML', $sdetalle);
		//if ($iAccion==2){
			//$objResponse->call('cargaridf1711('.$ofer11id.')');
			//}else{
...
function f1711_Traer($params){
	...
	if ($besta){
		...
		if ($ofer11idtercero_id!=0){
			list($ofer11idtercero_nombre, $ofer11idtercero_id, $ofer11idtercero_td, $ofer11idtercero_doc)=html_tercero($ofer11idtercero_td, $ofer11idtercero_doc, $ofer11idtercero_id, 0, $objdb);
			}
		...
		if ($paso==1){
			...
			$objResponse->call('MensajeAlarmaV2("No se encontro el registro de referencia:'.$ofer11id.'", 0)');
			}
		}
	if ($bHayDb){
		$objdb->CerrarConexion();
...
function f1711_PintarLlaves($params){
	$_SESSION['u_ultimominuto']=iminutoavance();
	//if(!is_array($params)){$params=json_decode(str_replace('\"','"',$params),true);}
	require './app.php';
	$objdb=new clsdbadmin($APP->dbhost, $APP->dbuser, $APP->dbpass, $APP->dbname);
