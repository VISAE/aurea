<?php
/*
--- © Angel Mauro Avellaneda Barreto - UNAD - 2021 - 2025 ---
--- angel.avellaneda@unad.edu.co - http://www.unad.edu.co
--- Modelo Version 2.25.10c lunes, 3 de mayo de 2021
--- Modelo Version 2.28.2 martes, 24 de mayo de 2022
--- Modelo Version 2.28.2 viernes, 22 de julio de 2022
--- Modelo 3.0.13b jueves, 27 de febrero de 2025
*/
$ETI['app_nombre_3047'] = 'SAI';
$ETI['grupo_nombre_3047'] = 'Tr&aacute;mites';
$ETI['app_nombre']='APP';
$ETI['grupo_nombre']='Grupo';
$ETI['titulo_tram']='Tramites';
$ETI['titulo']='Saldos a Favor y Devoluciones';
$ETI['titulo_dev']='Saldos a aplicar';
$ETI['titulo_sector2']='Saldos a Favor y Devoluciones';
$ETI['titulo_sector93']='Cambio de consecutivo';
$ETI['titulo_3047']='Saldos a Favor y Devoluciones';
$ETI['titulo_3047_cr']='Confirmaci&oacute;n de recaudos';
$ETI['titulo_3047_dev']='Saldos a Favor y Devoluciones';

$ETI['titulo']='Saldos a aplicar';
$ETI['titulo_sector2']='Saldos a aplicar';
$ETI['titulo_sector3'] = 'Anotaci&oacute;n de devoluci&oacute;n';
$ETI['titulo_sector4'] = 'Anotaci&oacute;n de No Procedencia';
//$ETI['titulo_3047']='Saldos a aplicar';
//$ETI['titulo_3047_dev']='Saldos a aplicar';

$ETI['sigla_3047']='Tramites';
$ETI['sigla_3047_dev']='Devoluciones';
$ETI['bt_ter_buscar']='Buscar tercero';
$ETI['bt_ter_crear']='Crear tercero';
$ETI['lnk_cargar']='Editar';
$ETI['saiu47agno']='A&ntilde;o';
$ETI['saiu47mes']='Mes';
$ETI['saiu47tiporadicado']='Tipo radicado';
$ETI['saiu47tipotramite']='Tipo tramite';
$ETI['saiu47consec']='Consecutivo';
$ETI['msg_saiu47consec']='Consecutivo actual';
$ETI['msg_saiu47consec_nuevo']='Nuevo consecutivo';
$ETI['saiu47id']='Ref :';
$ETI['saiu47origenagno']='Origen agno';
$ETI['saiu47origenmes']='Origen mes';
$ETI['saiu47origenid']='Origen id';
$ETI['saiu47dia']='Dia';
$ETI['saiu47hora']='Hora';
$ETI['saiu47minuto']='Minuto';
$ETI['saiu47idsolicitante']='Solicitante';
$ETI['saiu47idsolicitante_busca'] = 'Busqueda de Solicitante';
$ETI['saiu47idperiodo']='Periodo';
$ETI['saiu47idescuela']='Escuela';
$ETI['saiu47idprograma']='Programa';
$ETI['saiu47idzona']='Zona';
$ETI['saiu47idcentro']='Centro';
$ETI['saiu47estado']='Estado';
$ETI['saiu47t1idmotivo']='Motivo';
$ETI['saiu47t1vrsolicitado']='Vr Solicitado';
$ETI['saiu47t1vraprobado']='Vr Aprobado';
$ETI['saiu47t1vrsaldoafavor']='Vr saldo a favor';
$ETI['saiu47t1vrdevolucion']='Vr devoluci&oacute;n';
$ETI['saiu47idbenefdevol']='Beneficiario';
$ETI['saiu47idbenefdevol_busca'] = 'Busqueda de Benefdevol';
$ETI['saiu47idaprueba']='Aprobado por';
$ETI['saiu47idaprueba_busca'] = 'Busqueda de Aprueba';
$ETI['saiu47fechaaprueba']='Fecha';
$ETI['saiu47horaaprueba']='Hora';
$ETI['saiu47minutoaprueba']='Minuto';
$ETI['saiu47detalle']='Detalle';
$ETI['saiu47idunidad'] = 'Unidad';
$ETI['saiu47idgrupotrabajo'] = 'Grupo trabajo';
$ETI['saiu47idresponsable'] = 'Responsable';
$ETI['saiu47t707fecha'] = 'Fecha pago';
$ETI['saiu47t707formarecaudo'] = 'Forma recaudo';
$ETI['saiu47t707identidadconv'] = 'En convenio con';
$ETI['saiu47t707idbanco'] = 'Banco';
$ETI['saiu47t707idcuenta'] = 'Cuenta';
$ETI['saiu47prob_abandono'] = 'Posible abandono';
$ETI['saiu47tem_radicado'] = 'Radicado en';
$ETI['msg_cierre3047'] = '&iquest;Est&aacute; seguro de radicar la solicitud?';

$ERR['saiu47agno']='Necesita el dato '.$ETI['saiu47agno'];
$ERR['saiu47mes']='Necesita el dato '.$ETI['saiu47mes'];
$ERR['saiu47tiporadicado']='Necesita el dato '.$ETI['saiu47tiporadicado'];
$ERR['saiu47tipotramite']='Necesita el dato '.$ETI['saiu47tipotramite'];
$ERR['saiu47consec']='Necesita el dato '.$ETI['saiu47consec'];
$ERR['saiu47id']='Necesita el dato '.$ETI['saiu47id'];
$ERR['saiu47origenagno']='Necesita el dato '.$ETI['saiu47origenagno'];
$ERR['saiu47origenmes']='Necesita el dato '.$ETI['saiu47origenmes'];
$ERR['saiu47origenid']='Necesita el dato '.$ETI['saiu47origenid'];
$ERR['saiu47dia']='Necesita el dato '.$ETI['saiu47dia'];
$ERR['saiu47hora']='Necesita el dato '.$ETI['saiu47hora'];
$ERR['saiu47minuto']='Necesita el dato '.$ETI['saiu47minuto'];
$ERR['saiu47idsolicitante']='Necesita el dato '.$ETI['saiu47idsolicitante'];
$ERR['saiu47idperiodo']='Necesita el dato '.$ETI['saiu47idperiodo'];
$ERR['saiu47idescuela']='Necesita el dato '.$ETI['saiu47idescuela'];
$ERR['saiu47idprograma']='Necesita el dato '.$ETI['saiu47idprograma'];
$ERR['saiu47idzona']='Necesita el dato '.$ETI['saiu47idzona'];
$ERR['saiu47idcentro']='Necesita el dato '.$ETI['saiu47idcentro'];
$ERR['saiu47estado']='Necesita el dato '.$ETI['saiu47estado'];
$ERR['saiu47t1idmotivo']='Necesita el dato '.$ETI['saiu47t1idmotivo'];
$ERR['saiu47t1vrsolicitado']='Necesita el dato '.$ETI['saiu47t1vrsolicitado'];
$ERR['saiu47t1vraprobado']='Necesita el dato '.$ETI['saiu47t1vraprobado'];
$ERR['saiu47t1vrsaldoafavor']='Necesita el dato '.$ETI['saiu47t1vrsaldoafavor'];
$ERR['saiu47t1vrdevolucion']='Necesita el dato '.$ETI['saiu47t1vrdevolucion'];
$ERR['saiu47idbenefdevol']='Necesita el dato '.$ETI['saiu47idbenefdevol'];
$ERR['saiu47idaprueba']='Necesita el dato '.$ETI['saiu47idaprueba'];
$ERR['saiu47fechaaprueba']='Necesita el dato '.$ETI['saiu47fechaaprueba'];
$ERR['saiu47horaaprueba']='Necesita el dato '.$ETI['saiu47horaaprueba'];
$ERR['saiu47minutoaprueba']='Necesita el dato '.$ETI['saiu47minutoaprueba'];
$ERR['saiu47detalle']='Necesita el dato '.$ETI['saiu47detalle'];
$ERR['saiu47idunidad'] = 'Necesita el dato ' . $ETI['saiu47idunidad'];
$ERR['saiu47idgrupotrabajo'] = 'Necesita el dato ' . $ETI['saiu47idgrupotrabajo'];
$ERR['saiu47idresponsable'] = 'Necesita el dato ' . $ETI['saiu47idresponsable'];
$ERR['saiu47t707fecha'] = $ETI['saiu47t707fecha'] . ' incorrecta';
$ERR['saiu47t707formarecaudo'] = 'Necesita el dato ' . $ETI['saiu47t707formarecaudo'];
$ERR['saiu47t707identidadconv'] = 'Necesita el dato ' . $ETI['saiu47t707identidadconv'];
$ERR['saiu47t707idbanco'] = 'Necesita el dato ' . $ETI['saiu47t707idbanco'];
$ERR['saiu47t707idcuenta'] = 'Necesita el dato ' . $ETI['saiu47t707idcuenta'];

$ETI['msg_confirma36'] = 'Se va a enviar la solicitud a <b>Ajustes de resoluci&oacute;n</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirma41'] = 'Se va a enviar la solicitud a <b>Numeraci&oacute;n SGENERAL</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirma46'] = 'Se va a enviar la solicitud a <b>Firma de la VISAE</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirma51'] = 'Se va a enviar la solicitud a <b>Contabilidad</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirmaresuelto'] = 'Se va a enviar la solicitud a <b>Solicitud Resuelta</b>, <br>Esta seguro de continuar?';
$ETI['msg_aprobardoc'] = 'El documento ser&aacute; aprobado,<br>Esta seguro de continuar?';
$ETI['msg_confirmaproyeccion'] = 'Se cambiar&aacute; el estado a <b>En Proyecci&oacute;n de Resoluci&oacute;n</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirmacptojur'] = 'Se va a enviar la solicitud a <b>Concepto Jur&iacute;dico</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirmacptotecnico'] = 'Se va a enviar la solicitud a <b>Concepto T&eacute;cnico</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirmadevrca'] = 'Se va a devolver la solicitud a <b>Ajustes por parte de RCONT</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirmafirma'] = 'Se va a enviar la solicitud a <b>Firma de la GAF</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirmarnoprocedente'] = 'Se va a marcar la solicitud como <b>NO PROCEDENTE</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirmapago'] = 'Se va a enviar la solicitud a <b>Proceso de Pago</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirmaproyrta'] = 'Se va a enviar la solicitud a <b>Proyecci&oacute;n de respuesta</b>, <br>Esta seguro de continuar?';
$ETI['msg_confirmagaf'] = 'Se va a enviar la solicitud a <b>Aprobaci&oacute;n de la GAF</b>, <br>Esta seguro de continuar?';
$ETI['msg_devolver'] = 'Se va a marcar la solicitud como <b>DEVUELTA</b>, <br>Esta seguro de continuar?';
$ETI['msg_enrevision'] = 'Se va a marcar la solicitud a <b>En Aprobaci&oacute;n de Documentos</b>,<br>esta seguro de continuar?';
$ETI['msg_fecha']='Fecha';
$ETI['msg_historial']='Historial de solicitudes';
$ETI['msg_notadevol']='Nota de devoluci&oacute;n';
$ETI['msg_notanoprocede']='Nota de no procedente';
$ETI['msg_reasignar'] = 'Reasignaci&oacute;n de tramite';
$ETI['msg_tipo']='Tipo';
$ETI['msg_vrpagado']='Vr Pagado';

$aListar2=array('', 'Mis registros');
$iListar2=1;


$AYU['ayuda_cr']='Apreciado estudiante: Esta solicitud se debe tramitar cuando usted hace un pago diferente a los recibos de pago emitidos con codigos de barras.<br>
Favor describir en el detalle la forma de pago.';

$ETI['saiu47asignado'] = 'Asignado';
$ETI['titulo_sector2_saiu47'] = 'Reasignar responsable';

$ETI['msg_bagno'] = 'A&ntilde;o';

