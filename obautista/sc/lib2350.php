...
function f2350_HTMLComboV2_core50idcentro($objDB, $objCombos, $valor, $vrcore50idzona)
{
	...
	if ($sCondi != '') {
		$sCondi = ' WHERE ' . $sCondi . ' AND unad24activa="S"';
	}
...
function f2350_TablaDetalleV2($params, $objDB, $bDebug = false)
{
	...
	if (isset($params[101]) == 0) {
		$params[101] = 1;
	}
	if (isset($params[102]) == 0) {
		$params[102] = 20;
	}
...
<input id="titulos_2350" name="titulos_2350" type="hidden" value="' . $sTitulos . '"/>';
	...
	if ($tabladetalle == false) {
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' Consulta 2350: ' . $sql . '<br>';
		}
		$registros = 0;
		...
		if ($registros == 0) {
			//return array(cadena_codificar($sErrConsulta.'<input id="paginaf2350" name="paginaf2350" type="hidden" value="'.$pagina.'"/><input id="lppf2350" name="lppf2350" type="hidden" value="'.$lineastabla.'"/>'), $sDebug);
		}
