...
function f2307_HTMLComboV2_cara00idconsejero($objDB, $objCombos, $valor, $vrcara00zona)
{
	...
	require $mensajes_todas;
	$objCombos->nuevo('cara00idconsejero', $valor, true, '{' . $ETI['msg_todos'] . '}', '-1');
	//$objCombos->iAncho=450;
...
function f2307_Combocara00idconsejero($aParametros)
{
	...
	$objResponse->call('$("#cara00idconsejero").chosen({width: "100%"})');
	return $objResponse;
}
...
<input id="titulos_2300" name="titulos_2300" type="hidden" value="' . $sTitulos . '"/>';
	...
	if ($tabladetalle == false) {
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' Consulta 2300: ' . $sSQL . '<br>';
		}
		$registros = 0;
...
function f2300_db_GuardarV2($DATA, $objDB, $bDebug = false)
{
	...
	*/
	...
	$sSepara = ', ';
	if (true) {
		if ($DATA['cara00idperfillider'] == '') {
			$sError = $ERR['cara00idperfillider'] . $sSepara . $sError;
		}
		if ($DATA['cara00idperfilconsejero'] == '') {
			$sError = $ERR['cara00idperfilconsejero'] . $sSepara . $sError;
		}
	...
	if ($sError == '') {
		...
		if ($bpasa) {
			...
			if ($result == false) {
				...
				if ($bDebug) {
					$sDebug = $sDebug . fecha_microtiempo() . ' Guardar 2300 ' . $sSQL . '<br>';
				}
				if ($bAudita[$idaccion]) {
...
function f2300_ProcesarEdades($objDB, $bDebug = false)
{
	...
	if ($objDBRyC->Conectar()) {
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' Conectado con la db de Registro y control<br>';
		}
		$bHayDBRyC = true;
	} else {
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' <b>Procesar Edades</b> Falla al conectar con RCONT: ' . $objDBRyC->serror . '<br>';
		}
	...
	if ($sError == '') {
		...
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' <b>Procesar Edades</b> Datos a revisar ' . $sSQL . '<br>';
		}
		$tabla = $objDB->ejecutasql($sSQL);
		while ($fila = $objDB->sf($tabla)) {
			...
			if (fecha_esvalida($fila['unad11fechanace'])) {
				...
				if ($bHayDBRyC) {
					...
					if ($objDBRyC->nf($tablaRyC) > 0) {
						...
						if ($bDebug) {
							$sDebug = $sDebug . fecha_microtiempo() . ' Actualizando fecha de nacimiento: ' . $sSQL . '<br>';
						}
						$result = $objDB->ejecutasql($sSQL);
					}
				}
			...
			if ($cara01agnos != 0) {
				...
				if ($bDebug) {
					$sDebug = $sDebug . fecha_microtiempo() . ' Actualizando encuesta: ' . $sSQL . '<br>';
				}
				$result = $objDB->ejecutasql($sSQL);
				$iProcesados++;
			}
...
function f2307_TablaDetalleV2($aParametros, $objDB, $bDebug = false)
{
	...
	//}
	$sLeyenda = '';
	if ((int)$idConsejero == 0) {
		$sLeyenda = 'No ha seleccionado un consejero a consultar';
	}
	if ($sLeyenda != '') {
		$sLeyenda = '<div class="salto1px"></div>
<div class="GrupoCamposAyuda">
...
WHERE core20idtutor=' . $idConsejero . ' AND core20idperaca=' . $idPeriodo . ' AND core20idcurso IN (' . $sIdsCursosCatedra . ')';
		...
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' Consulta ENCUESTAS: ' . $sSQL . '<br>';
		}
		$tabla1 = $objDB->ejecutasql($sSQL);
...
GROUP BY cara23idtipo';
			if ($bDebug) {
				$sDebug = $sDebug . fecha_microtiempo() . ' Consulta ESTUDIANTES: ' . $sSQL . '<br>';
			}
			//$sRevisa=$sSQL;
			$tabla1 = $objDB->ejecutasql($sSQL);
			while ($fila1 = $objDB->sf($tabla1)) {
...
GROUP BY cara23idtipo, cara23estado';
			if ($bDebug) {
				$sDebug = $sDebug . fecha_microtiempo() . ' Consulta EVENTOS: ' . $sSQL . '<br>';
			}
			//$iM3=$sSQL;
			$tabla1 = $objDB->ejecutasql($sSQL);
			while ($fila1 = $objDB->sf($tabla1)) {
