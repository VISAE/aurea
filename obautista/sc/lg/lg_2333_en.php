<?php
/*
--- © Angel Mauro Avellaneda Barreto - UNAD - 2019 ---
--- angel.avellaneda@unad.edu.co - http://www.unad.edu.co
--- Model Version 2.23.8 Wednesday, October 30, 2019
*/
$ETI['app_nombre']='APP';
$ETI['grupo_nombre']='Grupo';
$ETI['titulo']='Consolidado de acompanamientos';
$ETI['titulo_sector2']='Consolidado de acompanamientos';
$ETI['titulo_2333']='Consolidado de acompanamientos';
$ETI['sigla_2333']='Consolidado de acompanamientos';
$ETI['lnk_cargar']='Edit';
$ETI['cara33idperaca']='Peraca';
$ETI['cara33idzona']='Zona';
$ETI['cara33idcentro']='Centro';
$ETI['cara33idescuela']='Escuela';
$ETI['cara33idprograma']='Programa';
$ETI['cara33tipo']='Tipo';
$ETI['cara33poblacion']='Poblacion';

$ERR['cara33idperaca']='Is necessary the data '.$ETI['cara33idperaca'];
$ERR['cara33idzona']='Is necessary the data '.$ETI['cara33idzona'];
$ERR['cara33idcentro']='Is necessary the data '.$ETI['cara33idcentro'];
$ERR['cara33tipo']='Is necessary the data '.$ETI['cara33tipo'];
$ERR['cara33poblacion']='Is necessary the data '.$ETI['cara33poblacion'];
$acara33tipo=array('', '');
$icara33tipo=0;
$acara33poblacion=array('', '');
$icara33poblacion=0;
?>