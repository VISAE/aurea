...
function f2308_ExisteDato($datos)
{
	...
	if ($bHayLlave) {
		...
		$objDB->xajax();
		$sSQL = 'SELECT 1 FROM cara08pregunta WHERE cara08idbloque=' . $cara08idbloque . ' AND cara08consec=' . $cara08consec . '';
...
function f2308_TablaDetalleV2($aParametros, $objDB, $bDebug = false)
{
	...
	*/
	$sLeyenda = '';
	$sBotones = '<input id="paginaf2308" name="paginaf2308" type="hidden" value="' . $pagina . '"/>';
	$sBotones = $sBotones . '<input id="lppf2308" name="lppf2308" type="hidden" value="' . $lineastabla . '"/>';
	if ($sLeyenda != '') {
	...
	*/
	if ($aParametros[104] != '') {
		$sSQLadd = $sSQLadd . ' AND TB.cara08titulo LIKE "%' . $aParametros[104] . '%"';
	}
	if ($aParametros[105] != '') {
	...
	$sLimite = '';
	$sCampos = 'SELECT TB.cara08idbloque, TB.cara08consec, TB.cara08id, TB.cara08activa, TB.cara08titulo, TB.cara08tipopreg, TB.cara08usosiniciales, TB.cara08usostotales, TB.cara08idgrupo'; 
	$sConsulta = 'FROM cara08pregunta AS TB 
	WHERE ' . $sSQLadd1 . 'TB.cara08idbloque=' . $aParametros[103] . ' ' . $sSQLadd . '';
	$sOrden = 'ORDER BY TB.cara08idbloque, TB.cara08consec';
	...
	if ($bGigante) {
		...
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' Totalizando consulta 2308: ' . $sSQLContador . '<br>';
		}
	...
	$sErrConsulta = $sErrConsulta . '<input id="titulos_2308" name="titulos_2308" type="hidden" value="' . $sTitulos . '"/>';
	if ($bDebug) {
		$sDebug = $sDebug . fecha_microtiempo() . ' Consulta 2308: ' . $sSQL . '<br>';
	}
	...
	if ($bDebug) {
		$sDebug = $sDebug . fecha_microtiempo() . ' Termina la consulta 2308<br>';
	}
	...
	$res = $res . '<th><b>' . $ETI['cara08consec'] . '</b></th>';
	$res = $res . '<th><b>' . $ETI['cara08activa'] . '</b></th>';
	$res = $res . '<th><b>' . $ETI['cara08titulo'] . '</b></th>';
	$res = $res . '<th colspan="2"><b>' . $ETI['msg_usos'] . '</b></th>';
	$res = $res . '<th colspan="2" class="text-right">';
	$res = $res . html_paginador('paginaf2308', $registros, $lineastabla, $pagina, 'paginarf2308()');
	$res = $res . html_lpp('lppf2308', $lineastabla, 'paginarf2308()');
	$res = $res . '</th>';
	...
	while ($filadet = $objDB->sf($tabladetalle)) {
		...
		$tlinea++;
		$et_cara08consec = $sPrefijo . $filadet['cara08consec'] . $sSufijo;
		$et_cara08activa = $sPrefijo . $ETI['no'] . $sSufijo;
		if ($filadet['cara08activa'] == 'S') {
			$et_cara08activa = $sPrefijo . $ETI['si'] . $sSufijo;
		}
		$sVisor = '<a href="javascript:verpregunta(' . $filadet['cara08id'] . ')" class="lnkresalte">' . $ETI['lnk_visor'] . '</a>';
		$et_cara08titulo = $sPrefijo . cadena_notildes($filadet['cara08titulo']) . $sSufijo;
		$et_cara08usosiniciales = 0;
		if ($filadet['cara08usosiniciales'] != 0) {
			$et_cara08usosiniciales = $sPrefijo . formato_numero($filadet['cara08usosiniciales']) . $sSufijo;
		}
		$et_cara08usostotales = 0;
		if ($filadet['cara08usostotales'] != 0) {
			$et_cara08usostotales = $sPrefijo . formato_numero($filadet['cara08usostotales']) . $sSufijo;
		}
		if ($bAbierta) {
			$sLink = '<a href="javascript:cargaridf2308(' . $filadet['cara08id'] . ')" class="lnkresalte">' . $ETI['lnk_cargar'] . '</a>';
		}
		$res = $res . '<tr' . $sClass . '>';
		$res = $res . '<td>' . $et_cara08consec . '</td>';
		$res = $res . '<td>' . $et_cara08activa . '</td>';
		$res = $res . '<td>' . $et_cara08titulo . '</td>';
		$res = $res . '<td>' . $et_cara08usosiniciales . '</td>';
		$res = $res . '<td>' . $et_cara08usostotales . '</td>';
		$res = $res . '<td align="right">' . $sVisor . '</td>';
...
function f2308_db_CargarPadre($DATA, $objDB, $bDebug = false)
{
	...
	if ($objDB->nf($tabla) > 0) {
		...
		$DATA['cara08retroalimenta'] = $fila['cara08retroalimenta'];
		$bcargo = true;
		$DATA['paso'] = 2;
		$DATA['boculta2308'] = 0;
		$bLimpiaHijos = true;
...
function f2308_db_GuardarV2($DATA, $objDB, $bDebug = false, $idTercero = 0, $iCodModulo = 2308)
{
	$bAudita[2] = true;
	$bAudita[3] = true;
	require './app.php';
	...
	/*
	if (isset($DATA['cara08idbloque']) == 0) {
		$DATA['cara08idbloque'] = 0;
	}
	if (isset($DATA['cara08consec']) == 0) {
		$DATA['cara08consec'] = '';
	}
	if (isset($DATA['cara08id']) == 0) {
		$DATA['cara08id'] = '';
	}
	if (isset($DATA['cara08activa']) == 0) {
		$DATA['cara08activa'] = '';
	}
	if (isset($DATA['cara08idgrupo']) == 0) {
		$DATA['cara08idgrupo'] = 0;
	}
	if (isset($DATA['cara08titulo']) == 0) {
		$DATA['cara08titulo'] = '';
	}
	if (isset($DATA['cara08cuerpo']) == 0) {
		$DATA['cara08cuerpo'] = '';
	}
	if (isset($DATA['cara08tipopreg']) == 0) {
		$DATA['cara08tipopreg'] = 0;
	}
	if (isset($DATA['cara08usosiniciales']) == 0) {
		$DATA['cara08usosiniciales'] = 0;
	}
	if (isset($DATA['cara08usostotales']) == 0) {
		$DATA['cara08usostotales'] = 0;
	}
	if (isset($DATA['cara08nivelpregunta']) == 0) {
		$DATA['cara08nivelpregunta'] = 0;
	}
	*/
	// -- Se inicia validando todas las posibles entradas de usuario.
	$DATA['cara08idbloque'] = numeros_validar($DATA['cara08idbloque']);
	$DATA['cara08consec'] = numeros_validar($DATA['cara08consec']);
	$DATA['cara08activa'] = cadena_Validar(trim($DATA['cara08activa']));
	$DATA['cara08idgrupo'] = numeros_validar($DATA['cara08idgrupo']);
	$DATA['cara08titulo'] = cadena_Validar(trim($DATA['cara08titulo']));
	$DATA['cara08cuerpo'] = cadena_Validar(trim($DATA['cara08cuerpo']));
	...
	$DATA['cara08retroalimenta'] = cadena_Validar($DATA['cara08retroalimenta']);
	// -- Se inicializan las variables que puedan pasar vacias {Especialmente números}.
	/*
	if ($DATA['cara08idgrupo'] == '') {
		$DATA['cara08idgrupo'] = 0;
	}
	if ($DATA['cara08tipopreg'] == '') {
		$DATA['cara08tipopreg'] = 0;
	}
	*/
	if ($DATA['cara08usosiniciales'] == '') {
		$DATA['cara08usosiniciales'] = 0;
	}
	if ($DATA['cara08usostotales'] == '') {
		$DATA['cara08usostotales'] = 0;
	}
	// -- Seccion para validar los posibles causales de error.
	$sSepara = ', ';
	if (true) {
		if ($DATA['cara08nivelpregunta'] == '') {
			$sError = $ERR['cara08nivelpregunta'] . $sSepara . $sError;
		}
		/*
		if ($DATA['cara08usostotales'] == '') {
			$sError = $ERR['cara08usostotales'] . $sSepara . $sError;
		}
		if ($DATA['cara08usosiniciales'] == '') {
			$sError = $ERR['cara08usosiniciales'] . $sSepara . $sError;
		}
		*/
		if ($DATA['cara08tipopreg'] == '') {
			$sError = $ERR['cara08tipopreg'] . $sSepara . $sError;
		}
		/*
		if ($DATA['cara08cuerpo'] == '') {
			$sError = $ERR['cara08cuerpo'] . $sSepara . $sError;
		}
		*/
		if ($DATA['cara08titulo'] == '') {
			$sError = $ERR['cara08titulo'] . $sSepara . $sError;
		}
		if ($DATA['cara08idgrupo'] == '') {
			$sError = $ERR['cara08idgrupo'] . $sSepara . $sError;
		}
		if ($DATA['cara08activa'] == '') {
			$sError = $ERR['cara08activa'] . $sSepara . $sError;
		}
		//Fin de las valiaciones NO LLAVE.
	}
	//Valiaciones de campos obligatorios en todo guardar.
	if ($DATA['cara08idbloque'] == '') {
		$sError = $ERR['cara08idbloque'];
	}
	...
	if ($sError == '') {
		...
		$cara08retroalimenta = addslashes($DATA['cara08retroalimenta']);
		$cara08retroalimenta = str_replace('"', '\"', $DATA['cara08retroalimenta']);
		$bPasa = false;
		if ($DATA['paso'] == 10) {
			$sCampos2308 = 'cara08idbloque, cara08consec, cara08id, cara08activa, cara08idgrupo, 
			cara08titulo, cara08cuerpo, cara08tipopreg, cara08usosiniciales, cara08usostotales, 
			cara08nivelpregunta, cara08retroalimenta';
			$sValores2308 = '' . $DATA['cara08idbloque'] . ', ' . $DATA['cara08consec'] . ', ' . $DATA['cara08id'] . ', "' . $DATA['cara08activa'] . '", ' . $DATA['cara08idgrupo'] . ', 
			"' . $DATA['cara08titulo'] . '", "' . $cara08cuerpo . '", ' . $DATA['cara08tipopreg'] . ', ' . $DATA['cara08usosiniciales'] . ', ' . $DATA['cara08usostotales'] . ', 
			' . $DATA['cara08nivelpregunta'] . ', "' . $cara08retroalimenta . '"';
			if ($APP->utf8 == 1) {
				$sSQL = 'INSERT INTO cara08pregunta (' . $sCampos2308 . ') VALUES (' . cadena_codificar($sValores2308) . ');';
				$sdetalle = $sCampos2308 . '[' . cadena_codificar($sValores2308) . ']';
			} else {
			...
			if ($bPasa) {
				if ($APP->utf8 == 1) {
					...
					$sSQL = 'UPDATE cara08pregunta SET ' . cadena_codificar($sdatos) . ' WHERE ' . $sWhere . ';';
				} else {
		...
		if ($bPasa) {
			if ($bDebug) {
				$sDebug = $sDebug . fecha_microtiempo() . ' Guardar 2308 ' . $sSQL . '<br>';
			}
			$result = $objDB->ejecutasql($sSQL);
			if ($result == false) {
...
function f2308_db_Eliminar($cara08id, $objDB, $bDebug = false)
{
	...
	if ($sError == '') {
		...
		if ($result == false) {
			...
			if ($bAudita[4]) {
				seg_auditar($iCodModulo, $_SESSION['unad_id_tercero'], 4, $cara08id, $sWhere, $objDB);
			}
		}
	}
...
function f2308_TablaDetalleBusquedas($aParametros, $objDB)
{
	...
	*/
	...
	$sSQL = 'SELECT TB.cara08idbloque, TB.cara08consec, TB.cara08id, TB.cara08activa, T5.cara06nombre, TB.cara08titulo, TB.cara08cuerpo, TB.cara08tipopreg, TB.cara08usosiniciales, TB.cara08usostotales, TB.cara08nivelpregunta, TB.cara08idgrupo, TB.cara08retroalimenta 
	...
	if ($tabladetalle == false) {
		...
		$registros = $objDB->nf($tabladetalle);
		if ($registros == 0) {
			//return array(cadena_codificar($sErrConsulta.'<input id="paginaf2308" name="paginaf2308" type="hidden" value="'.$pagina.'"/><input id="lppf2308" name="lppf2308" type="hidden" value="'.$lineastabla.'"/>'), $sDebug);
		}
		if ((($registros - 1) / $lineastabla) < ($pagina - 1)) {
			$pagina = (int)(($registros - 1) / $lineastabla) + 1;
		}
