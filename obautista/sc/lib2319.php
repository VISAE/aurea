...
<input id="titulos_2319" name="titulos_2319" type="hidden" value="' . $sTitulos . '"/>';
	...
	if ($tabladetalle == false) {
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' Consulta 2319: ' . $sSQL . '<br>';
		}
		$registros = 0;
...
<input id="titulos_2320" name="titulos_2320" type="hidden" value="' . $sTitulos . '"/>';
	...
	if ($tabladetalle == false) {
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' Consulta 2320: ' . $sSQL . '<br>';
		}
		$registros = 0;
		$sErrConsulta = $sErrConsulta . '..<input id="err" name="err" type="hidden" value="' . $sSQL . ' ' . $objDB->serror . '"/>';
	} else {
...
' . html_lpp('lppf2320', $lineastabla, 'paginarf2320()') . '
	...
	while ($filadet = $objDB->sf($tabladetalle)) {
		...
		$bConPreg = false;
		if ($filadet['cara01fichafam'] != -1) {
			$et_fichas = $ETI['cara11fichafamilia'];
		}
		if ($filadet['cara01fichadigital'] != -1) {
			$et_fichas = $et_fichas . ' - ' . $ETI['cara11ficha1'] . ' <b>' . $aBloque[1] . '</b>';
			$bConPreg = true;
		}
		if ($filadet['cara01fichalectura'] != -1) {
			$et_fichas = $et_fichas . ' - ' . $ETI['cara11ficha2'] . ' <b>' . $aBloque[2] . '</b>';
			$bConPreg = true;
		}
		if ($filadet['cara01ficharazona'] != -1) {
			$et_fichas = $et_fichas . ' - ' . $ETI['cara11ficha3'] . ' <b>' . $aBloque[3] . '</b>';
			$bConPreg = true;
		}
		if ($filadet['cara01fichaingles'] != -1) {
			$et_fichas = $et_fichas . ' - ' . $ETI['cara11ficha4'] . ' <b>' . $aBloque[4] . '</b>';
			$bConPreg = true;
		}
		if ($filadet['cara01fichaquimica'] != -1) {
			$et_fichas = $et_fichas . ' - ' . $ETI['cara11ficha5'] . ' <b>' . $aBloque[5] . '</b>';
			$bConPreg = true;
		}
		if ($filadet['cara01fichabiolog'] != -1) {
			$et_fichas = $et_fichas . ' - ' . $ETI['cara11ficha6'] . ' <b>' . $aBloque[6] . '</b>';
			$bConPreg = true;
		}
		if ($filadet['cara01fichafisica'] != -1) {
			$et_fichas = $et_fichas . ' - ' . $ETI['cara11ficha7'] . ' <b>' . $aBloque[7] . '</b>';
			$bConPreg = true;
		}
