...
if (!file_exists($mensajes_2323)){$mensajes_2323=$APP->rutacomun.'lg/lg_2323_es.php';}
...
if (isset($_REQUEST['deb_doc'])!=0){
	...
	if ($bDevuelve){
		...
		if ($objDB->nf($tabla)>0){
			...
			if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Documento NO ENCONTRADO '.$sSQL.'.<br>';}
...
if (!file_exists($mensajes_2343)){$mensajes_2343=$APP->rutacomun.'lg/lg_2343_es.php';}
...
require 'lib2323.php';
require $APP->rutacomun.'lib2451.php';
// -- 3041 saiu41docente
...
$xajax->register(XAJAX_FUNCTION,'f2323_Combocara23idperiodo');
$xajax->register(XAJAX_FUNCTION,'sesion_abandona_V2');
...
if (isset($_REQUEST['paginaf304111'])==0){$_REQUEST['paginaf304111']=1;}
if (isset($_REQUEST['lppf304111'])==0){$_REQUEST['lppf304111']=20;}
if (isset($_REQUEST['paginaf2323res'])==0){$_REQUEST['paginaf2323res']=1;}
if (isset($_REQUEST['lppf2323res'])==0){$_REQUEST['lppf2323res']=20;}
if (isset($_REQUEST['boculta2323'])==0){$_REQUEST['boculta2323']=0;}
if (isset($_REQUEST['paginaf3042'])==0){$_REQUEST['paginaf3042']=1;}
if (isset($_REQUEST['lppf3042'])==0){$_REQUEST['lppf3042']=20;}
if (isset($_REQUEST['boculta3042'])==0){$_REQUEST['boculta3042']=0;}
if (isset($_REQUEST['paginaf2343'])==0){$_REQUEST['paginaf2343']=1;}
if (isset($_REQUEST['lppf2343'])==0){$_REQUEST['lppf2343']=20;}
if (isset($_REQUEST['boculta2343'])==0){$_REQUEST['boculta2343']=0;}
...
if (isset($_REQUEST['cara23idperiodo'])==0){$_REQUEST['cara23idperiodo']='';}
if ((int)$_REQUEST['paso']>0){
	//Alertas
	if (isset($_REQUEST['cara43consec'])==0){$_REQUEST['cara43consec']='';}
	if (isset($_REQUEST['cara43id'])==0){$_REQUEST['cara43id']='';}
	if (isset($_REQUEST['cara43idalerta'])==0){$_REQUEST['cara43idalerta']='';}
	if (isset($_REQUEST['cara43fecharegistro'])==0){$_REQUEST['cara43fecharegistro']='';}//{fecha_hoy();}
	if (isset($_REQUEST['cara43idregistro'])==0){$_REQUEST['cara43idregistro']=0;}//{$_SESSION['unad_id_tercero'];}
	if (isset($_REQUEST['cara43idregistro_td'])==0){$_REQUEST['cara43idregistro_td']=$APP->tipo_doc;}
	if (isset($_REQUEST['cara43idregistro_doc'])==0){$_REQUEST['cara43idregistro_doc']='';}
	if (isset($_REQUEST['cara43fechacierre'])==0){$_REQUEST['cara43fechacierre']='';}
	if (isset($_REQUEST['cara43idcierre'])==0){$_REQUEST['cara43idcierre']='';}
	}
...
if (($_REQUEST['paso']==1)||($_REQUEST['paso']==3)){
	...
	if ($_REQUEST['paso']==1){
		$sSQLcondi='cara23idtercero='.$_REQUEST['cara23idtercero'].' AND cara23consec='.$_REQUEST['cara23consec'].'';
		}else{
		$sSQLcondi='cara23id='.$_REQUEST['cara23id'].'';
		}
	$sSQL='SELECT * FROM cara23acompanamento WHERE '.$sSQLcondi;
	if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Datos a cargar '.$sSQL.'<br>';}
	$tabla=$objDB->ejecutasql($sSQL);
	if ($objDB->nf($tabla)>0){
		$fila=$objDB->sf($tabla);
		...
		$_REQUEST['cara23idperiodo']=$fila['cara23idperiodo'];
...
if ($bTraerDatosBase){
	...
	if ($sErrorE==''){
		...
		if ((int)$_REQUEST['cara23idperiodo']!=0){
			$_REQUEST['bperacamat']=$_REQUEST['cara23idperiodo'];
			}else{
...
if ($bTraeInfoCursos){
	...
	list($iTotalCursos, $iCursosSinIngreso, $iCursosPerdidos, $iNumCreditos, $iNumAprobados, $sDebugC)=f2400_AvanceEstudiantePeriodo($_REQUEST['cara23idtercero'], $_REQUEST['cara23idperiodo'], $iFechaConsulta, $objDB, $idContTercero, $bDebug);
	$sDebug=$sDebug.$sDebugC;
...
if (true){
	...
	if ($_REQUEST['paso']!=0){
		...
		//Octubre 6 de 2020 - se cambia el periodo de matricula por el periodo de acompañamiento. -- bperacamat -$_REQUEST['cara01idperiodoacompana']
		$sSQL='SELECT TB.core04idcurso 
		FROM core04matricula_'.$iContenedor.' AS TB, unad40curso AS T4 
		WHERE TB.core04peraca IN ('.$_REQUEST['cara23idperiodo'].', '.$_REQUEST['cara01idperiodoacompana'].') AND TB.core04tercero='.$_REQUEST['cara23idtercero'].' 
		AND TB.core04idcurso=T4.unad40id AND T4.unad40catedraunadista<>0';
		$tabla=$objDB->ejecutasql($sSQL);
		if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Cursos de Catedra: '.$sSQL.'<br>';}
		while($fila=$objDB->sf($tabla)){
...
if ((int)$_REQUEST['paso']==0){
	...
	//$html_cara23consec=f2323_HTMLComboV2_cara23consec($objDB, $objCombos, $_REQUEST['cara23consec']);
	...
	$objCombos->nuevo('cara23idperiodo', $_REQUEST['cara23idperiodo'], true, '{'.$ETI['msg_seleccione'].'}');
	$html_cara23idperiodo=f2323_HTMLComboV2_cara23idperiodo($objDB, $objCombos, $_REQUEST['cara23idperiodo'], $_REQUEST['cara23idtercero']);
	}else{
	...
	list($cara23idperiodo_nombre, $sErrorDet)=tabla_campoxid('exte02per_aca','exte02nombre','exte02id',$_REQUEST['cara23idperiodo'],'{'.$ETI['msg_sindato'].'}', $objDB);
	$html_cara23idperiodo=html_oculto('cara23idperiodo', $_REQUEST['cara23idperiodo'], $cara23idperiodo_nombre);
	...
	list($cara43idregistro_rs, $_REQUEST['cara43idregistro'], $_REQUEST['cara43idregistro_td'], $_REQUEST['cara43idregistro_doc'])=html_tercero($_REQUEST['cara43idregistro_td'], $_REQUEST['cara43idregistro_doc'], $_REQUEST['cara43idregistro'], 0, $objDB);
...
$sTabla2323Res='<input id="paginaf2323res" name="paginaf2323res" type="hidden" value="'.$_REQUEST['paginaf2323res'].'" />
...
if ($_REQUEST['cara23idtercero']!=0){
	...
	$aParametros2323[103]=$_REQUEST['cara23idtercero'];
	list($sTabla2323Res, $sDebugTabla)=f2323_TablaDetalleV2Res($aParametros2323, $objDB, $bDebug);
	$sDebug=$sDebug.$sDebugTabla;
	//Historico
	$aParametros3042[91]=$_REQUEST['paginaf3042'];
	$aParametros3042[92]=$_REQUEST['lppf3042'];
	$aParametros3042[93]=-1;//$_REQUEST['saiu41id']
	$aParametros3042[100]=$idTercero;
	$aParametros3042[103]=$_REQUEST['cara23idtercero'];
...
if ($_REQUEST['paso']!=0){
	...
	//Alertas
	$aParametros2343[0]=$_REQUEST['cara23idtercero'];
...
function carga_combo_cara23idencuesta(valor=0){
	...
	params[1]=valor;
	document.getElementById('div_cara23idencuesta').innerHTML='<b>Procesando datos, por favor espere...</b><input id="cara23idencuesta" name="cara23idencuesta" type="hidden" value="" />';
...
function carga_combo_cara23idperiodo(){
	var params=new Array();
	params[0]=window.document.frmedita.cara23idtercero.value;
	document.getElementById('div_cara23idperiodo').innerHTML='<b>Procesando datos, por favor espere...</b><input id="cara23idperiodo" name="cara23idperiodo" type="hidden" value="" />';
	xajax_f2323_Combocara23idperiodo(params);
	}
...
//Los acompañamientos compartidos
function paginarf2323Res(){
	var params=new Array();
	params[99]=window.document.frmedita.debug.value;
	...
	params[103]=window.document.frmedita.cara23idtercero.value;
	document.getElementById('div_f2323detalle').innerHTML='<div class="GrupoCamposAyuda"><div class="MarquesinaMedia">Procesando datos, por favor espere.</div></div><input id="paginaf2323res" name="paginaf2323res" type="hidden" value="'+params[101]+'" /><input id="lppf2323res" name="lppf2323res" type="hidden" value="'+params[102]+'" />';
...
<label class="Label130">
<?php
echo $ETI['cara23idperiodo'];
?>
</label>
<label>
<div id="div_cara23idperiodo">
<?php
echo $html_cara23idperiodo;
?>
</div>
</label>
<div class="salto1px"></div>
...
-->
<label class="Label30">
<input id="btexpande2343" name="btexpande2343" type="button" value="Expandir" class="btMiniExpandir" onclick="expandepanel(2343,'block',0);" title="<?php echo $ETI['bt_mostrar']; ?>" style="display:<?php if ($_REQUEST['boculta2343']==0){echo 'none'; }else{echo 'block';} ?>;"/>
</label>
<label class="Label30">
<input id="btrecoge2343" name="btrecoge2343" type="button" value="Recoger" class="btMiniRecoger" onclick="expandepanel(2343,'none',1);" title="<?php echo $ETI['bt_ocultar']; ?>" style="display:<?php if ($_REQUEST['boculta2343']==0){echo 'block'; }else{echo 'none';} ?>;"/>
</label>
</div>
<div class="salto1px"></div>
<div id="div_p2343" style="display:<?php if ($_REQUEST['boculta2343']==0){echo 'block'; }else{echo 'none';} ?>;">
<label class="Label130">
<?php
echo $ETI['cara43consec'];
?>
</label>
<label class="Label130"><div id="div_cara43consec">
<?php
...
<label class="Label60"><div id="div_cara43id">
<?php
...
<label class="Label30">
<input id="belimina2343" name="belimina2343" type="button" value="Eliminar" class="btMiniEliminar" onclick="eliminaf2343()" title="<?php echo $ETI['bt_mini_eliminar_2343']; ?>" style="display:<?php if ((int)$_REQUEST['cara43id']!=0){echo 'block';}else{echo 'none';} ?>;"/>
</label>
<?php
...
<label>
<input id="bnombre2343" name="bnombre2343" type="text" value="<?php echo $_REQUEST['bnombre2343']; ?>" onchange="paginarf2343()"/>
</label>
<label class="Label130">
<?php
echo $ETI['msg_Listar'];
