...
function f2310_db_Guardar($valores, $objDB, $bDebug=false){
	...
	$sSepara=', ';
	if ($cara10nivelpregunta==''){$sError=$ERR['cara10nivelpregunta'].$sSepara.$sError;}
	if ($cara10puntaje==''){$sError=$ERR['cara10puntaje'].$sSepara.$sError;}
	if ($cara10idrpta==''){$sError=$ERR['cara10idrpta'].$sSepara.$sError;}
	if ($cara10idpregunta==''){$sError=$ERR['cara10idpregunta'].$sSepara.$sError;}
	...
	if ($cara10idbloque==''){$sError=$ERR['cara10idbloque'].$sSepara.$sError;}
	if ($cara10idcara==''){$sError=$ERR['cara10idcara'].$sSepara.$sError;}
	if ($sError==''){
		if ((int)$cara10id==0){
			if ((int)$cara10consec==0){
				...
				if ($cara10consec==-1){$sError=$objDB->serror;}
				}else{
			...
			if ($sError==''){
				...
				if ($cara10id==-1){$sError=$objDB->serror;}
				$binserta=true;
				$iAccion=2;
				}
			}else{
...
'.$cara10idrpta.', '.$cara10puntaje.', '.$cara10nivelpregunta.'';
			if ($APP->utf8==1){
				$sSQL='INSERT INTO cara10pregprueba ('.$scampos.') VALUES ('.utf8_encode($svalores).');';
				}else{
			...
			if ($bpasa){
				if ($APP->utf8==1){
					$sSQL='UPDATE cara10pregprueba SET '.utf8_encode($sdatos).' WHERE '.$sWhere.';';
					}else{
...
function f2310_TablaDetalleV2($aParametros, $objDB, $bDebug=false){
	...
	$idBloque=$aParametros[100];
	$bEstudiante=false;
	if ($aParametros[103]==1){$bEstudiante=true;}
	$babierta=false;
	$sSQL='SELECT cara01completa FROM cara01encuesta WHERE cara01id='.$cara01id;
	$tabla=$objDB->ejecutasql($sSQL);
	if ($objDB->nf($tabla)>0){
		$fila=$objDB->sf($tabla);
		if ($fila['cara01completa']!='S'){$babierta=true;}
		}
	$sSQLadd='';
	$sSQLadd1='';
	$sLeyenda='';
...
<div class="salto1px"></div>
	...
	*/
	...
	$sSQL='SELECT TB.cara10idcara, TB.cara10consec, TB.cara10id, T5.cara08cuerpo, TB.cara10idrpta, TB.cara10puntaje, TB.cara10nivelpregunta, TB.cara10idbloque, TB.cara10idpregunta, T5.cara08idgrupo 
...
<input id="titulos_2310" name="titulos_2310" type="hidden" value="'.$sTitulos.'"/>';
	...
	if ($tabladetalle==false){
		if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Consulta 2310: '.$sSQL.'<br>';}
		$registros=0;
		...
		if ($registros==0){
			return array(utf8_encode($sErrConsulta.'<input id="paginaf2310" name="paginaf2310" type="hidden" value="'.$pagina.'"/><input id="lppf2310" name="lppf2310" type="hidden" value="'.$lineastabla.'"/>'), $sDebug);
			}
		...
		if ($filadet['cara08idgrupo']!=$idGrupo){
			...
			if ($filadet['cara08idgrupo']!=0){
				...
				if ($fila6=$objDB->nf($tabla6)>0){
					...
					$res=$res.'<tr>
<td colspan="'.$iColumnas.'">'.$fila6['cara06cuerpo'].'</td>
</tr>';
					}
				}
			}
		...
		if (!$bEstudiante){
			...
			if ($filadet['cara10nivelpregunta']==1){$et_cara10nivelpregunta='Profundizaci&oacute;n';}
			...
			if ($babierta){
				...
				$sSQL='SELECT cara09id, cara09contenido FROM cara09pregrpta WHERE cara09idpregunta='.$filadet['cara10idpregunta'].' ORDER BY cara09consec';
				$tabla9=$objDB->ejecutasql($sSQL);
				...
				$et_cara10idrpta=$sPrefijo.cadena_notildes(f2310_TituloRespuesta($filadet['cara10idrpta'], $objDB)).$sSufijo;
				}
			}
		$res=$res.'<tr'.$sClass.'>
<td>'.$et_cara10consec.'</td>
<td colspan="4" align="justify">'.$et_cara10idpregunta.'</td>
</tr>
<tr'.$sClass.'>
<td></td>
<td align="right">'.$et_cara10idrpta.'</td>
'.$et_cara10puntaje.'
'.$et_cara10nivelpregunta.'
<td><div id="div_preg'.$filadet['cara10id'].'"></div></td>
</tr>';
		}
	$res=$res.'</table>';
...
function f2310_Traer($aParametros){
	...
	if ($paso==1){
		...
		$cara10idbloque=numeros_validar($aParametros[2]);
		$cara10consec=numeros_validar($aParametros[3]);
		if (($cara10idcara!='')&&($cara10idbloque!='')&&($cara10consec!='')){$besta=true;}
		}else{
		$cara10id=$aParametros[103];
		if ((int)$cara10id!=0){$besta=true;}
		}
	if ($besta){
		$besta=false;
		$objDB=new clsdbadmin($APP->dbhost, $APP->dbuser, $APP->dbpass, $APP->dbname);
...
function f2310_GuardarRespuesta($valores){
	...
	//if (isset($opts[99])!=0){if ($opts[99]==1){$bDebug=true;}}
	if(!is_array($valores)){$valores=json_decode(str_replace('\"','"',$valores),true);}
	if (isset($valores[0])==0){$valores[0]='';}
	//if ($datos[0]==''){$sError=$ERR[''];}
	if ($sError==''){
		$objDB=new clsdbadmin($APP->dbhost, $APP->dbuser, $APP->dbpass, $APP->dbname);
		if ($APP->dbpuerto!=''){$objDB->dbPuerto=$APP->dbpuerto;}
...
function f2310_TituloRespuesta($id, $objDB){
	$res=$id;
	$sSQL='SELECT cara09contenido FROM cara09pregrpta WHERE cara09id='.$id.'';
	$tabla9=$objDB->ejecutasql($sSQL);
	if ($objDB->nf($tabla9)>0){
		$fila9=$objDB->sf($tabla9);
		$res=$fila9['cara09contenido'];
		}
	return $res;
	}
...
<div class="salto1px"></div>
	...
	//if ((int)$aParametros[103]!=-1){$sSQLadd=$sSQLadd.' AND TB.campo='.$aParametros[103];}
	if ($aParametros[104]!=''){$sSQLadd1=$sSQLadd1.'TB.cara10idbloque='.$aParametros[104].' AND ';}
	/*
	if ($aParametros[104]!=''){
		$sBase=trim(strtoupper($aParametros[104]));
		$aNoms=explode(' ', $sBase);
