...
function f3049_db_Guardar($valores, $objDB, $bDebug=false){
	...
	if (!file_exists($mensajes_3049)){$mensajes_3049='lg/lg_3049_es.php';}
	...
	$sSepara=', ';
	if ($saiu49minuto==''){$sError=$ERR['saiu49minuto'].$sSepara.$sError;}
	if ($saiu49hora==''){$sError=$ERR['saiu49hora'].$sSepara.$sError;}
	if ($saiu49usuario==0){$sError=$ERR['saiu49usuario'].$sSepara.$sError;}
	if ($saiu49detalle==''){$sError=$ERR['saiu49detalle'].$sSepara.$sError;}
	if ($saiu49idestadofin==''){$sError=$ERR['saiu49idestadofin'].$sSepara.$sError;}
	if ($saiu49idestadorigen==''){$sError=$ERR['saiu49idestadorigen'].$sSepara.$sError;}
	if ($saiu49idresponsable==0){$sError=$ERR['saiu49idresponsable'].$sSepara.$sError;}
	...
	if ($saiu49idtramite==''){$sError=$ERR['saiu49idtramite'].$sSepara.$sError;}
	if ($sError==''){
	...
	if ($sError==''){
		if ((int)$saiu49id==0){
			if ((int)$saiu49consec==0){
				...
				if ($saiu49consec==-1){$sError=$objDB->serror;}
				}else{
			...
			if ($sError==''){
				...
				if ($saiu49id==-1){$sError=$objDB->serror;}
				$bInserta=true;
				$iAccion=2;
				}
			}else{
	...
	if ($sError==''){
		...
		if ($bInserta){
			...
			'.$saiu49minuto.', '.$saiu49correterminos.', '.$saiu49tiempousado.', '.$saiu49tiempocalusado.'';
			if ($APP->utf8==1){
				$sSQL='INSERT INTO saiu49cambioesttra ('.$sCampos3049.') VALUES ('.utf8_encode($sValores3049).');';
				}else{
				$sSQL='INSERT INTO saiu49cambioesttra ('.$sCampos3049.') VALUES ('.$sValores3049.');';
				}
			if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Guardar 3049 '.$sSQL.'<br>';}
			$result=$objDB->ejecutasql($sSQL);
			if ($result==false){
			...
			if ($bpasa){
				if ($APP->utf8==1){
					$sSQL='UPDATE saiu49cambioesttra SET '.utf8_encode($sdatos).' WHERE '.$sWhere.';';
					}else{
...
function f3049_TablaDetalleV2($aParametros, $objDB, $bDebug=false){
	...
	*/
	...
	if ($tabladetalle==false){
		if ($bDebug){$sDebug=$sDebug.fecha_microtiempo().' Consulta 3049: '.$sSQL.'<br>';}
		$registros=0;
		...
		if ($registros==0){
			return array(utf8_encode($sErrConsulta.'<input id="paginaf3049" name="paginaf3049" type="hidden" value="'.$pagina.'"/><input id="lppf3049" name="lppf3049" type="hidden" value="'.$lineastabla.'"/>'), $sDebug);
			}
...
function f3049_Traer($aParametros){
	...
	if ($paso==1){
		$saiu49idtramite=numeros_validar($aParametros[1]);
		$saiu49consec=numeros_validar($aParametros[2]);
		if (($saiu49idtramite!='')&&($saiu49consec!='')){$besta=true;}
		}else{
		$saiu49id=$aParametros[103];
		if ((int)$saiu49id!=0){$besta=true;}
		}
	if ($besta){
		$besta=false;
		$objDB=new clsdbadmin($APP->dbhost, $APP->dbuser, $APP->dbpass, $APP->dbname);
