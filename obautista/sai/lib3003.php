...
function f3003_TablaDetalleV2($aParametros, $objDB, $bDebug = false)
{
	...
	$bunidad = numeros_validar($aParametros[104]);
	$borden = numeros_validar($aParametros[105]);
	$btiposol = numeros_validar($aParametros[106]);
	$bAbierta = true;
	/*
	...
	*/
	$sLeyenda = '';
	$sBotones = '<input id="paginaf3003" name="paginaf3003" type="hidden" value="' . $pagina . '"/>';
	$sBotones = $sBotones . '<input id="lppf3003" name="lppf3003" type="hidden" value="' . $lineastabla . '"/>';
	if ($sLeyenda != '') {
	...
	*/
	...
	if ($aParametros[104] != '') {
		$sSQLadd1 = $sSQLadd1 . 'TB.saiu03tiposol=' . $aParametros[104] . ' AND ';
	}
	if ($aParametros[105] != '') {
		$sSQLadd1 = $sSQLadd1 . 'TB.saiu03idunidadresp1=' . $aParametros[105] . ' AND ';
	}
	if ($aParametros[107] != '') {
		$sSQLadd1 = $sSQLadd1 . 'TB.saiu03tiposol=' . $aParametros[107] . ' AND ';
	}
	if ($aParametros[103] != '') {
...
<input id="titulos_3003" name="titulos_3003" type="hidden" value="' . $sTitulos . '"/>';
	...
	if ($bDebug) {
		$sDebug = $sDebug . fecha_microtiempo() . ' Consulta 3003: ' . $sSQL . '<br>';
	}
	if ($tabladetalle == false) {
		...
		if ($registros == 0) {
			//return array(cadena_codificar($sErrConsulta.'<input id="paginaf3003" name="paginaf3003" type="hidden" value="'.$pagina.'"/><input id="lppf3003" name="lppf3003" type="hidden" value="'.$lineastabla.'"/>'), $sDebug);
		}
...
' . html_lpp('lppf3003', $lineastabla, 'paginarf3003()') . '
	...
	while ($filadet = $objDB->sf($tabladetalle)) {
		...
		if ($filadet['saiu03activo'] == 'S') {
			$et_saiu03activo = $sPrefijo . $ETI['si'] . $sSufijo;
		}
		/*
...
function f3003_db_CargarPadre($DATA, $objDB, $bDebug = false)
{
	...
	if ($objDB->nf($tabla) > 0) {
		...
		$DATA['saiu03ordenfav'] = $fila['saiu03ordenfav'];
		$DATA['saiu03reddeservicio'] = $fila['saiu03reddeservicio'];
		$bcargo = true;
		$DATA['paso'] = 2;
		$DATA['boculta3003'] = 0;
		$bLimpiaHijos = true;
...
function f3003_db_GuardarV2($DATA, $objDB, $bDebug = false, $idTercero = 0, $iCodModulo = 3003)
{
	$bAudita[2] = true;
	$bAudita[3] = true;
	require './app.php';
	...
	*/
	if (isset($DATA['saiu03ordenfav']) == 0) {
		$DATA['saiu03ordenfav'] = 0;
	}
	if (isset($DATA['saiu03reddeservicio']) == 0) {
		$DATA['saiu03reddeservicio'] = 0;
	}
	...
	$DATA['saiu03ordenfav'] = numeros_validar($DATA['saiu03ordenfav']);
	$DATA['saiu03reddeservicio'] = numeros_validar($DATA['saiu03reddeservicio']);
	// -- Se inicializan las variables que puedan pasar vacias {Especialmente números}.
	//if ($DATA['saiu03numetapas']==''){$DATA['saiu03numetapas']=0;}
	if ($DATA['saiu03idunidadresp1'] == '') {
		$DATA['saiu03idunidadresp1'] = 0;
	}
	if ($DATA['saiu03idequiporesp1'] == '') {
		$DATA['saiu03idequiporesp1'] = 0;
	}
	if ($DATA['saiu03tiemprespdias1'] == '') {
		$DATA['saiu03tiemprespdias1'] = 0;
	}
	if ($DATA['saiu03tiempresphoras1'] == '') {
		$DATA['saiu03tiempresphoras1'] = 0;
	}
	if ($DATA['saiu03idunidadresp2'] == '') {
		$DATA['saiu03idunidadresp2'] = 0;
	}
	if ($DATA['saiu03idequiporesp2'] == '') {
		$DATA['saiu03idequiporesp2'] = 0;
	}
	if ($DATA['saiu03tiemprespdias2'] == '') {
		$DATA['saiu03tiemprespdias2'] = 0;
	}
	if ($DATA['saiu03tiempresphoras2'] == '') {
		$DATA['saiu03tiempresphoras2'] = 0;
	}
	if ($DATA['saiu03idunidadresp3'] == '') {
		$DATA['saiu03idunidadresp3'] = 0;
	}
	if ($DATA['saiu03idequiporesp3'] == '') {
		$DATA['saiu03idequiporesp3'] = 0;
	}
	if ($DATA['saiu03tiemprespdias3'] == '') {
		$DATA['saiu03tiemprespdias3'] = 0;
	}
	if ($DATA['saiu03tiempresphoras3'] == '') {
		$DATA['saiu03tiempresphoras3'] = 0;
	}
	if ($DATA['saiu03moduloasociado'] == '') {
	...
	if ($DATA['saiu03firmacertificada'] == '') {
		$DATA['saiu03firmacertificada'] = 0;
	}
	...
	if ($DATA['saiu03ordenfav'] == '') {
		$DATA['saiu03ordenfav'] = 0;
	}
	if ($DATA['saiu03reddeservicio'] == '') {
		$DATA['saiu03reddeservicio'] = 0;
	}
	...
	if (true) {
		...
		*/
		if ($DATA['saiu03anexoslibres'] == '') {
			$sError = $ERR['saiu03anexoslibres'] . $sSepara . $sError;
		}
		if ($DATA['saiu03anonimos'] == '') {
			$sError = $ERR['saiu03anonimos'] . $sSepara . $sError;
		}
		if ($DATA['saiu03consupervisor'] == '') {
			$sError = $ERR['saiu03consupervisor'] . $sSepara . $sError;
		}
		if ($DATA['saiu03otrosusaurios'] == '') {
			$sError = $ERR['saiu03otrosusaurios'] . $sSepara . $sError;
		}
		/*
		...
		*/
		...
		if ($DATA['saiu03titulo'] == '') {
			$sError = $ERR['saiu03titulo'] . $sSepara . $sError;
		}
		if ($DATA['saiu03activo'] == '') {
			$sError = $ERR['saiu03activo'] . $sSepara . $sError;
		}
		//Fin de las valiaciones NO LLAVE.
	}
	//Valiaciones de campos obligatorios en todo guardar.
	if ($sError == '') {
		$aListaCampos = array('', 'saiu03nometapa3', 'saiu03nometapa2', 'saiu03nometapa1', 'saiu03titulo');
		$aLargoCampos = array(0, 100, 100, 100, 250);
		for ($k = 1; $k <= 4; $k++) {
	...
	if ($DATA['saiu03tiposol'] == '') {
		$sError = $ERR['saiu03tiposol'];
	}
	// -- Se verifican los valores de campos de otras tablas.
	if ($sError == '') {
		if ($DATA['saiu03idliderrespon3_doc'] != '') {
			$sError = tabla_terceros_existe($DATA['saiu03idliderrespon3_td'], $DATA['saiu03idliderrespon3_doc'], $objDB, 'El tercero Liderrespon3 ');
		}
		if ($sError == '') {
	...
	if ($sError == '') {
		if ($DATA['saiu03idliderrespon2_doc'] != '') {
			$sError = tabla_terceros_existe($DATA['saiu03idliderrespon2_td'], $DATA['saiu03idliderrespon2_doc'], $objDB, 'El tercero Liderrespon2 ');
		}
		if ($sError == '') {
	...
	if ($sError == '') {
		if ($DATA['saiu03idliderrespon1_doc'] != '') {
			$sError = tabla_terceros_existe($DATA['saiu03idliderrespon1_td'], $DATA['saiu03idliderrespon1_doc'], $objDB, 'El tercero Liderrespon1 ');
		}
		if ($sError == '') {
...
saiu03firmacertificada, saiu03ordenllamada, saiu03ordenchat, saiu03ordencorreo, saiu03ordenpresencial, 
			saiu03ordensoporte, saiu03ordenpqrs, saiu03ordentramites, saiu03ordencorresp, saiu03ordenfav, 
			saiu03reddeservicio';
...
' . $DATA['saiu03firmacertificada'] . ', ' . $DATA['saiu03ordenllamada'] . ', ' . $DATA['saiu03ordenchat'] . ', ' . $DATA['saiu03ordencorreo'] . ', ' . $DATA['saiu03ordenpresencial'] . ', 
			' . $DATA['saiu03ordensoporte'] . ', ' . $DATA['saiu03ordenpqrs'] . ', ' . $DATA['saiu03ordentramites'] . ', ' . $DATA['saiu03ordencorresp'] . ', ' . $DATA['saiu03ordenfav'] . ', 
			' . $DATA['saiu03reddeservicio'] . '';
			if ($APP->utf8 == 1) {
				$sSQL = 'INSERT INTO saiu03temasol (' . $sCampos3003 . ') VALUES (' . cadena_codificar($sValores3003) . ');';
				$sdetalle = $sCampos3003 . '[' . cadena_codificar($sValores3003) . ']';
			} else {
			...
			$scampo[46] = 'saiu03ordenfav';
			$scampo[47] = 'saiu03reddeservicio';
			$scampo[48] = 'saiu03tiposol';
			...
			$sdato[46] = $DATA['saiu03ordenfav'];
			$sdato[47] = $DATA['saiu03reddeservicio'];
			$sdato[48] = $DATA['saiu03tiposol'];
			$iNumCamposMod = 48;
			...
			if ($bPasa) {
				if ($APP->utf8 == 1) {
					...
					$sSQL = 'UPDATE saiu03temasol SET ' . cadena_codificar($sdatos) . ' WHERE ' . $sWhere . ';';
				} else {
		...
		if ($bPasa) {
			if ($bDebug) {
				$sDebug = $sDebug . fecha_microtiempo() . ' Guardar 3003 ' . $sSQL . '<br>';
			}
			$result = $objDB->ejecutasql($sSQL);
			if ($result == false) {
				...
				if ($bAudita[$idAccion]) {
					seg_auditar($iCodModulo, $_SESSION['unad_id_tercero'], $idAccion, $DATA['saiu03id'], $sdetalle, $objDB);
				}
				$DATA['paso'] = 2;
			}
		} else {
...
function f3003_db_Eliminar($saiu03id, $objDB, $bDebug = false)
{
	...
	if ($sError == '') {
		...
		if ($result == false) {
			...
			if ($bAudita[4]) {
				seg_auditar($iCodModulo, $_SESSION['unad_id_tercero'], 4, $saiu03id, $sWhere, $objDB);
			}
		}
	}
...
function f3003_saiu03idliderrespon($aParametros)
{
	...
	if ($aParametros[101] != '') {
		$iEtapa = $aParametros[101];
	}
	if ($aParametros[102] != '') {
		$saiu03idequiporesp = $aParametros[102];
	}
	$objDB->xajax();
	$objResponse = new xajaxResponse();
	if ($iEtapa > 0) {
