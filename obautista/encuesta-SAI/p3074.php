...
class clsPDF extends FPDF
{
	...
	var $sRefRpt = '';
	var $sNumCopia = '';
	var $sTituloReporte = 'Encuestas de satisfacción';
	...
	function ArmarReporte3074($PARAMS, $objDB)
	{
		$this->SetTextColor(0, 0, 0);
		...
		$sSQL = 'SELECT * FROM saiu74encuesta WHERE saiu74id=' . $PARAMS['id3074'] . '';
		if ($this->bDebug) {
		...
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			$ssaiu74codmodulo = $fila['saiu74codmodulo'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['saiu74codmodulo'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$ssaiu74codmodulo = $filat[''];
				if ($this->bCodificar) {
					$ssaiu74codmodulo = cadena_codificar($filat['']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Codmodulo: ' . cadena_codificar($ssaiu74codmodulo));
			$this->Ln();
			$ssaiu74idreg = $fila['saiu74idreg'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['saiu74idreg'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$ssaiu74idreg = $filat[''];
				if ($this->bCodificar) {
					$ssaiu74idreg = cadena_codificar($filat['']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Reg: ' . cadena_codificar($ssaiu74idreg));
			$this->Ln();
			$ssaiu74agno = $fila['saiu74agno'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['saiu74agno'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$ssaiu74agno = $filat[''];
				if ($this->bCodificar) {
					$ssaiu74agno = cadena_codificar($filat['']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Agno: ' . cadena_codificar($ssaiu74agno));
			$this->Ln();
			$ssaiu74acepta = $fila['saiu74acepta'];
			$sSQL = 'SELECT  FROM  WHERE =' . $fila['saiu74acepta'];
			$tablat = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tablat) > 0) {
				$filat = $objDB->sf($tablat);
				$ssaiu74acepta = $filat[''];
				if ($this->bCodificar) {
					$ssaiu74acepta = cadena_codificar($filat['']);
				}
			}
			$this->Cell($this->iAnchoLibre, 5, 'Acepta: ' . cadena_codificar($ssaiu74acepta));
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Fecharespuesta: ' . $fila['saiu74fecharespuesta']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Preg1: ' . $fila['saiu74preg1']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Preg2: ' . $fila['saiu74preg2']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Preg3: ' . $fila['saiu74preg3']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Preg4: ' . $fila['saiu74preg4']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Preg5: ' . $fila['saiu74preg5']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Preg6: ' . $fila['saiu74preg6']);
			$this->Ln();
			$this->Cell($this->iAnchoLibre, 5, 'Comentario: ' . $fila['saiu74comentario']);
			$this->Ln();
		}
		// Fin de ArmarReporte3074
	}
	// Fin de clsPDF
}
function pdfReporteV2($iReporte, $PARAMS, $iFormato, $sNumCopiaReporte, $bCodificarUTF8, $objDB, $bDebug = false)
{
	...
	if ($sError == '') {
		...
		$objpdf->sTituloReporte = 'Encuestas de satisfacción';
		$sDetalle = '';
		/*
		if ($PARAMS['v7'] != '') {
		...
		*/
		...
		$objpdf->AddPage();
		$objpdf->ArmarReporte3074($PARAMS, $objDB);
		//$objpdf->AddPage();
...
if ($sError == '') {
	...
	$sTituloRpt = 'Encuestas de satisfacción';
