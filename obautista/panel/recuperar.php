...
if (file_exists('./opts.php')) {
	require './opts.php';
	if (isset($OPT->cerrado) != 0) {
		if ($OPT->cerrado == 1) {
			$bCerrado = true;
		}
	}
	if (isset($OPT->https) != 0) {
		$iHTTPS = $OPT->https;
	}
}
...
if (isset($_REQUEST['txtuser']) == 0) {
	$_REQUEST['txtuser'] = '';
}
if (isset($_REQUEST['btipodoc']) == 0) {
	$_REQUEST['btipodoc'] = '';
}
...
$bCerrarLaCasa = false;
if ($_REQUEST['txtuser'] != '') {
	$sUsuario = cadena_letrasynumeros($_REQUEST['txtuser'], '_-.@');
...
if ($idEntidad == 1) {
	$sCorreoSoporte = 'info@unad.us';
}
//No tengo usuario... conseguir el usuario...
//Revisar el usuario
$sMensajeCampus = $ETI['msg_mensajecampus_a'] . ' ' . $sCorreoSoporte . $ETI['msg_mensajecampus_b'];
$bConUsuario = false;
...
if ($_REQUEST['paso'] == 19) {
	...
	if ($sError == '') {
		$sSQL = 'SELECT unad11id, unad11tipodoc, unad11doc, unad11usuario FROM unad11terceros WHERE unad11doc="' . $_REQUEST['txtdoc'] . '"';
		$tabla = $objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla) == 0) {
			//intentamos importarlo en primera instancia.
			unad11_importar_V2($_REQUEST['txtdoc'], '', $objDB);
			$sSQL = 'SELECT unad11id, unad11tipodoc, unad11doc, unad11usuario FROM unad11terceros WHERE unad11usuario="' . $_REQUEST['txtdoc'] . '"';
			$tabla = $objDB->ejecutasql($sSQL);
			if ($objDB->nf($tabla) == 0) {
...
if ($_REQUEST['paso'] == 21) {
	...
	if ($sError == '') {
		...
		if ($sDocFinal == $_REQUEST['txtuser']) {
			$sSQL = 'SELECT unad11id FROM unad11terceros WHERE unad11doc="' . $_REQUEST['txtuser'] . '"';
		} else {
			$sSQL = 'SELECT unad11id FROM unad11terceros WHERE unad11usuario="' . $_REQUEST['txtuser'] . '"';
		}
		$tabla = $objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla) == 0) {
			//intentamos importarlo en primera instancia.
...
if ($_REQUEST['paso'] == 22) {
	$_REQUEST['paso'] = 2;
	list($aure01codigo, $sError, $sDebugL) = AUREA_IniciarLogin($_REQUEST['id11'], $objDB, '', 1, $bDebug);
...
if ($_REQUEST['paso'] == 23) {
	$_REQUEST['paso'] = 2;
	if (isset($_REQUEST['txtcodigo']) == 0) {
		$_REQUEST['txtcodigo'] = '';
	}
	$_REQUEST['txtcodigo'] = numeros_validar($_REQUEST['txtcodigo']);
	if ($_REQUEST['txtcodigo'] == '') {
		$sError = 'No ha ingresado el c&oacute;digo de acceso';
	}
	if ($sError == '') {
		$sTabla = 'aure01login' . date('Ym');
		$aure01fecha = fecha_DiaMod();
		...
		$tabla = $objDB->ejecutasql($sSQL);
		if ($bDebug) {
			$sDebug = $sDebug . fecha_microtiempo() . ' Consulta de codigo ' . $sSQL . '<br>';
		}
		if ($objDB->nf($tabla) > 0) {
			$fila = $objDB->sf($tabla);
			...
			if ($sError == '') {
				if ($fila['aure01ip'] == $aure01ip) {
					if ($fila['aure01punto'] == $aure01punto) {
						$sUsuario = '';
						$sSQL = 'SELECT unad11tipodoc, unad11doc, unad11usuario FROM unad11terceros WHERE unad11id=' . $_REQUEST['id11'] . '';
						$tabla = $objDB->ejecutasql($sSQL);
						if ($bDebug) {
							$sDebug = $sDebug . fecha_microtiempo() . ' Se saca el usuario y el id de moodle<br>';
						}
						if ($objDB->nf($tabla) > 0) {
							$fila = $objDB->sf($tabla);
							$_REQUEST['paso'] = 3;
...
if (isset($_GET['u']) != 0) {
	...
	if ($bDebug) {
		$sDebug = $sDebug . fecha_microtiempo() . ' ' . $sURL . ' <br>';
	}
	$aURL = explode('|', $sURL);
	$sTabla = 'aure01login' . date('Ym');
	...
	if ($objDB->nf($tabla) > 0) {
		...
		if ($fila['aure01codigo'] == $aURL[1]) {
			if ($bDebug) {
				$sDebug = $sDebug . fecha_microtiempo() . ' El c&oacute;digo coincide <br>';
			}
...
<?php
			...
			$sError = $ETI['msg_codigonoconcuerda'];
		}
	} else {
		$sError = $ETI['msg_codigonoencontrado'];
	}
	//Si hubo error dejar el rastro...
	if ($sError != '') {
...
if ($_REQUEST['paso'] == 24) {
	...
	if ($sBase != $_REQUEST['txtnuevaclave']) {
		$sError = $ETI['msg_clave_nopermitidos'];
	}
	if ($sError == '') {
		if ($_REQUEST['txtnuevaclave'] != $_REQUEST['txtnuevaclave2']) {
			$sError = $ETI['msg_clave_nocoincide'];
		}
	}
	if ($sError == '') {
		list($sError, $sDebugC) = AUREA_ClaveValidaV3($_REQUEST['txtnuevaclave'], $_REQUEST['id11'], $objDB, $sPermitidos);
	}
	if ($sError == '') {
		...
		if ($sError == '') {
			$sError = $ETI['msg_clave_actualizada'];
			$iTipoError = 1;
			$_REQUEST['paso'] = 4;
		}
	}
...
if ($bConUsuario) {
if ($_REQUEST['paso'] == 0) {
echo $ETI['msg_envio1'] . ' <b>' . formato_CorreoOculto($sCorreoNotifica) . '</b> ' . $ETI['msg_envio2'] . ' <br>' . $sCorreoSoporte . '.';
}
if ($_REQUEST['paso'] == 2) {
...
if ($_REQUEST['paso'] == 20) {
?>
<input id="txtdoc" name="txtdoc" type="text" value="<?php echo $_REQUEST['txtuser'] ?>" maxlength="20" autocomplete="off" placeholder="<?php echo $ETI['msg_documento']; ?>" autofocus />
<input id="txtuser" name="txtuser" type="hidden" value="<?php echo $_REQUEST['txtuser'] ?>" />
<?php
} else {
?>
<input id="txtuser" name="txtuser" type="text" value="<?php echo $_REQUEST['txtuser'] ?>" maxlength="20" autocomplete="off" placeholder="<?php echo $ETI['msg_usuario']; ?>" autofocus />
<?php
}
}
?>
...
$bConEnter = true;
$sAccion = 'revisauser();';
$sTituloBoton = $ETI['bt_consulta'];
if ($_REQUEST['paso'] == 20) {
$sAccion = 'revisadoc();';
}
if ($bConUsuario) {
$sAccion = 'enviarmail();';
$sTituloBoton = $ETI['bt_codigo'];
if (!$bPuedeRecuperar) {
$sAccion = 'cancelar()';
$sTituloBoton = $ETI['bt_volver'];
}
}
?>
<div class="salto1px"></div>
...
<label class="Label90">&nbsp;</label>
<label>
<input id="cmdIngresa3" name="cmdIngresa3" type="button" value="<?php echo $ETI['msg_continuar']; ?>" onClick="revisacodigo();" class="BotonAzul" />
</label>
<?php
}
if ($_REQUEST['paso'] == 3) {
...
<label class="Label90">&nbsp;</label>
<label>
<input id="cmdIngresa4" name="cmdIngresa4" type="button" value="<?php echo $ETI['msg_continuar']; ?>" onClick="cambiaclave();" class="BotonAzul" />
</label>
<?php
}
if ($_REQUEST['paso'] == 4) {
...
<label class="Label90">&nbsp;</label>
<label>
<input id="cmdIngresa5" name="cmdIngresa5" type="button" value="<?php echo $ETI['msg_volver']; ?>" onClick="volver();" class="BotonAzul" />
</label>
<?php
}
