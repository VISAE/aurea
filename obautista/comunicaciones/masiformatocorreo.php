...
*/
/** Archivo masiformatocorreo.php.
 * Modulo 1210 masi10formato.
...
if (!$bPeticionXAJAX) {
	if (noticias_pendientes($objDB)) {
		$objDB->CerrarConexion();
		header('Location:noticia.php?ret=masiformatocorreo.php');
		die();
	}
}
$seg_1707 = 0;
...
$xajax->register(XAJAX_FUNCTION, 'f1210_HtmlTabla');
$xajax->register(XAJAX_FUNCTION, 'f1210_ExisteDato');
$xajax->processRequest();
if ($bPeticionXAJAX) {
...
if (isset($_REQUEST['paginaf1210']) == 0) {
	$_REQUEST['paginaf1210'] = 1;
}
if (isset($_REQUEST['lppf1210']) == 0) {
	$_REQUEST['lppf1210'] = 20;
}
if (isset($_REQUEST['boculta1210']) == 0) {
	$_REQUEST['boculta1210'] = 0;
}
// -- Inicializar variables de datos.
if (isset($_REQUEST['masi10consec']) == 0) {
	$_REQUEST['masi10consec'] = '';
}
if (isset($_REQUEST['masi10consec_nuevo']) == 0) {
	$_REQUEST['masi10consec_nuevo'] = '';
}
if (isset($_REQUEST['masi10id']) == 0) {
	$_REQUEST['masi10id'] = '';
}
if (isset($_REQUEST['masi1oactivo']) == 0) {
	$_REQUEST['masi1oactivo'] = 0;
}
if (isset($_REQUEST['masi10titulo']) == 0) {
	$_REQUEST['masi10titulo'] = '';
}
if (isset($_REQUEST['masi10encabezado']) == 0) {
	$_REQUEST['masi10encabezado'] = '';
}
if (isset($_REQUEST['masi10divcuerpo']) == 0) {
	$_REQUEST['masi10divcuerpo'] = '';
}
if (isset($_REQUEST['masi10divcodigocorreo']) == 0) {
	$_REQUEST['masi10divcodigocorreo'] = '';
}
if (isset($_REQUEST['masi10divcodigoconfirma']) == 0) {
	$_REQUEST['masi10divcodigoconfirma'] = '';
}
if (isset($_REQUEST['masi10divcodigorecupera']) == 0) {
	$_REQUEST['masi10divcodigorecupera'] = '';
}
if (isset($_REQUEST['masi10divfirma']) == 0) {
	$_REQUEST['masi10divfirma'] = '';
}
if (isset($_REQUEST['masi10piedepagina']) == 0) {
	$_REQUEST['masi10piedepagina'] = '';
}
$_REQUEST['masi10consec'] = numeros_validar($_REQUEST['masi10consec']);
$_REQUEST['masi10id'] = numeros_validar($_REQUEST['masi10id']);
$_REQUEST['masi1oactivo'] = numeros_validar($_REQUEST['masi1oactivo']);
$_REQUEST['masi10titulo'] = cadena_Validar($_REQUEST['masi10titulo']);
$_REQUEST['masi10encabezado'] = cadena_Validar($_REQUEST['masi10encabezado'], true);
$_REQUEST['masi10divcuerpo'] = cadena_Validar($_REQUEST['masi10divcuerpo'], true);
$_REQUEST['masi10divcodigocorreo'] = cadena_Validar($_REQUEST['masi10divcodigocorreo'], true);
$_REQUEST['masi10divcodigoconfirma'] = cadena_Validar($_REQUEST['masi10divcodigoconfirma'], true);
$_REQUEST['masi10divcodigorecupera'] = cadena_Validar($_REQUEST['masi10divcodigorecupera'], true);
$_REQUEST['masi10divfirma'] = cadena_Validar($_REQUEST['masi10divfirma'], true);
$_REQUEST['masi10piedepagina'] = cadena_Validar($_REQUEST['masi10piedepagina'], true);
...
if (($_REQUEST['paso'] == 1) || ($_REQUEST['paso'] == 3)) {
	if ($_REQUEST['paso'] == 1) {
		$sSQLcondi = 'masi10consec=' . $_REQUEST['masi10consec'] . '';
	} else {
		$sSQLcondi = 'masi10id=' . $_REQUEST['masi10id'] . '';
	}
	...
	if ($objDB->nf($tabla) > 0) {
		$fila = $objDB->sf($tabla);
		$_REQUEST['masi10consec'] = $fila['masi10consec'];
		$_REQUEST['masi10id'] = $fila['masi10id'];
		$_REQUEST['masi1oactivo'] = $fila['masi1oactivo'];
		$_REQUEST['masi10titulo'] = $fila['masi10titulo'];
...
if (($_REQUEST['paso'] == 10) || ($_REQUEST['paso'] == 12)) {
	$bMueveScroll = true;
	list($_REQUEST, $sError, $iTipoError, $sDebugGuardar) = f1210_db_GuardarV2b($_REQUEST, $objDB, $bDebug, $idTercero);
...
if ($_REQUEST['paso'] == 93) {
	$_REQUEST['paso'] = 2;
	$_REQUEST['masi10consec_nuevo'] = numeros_validar($_REQUEST['masi10consec_nuevo']);
	if ($_REQUEST['masi10consec_nuevo'] == '') {
		$sError = $ERR['masi10consec'];
	}
	if ($sError == '') {
	...
	if ($sError == '') {
		...
		$sSQL = 'SELECT masi10id FROM masi10formato WHERE masi10consec=' . $_REQUEST['masi10consec_nuevo'] . '';
		$tabla = $objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla) > 0) {
			$sError = 'El consecutivo ' . $_REQUEST['masi10consec_nuevo'] . ' ya existe';
		}
	}
	if ($sError == '') {
		//Aplicar el cambio.
		$sSQL = 'UPDATE masi10formato SET masi10consec=' . $_REQUEST['masi10consec_nuevo'] . ' WHERE masi10id=' . $_REQUEST['masi10id'] . '';
		$tabla = $objDB->ejecutasql($sSQL);
		$sDetalle = 'Cambia el consecutivo de ' . $_REQUEST['masi10consec'] . ' a ' . $_REQUEST['masi10consec_nuevo'] . '';
		$_REQUEST['masi10consec'] = $_REQUEST['masi10consec_nuevo'];
		$_REQUEST['masi10consec_nuevo'] = '';
		seg_auditar($iCodModulo, $_SESSION['unad_id_tercero'], 8, $_REQUEST['masi10id'], $sDetalle, $objDB);
...
if ($_REQUEST['paso'] == 13) {
	$_REQUEST['paso'] = 2;
	if ($sError == '') {
		list($sError, $iTipoError, $sDebugElimina) = f1210_db_Eliminar($_REQUEST['masi10id'], $objDB, $bDebug);
...
$objCombos->nuevo('masi1oactivo', $_REQUEST['masi1oactivo'], true, $ETI['no'], 0);
$objCombos->addItem(1, $ETI['si']);
//$objCombos->addArreglo($amasi1oactivo, $imasi1oactivo);
$sSQL = '';
$html_masi1oactivo = $objCombos->html($sSQL, $objDB);
if ((int)$_REQUEST['paso'] == 0) {
} else {
}
...
$aParametros[0] = ''; //$_REQUEST['p1_1210'];
$aParametros[100] = $idTercero;
$aParametros[101] = $_REQUEST['paginaf1210'];
$aParametros[102] = $_REQUEST['lppf1210'];
$aParametros[103] = $_REQUEST['bnombre'];
list($sTabla1210, $sDebugTabla) = f1210_TablaDetalleV2($aParametros, $objDB, $bDebug);
...
?>
	...
	function imprimelista() {
		if (window.document.frmedita.seg_6.value == 1) {
			window.document.frmlista.consulta.value = window.document.frmedita.consulta_1210.value;
			window.document.frmlista.titulos.value = window.document.frmedita.titulos_1210.value;
			window.document.frmlista.nombrearchivo.value = 'Formatos de correo';
	...
	function imprimeexcel() {
		...
		if (sError == '') {
			asignarvariables();
			window.document.frmimpp.action = 'e1210_ss.php';
	...
	function imprimep() {
		if (window.document.frmedita.seg_5.value == 1) {
			asignarvariables();
			window.document.frmimpp.action = 'p1210.php';
			window.document.frmimpp.submit();
		} else {
	...
	function RevisaLlave() {
		let datos = new Array();
		datos[1] = window.document.frmedita.masi10consec.value;
		if ((datos[1] != '')) {
			xajax_f1210_ExisteDato(datos);
		}
	}

	function cargadato(llave1) {
		window.document.frmedita.masi10consec.value = String(llave1);
		window.document.frmedita.paso.value = 1;
		window.document.frmedita.submit();
	}

	function cargaridf1210(llave1) {
		window.document.frmedita.masi10id.value = String(llave1);
		window.document.frmedita.paso.value = 3;
		window.document.frmedita.submit();
	}

	function paginarf1210() {
		let params = new Array();
		params[99] = window.document.frmedita.debug.value;
		params[100] = <?php echo $idTercero; ?>;
		params[101] = window.document.frmedita.paginaf1210.value;
		params[102] = window.document.frmedita.lppf1210.value;
		params[103] = window.document.frmedita.bnombre.value;
		document.getElementById('div_f1210detalle').innerHTML = '<div class="GrupoCamposAyuda"><div class="MarquesinaMedia">Procesando datos, por favor espere.</div></div><input id="paginaf1210" name="paginaf1210" type="hidden" value="' + params[101] + '" /><input id="lppf1210" name="lppf1210" type="hidden" value="' + params[102] + '" />';
		xajax_f1210_HtmlTabla(params);
	}
	...
	function objinicial() {
		document.getElementById("masi10consec").focus();
	}
...
?>
	function previsual() {
		window.document.frmimpp.action = 'prev_formatocorreo.php';
...
?>
<form id="frmimpp" name="frmimpp" method="post" action="p1210.php" target="_blank" style="display:none">
<input id="r" name="r" type="hidden" value="1210" />
<input id="id1210" name="id1210" type="hidden" value="<?php echo $_REQUEST['masi10id']; ?>" />
...
<?php
echo $objForma->htmlExpande(1210, $_REQUEST['boculta1210'], $ETI['bt_mostrar'], $ETI['bt_ocultar']);
$sEstiloDiv = '';
if ($_REQUEST['boculta1210'] != 0) {
	$sEstiloDiv = ' style="display:none;"';
}
?>
</div>
<div id="div_p1210"<?php echo $sEstiloDiv; ?>>
<?php
}
...
if ($_REQUEST['paso'] != 2) {
?>
<input id="masi10consec" name="masi10consec" type="text" value="<?php echo $_REQUEST['masi10consec']; ?>" onchange="RevisaLlave()" class="cuatro" />
<?php
} else {
	echo html_oculto('masi10consec', $_REQUEST['masi10consec'], formato_numero($_REQUEST['masi10consec']));
}
?>
</label>
<?php
...
<label class="Label60"<?php echo $sOcultaId; ?>>
<?php
	echo html_oculto('masi10id', $_REQUEST['masi10id'], formato_numero($_REQUEST['masi10id']));
?>
</label>
<label class="Label90">
<?php
echo $ETI['masi1oactivo'];
?>
</label>
<label class="Label60">
<?php
echo $html_masi1oactivo;
?>
</label>
<?php
if ($_REQUEST['paso'] != 0) {
...
<label class="L">
<?php
echo $ETI['masi10titulo'];
?>

<input id="masi10titulo" name="masi10titulo" type="text" value="<?php echo $_REQUEST['masi10titulo']; ?>" maxlength="100" class="L" placeholder="<?php echo $ETI['ing_campo'] . $ETI['masi10titulo']; ?>" />
</label>
<label class="txtAreaL">
<?php
echo $ETI['masi10encabezado'];
?>
<textarea id="masi10encabezado" name="masi10encabezado" placeholder="<?php echo $ETI['ing_campo'] . $ETI['masi10encabezado']; ?>"><?php echo $_REQUEST['masi10encabezado']; ?></textarea>
</label>
<label class="txtAreaL">
<?php
echo $ETI['masi10divcuerpo'];
?>
<textarea id="masi10divcuerpo" name="masi10divcuerpo" placeholder="<?php echo $ETI['ing_campo'] . $ETI['masi10divcuerpo']; ?>"><?php echo $_REQUEST['masi10divcuerpo']; ?></textarea>
</label>
...
<label class="txtAreaL">
<?php
echo $ETI['masi10divcodigocorreo'];
?>
<textarea id="masi10divcodigocorreo" name="masi10divcodigocorreo" placeholder="<?php echo $ETI['ing_campo'] . $ETI['masi10divcodigocorreo']; ?>"><?php echo $_REQUEST['masi10divcodigocorreo']; ?></textarea>
</label>
...
<label class="txtAreaL">
<?php
echo $ETI['masi10divcodigoconfirma'];
?>
<textarea id="masi10divcodigoconfirma" name="masi10divcodigoconfirma" placeholder="<?php echo $ETI['ing_campo'] . $ETI['masi10divcodigoconfirma']; ?>"><?php echo $_REQUEST['masi10divcodigoconfirma']; ?></textarea>
</label>
<div class="salto1px"></div>
<div class="GrupoCamposAyuda">
Variables disponibles:<br>
...
<label class="txtAreaL">
<?php
echo $ETI['masi10divcodigorecupera'];
?>
<textarea id="masi10divcodigorecupera" name="masi10divcodigorecupera" placeholder="<?php echo $ETI['ing_campo'] . $ETI['masi10divcodigorecupera']; ?>"><?php echo $_REQUEST['masi10divcodigorecupera']; ?></textarea>
</label>
<div class="salto1px"></div>
<div class="GrupoCamposAyuda">
Variables disponibles:<br>
...
?>
<textarea id="masi10piedepagina" name="masi10piedepagina" placeholder="<?php echo $ETI['ing_campo'] . $ETI['masi10piedepagina']; ?>"><?php echo $_REQUEST['masi10piedepagina']; ?></textarea>
</label>
<?php
if (false) {
...
if ($bConExpande) {
	//Este es el cierre del div_p1210
?>
<div class="salto1px"></div>
</div>
<?php
...
<label>
<input id="bnombre" name="bnombre" type="text" value="<?php echo $_REQUEST['bnombre']; ?>" onchange="paginarf1210()" autocomplete="off" />
</label>
<div class="salto1px"></div>
</div>
<div class="salto1px"></div>
...
<label class="Label160">
<?php
echo $ETI['msg_masi10consec'];
?>
</label>
<label class="Label90">
<?php
echo '<b>' . $_REQUEST['masi10consec'] . '</b>';
?>
</label>
<div class="salto1px"></div>
<label class="Label160">
<?php
echo $ETI['msg_masi10consec_nuevo'];
// onchange="RevisaConsec()"
?>
</label>
<label class="Label90">
<input id="masi10consec_nuevo" name="masi10consec_nuevo" type="text" value="<?php echo $_REQUEST['masi10consec_nuevo']; ?>" class="cuatro" />
</label>
<div class="salto1px"></div>
...
<input id="titulo_1210" name="titulo_1210" type="hidden" value="<?php echo $sTituloModulo; ?>" />
<?php
if ($bBloqueTitulo) {
?>
<div class="titulos">
