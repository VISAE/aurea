...
if (isset($_REQUEST['paginaf1209']) == 0) {
	$_REQUEST['paginaf1209'] = 1;
}
if (isset($_REQUEST['lppf1209']) == 0) {
	$_REQUEST['lppf1209'] = 20;
}
if (isset($_REQUEST['boculta1209']) == 0) {
	$_REQUEST['boculta1209'] = 0;
}
// -- Inicializar variables de datos.
if (isset($_REQUEST['masi09consec']) == 0) {
	$_REQUEST['masi09consec'] = '';
}
if (isset($_REQUEST['masi09consec_nuevo']) == 0) {
	$_REQUEST['masi09consec_nuevo'] = '';
}
if (isset($_REQUEST['masi09id']) == 0) {
	$_REQUEST['masi09id'] = '';
}
if (isset($_REQUEST['masi09activa']) == 0) {
	$_REQUEST['masi09activa'] = 0;
}
if (isset($_REQUEST['masi09nombre']) == 0) {
	$_REQUEST['masi09nombre'] = '';
}
if (isset($_REQUEST['masi09cuerpo']) == 0) {
	$_REQUEST['masi09cuerpo'] = '';
}
if (isset($_REQUEST['masi09unidadfuncional']) == 0) {
	$_REQUEST['masi09unidadfuncional'] = 0;
}
if (isset($_REQUEST['masi09idescuela']) == 0) {
...
if (($_REQUEST['paso'] == 1) || ($_REQUEST['paso'] == 3)) {
	if ($_REQUEST['paso'] == 1) {
		$sSQLcondi = 'masi09consec=' . $_REQUEST['masi09consec'] . '';
	} else {
		$sSQLcondi = 'masi09id=' . $_REQUEST['masi09id'] . '';
	}
	$sSQL = 'SELECT * FROM masi09firma WHERE ' . $sSQLcondi;
	$tabla = $objDB->ejecutasql($sSQL);
	if ($objDB->nf($tabla) > 0) {
		$fila = $objDB->sf($tabla);
		$_REQUEST['masi09consec'] = $fila['masi09consec'];
		$_REQUEST['masi09id'] = $fila['masi09id'];
		$_REQUEST['masi09activa'] = $fila['masi09activa'];
		$_REQUEST['masi09nombre'] = $fila['masi09nombre'];
		$_REQUEST['masi09cuerpo'] = $fila['masi09cuerpo'];
...
if (($_REQUEST['paso'] == 10) || ($_REQUEST['paso'] == 12)) {
	$bMueveScroll = true;
	list($_REQUEST, $sError, $iTipoError, $sDebugGuardar) = f1209_db_GuardarV2($_REQUEST, $objDB, $bDebug, $idTercero);
...
if ($_REQUEST['paso'] == 93) {
	$_REQUEST['paso'] = 2;
	$_REQUEST['masi09consec_nuevo'] = numeros_validar($_REQUEST['masi09consec_nuevo']);
	if ($_REQUEST['masi09consec_nuevo'] == '') {
		$sError = $ERR['masi09consec'];
	}
	if ($sError == '') {
	...
	if ($sError == '') {
		...
		$sSQL = 'SELECT masi09id FROM masi09firma WHERE masi09consec=' . $_REQUEST['masi09consec_nuevo'] . '';
		$tabla = $objDB->ejecutasql($sSQL);
		if ($objDB->nf($tabla) > 0) {
			$sError = 'El consecutivo ' . $_REQUEST['masi09consec_nuevo'] . ' ya existe';
		}
	}
	if ($sError == '') {
		//Aplicar el cambio.
		$sSQL = 'UPDATE masi09firma SET masi09consec=' . $_REQUEST['masi09consec_nuevo'] . ' WHERE masi09id=' . $_REQUEST['masi09id'] . '';
		$tabla = $objDB->ejecutasql($sSQL);
		$sDetalle = 'Cambia el consecutivo de ' . $_REQUEST['masi09consec'] . ' a ' . $_REQUEST['masi09consec_nuevo'] . '';
		$_REQUEST['masi09consec'] = $_REQUEST['masi09consec_nuevo'];
		$_REQUEST['masi09consec_nuevo'] = '';
		seg_auditar($iCodModulo, $_SESSION['unad_id_tercero'], 8, $_REQUEST['masi09id'], $sDetalle, $objDB);
...
if ($_REQUEST['paso'] == 13) {
	$_REQUEST['paso'] = 2;
	if ($sError == '') {
		list($sError, $iTipoError, $sDebugElimina) = f1209_db_Eliminar($_REQUEST['masi09id'], $objDB, $bDebug);
...
$objCombos->nuevo('masi09activa', $_REQUEST['masi09activa'], true, $ETI['no'], 0);
...
$objCombos->sAccion = 'paginarf1209()';
$sSQL = '';
$html_blistar = $objCombos->html($sSQL, $objDB);
//$html_blistar = $objCombos->comboSistema(1209, 1, $objDB, 'paginarf1209()');
*/
if (false) {
...
$aParametros[0] = ''; //$_REQUEST['p1_1209'];
$aParametros[100] = $idTercero;
$aParametros[101] = $_REQUEST['paginaf1209'];
$aParametros[102] = $_REQUEST['lppf1209'];
//$aParametros[103] = $_REQUEST['bnombre'];
//$aParametros[104] = $_REQUEST['blistar'];
list($sTabla1209, $sDebugTabla) = f1209_TablaDetalleV2($aParametros, $objDB, $bDebug);
...
?>
	...
	function imprimelista() {
		if (window.document.frmedita.seg_6.value == 1) {
			window.document.frmlista.consulta.value = window.document.frmedita.consulta_1209.value;
			window.document.frmlista.titulos.value = window.document.frmedita.titulos_1209.value;
			window.document.frmlista.nombrearchivo.value = 'Firmas';
	...
	function imprimeexcel() {
		...
		if (sError == '') {
			asignarvariables();
			window.document.frmimpp.action = 'e1209_ss.php';
	...
	function imprimep() {
		if (window.document.frmedita.seg_5.value == 1) {
			asignarvariables();
			window.document.frmimpp.action = 'p1209.php';
			window.document.frmimpp.submit();
		} else {
	...
	function RevisaLlave() {
		let datos = new Array();
		datos[1] = window.document.frmedita.masi09consec.value;
		if ((datos[1] != '')) {
			xajax_f1209_ExisteDato(datos);
		}
	}

	function cargadato(llave1) {
		window.document.frmedita.masi09consec.value = String(llave1);
		window.document.frmedita.paso.value = 1;
		window.document.frmedita.submit();
	}

	function cargaridf1209(llave1) {
		window.document.frmedita.masi09id.value = String(llave1);
		window.document.frmedita.paso.value = 3;
		window.document.frmedita.submit();
	}

	function paginarf1209() {
		let params = new Array();
		params[99] = window.document.frmedita.debug.value;
		params[100] = <?php echo $idTercero; ?>;
		params[101] = window.document.frmedita.paginaf1209.value;
		params[102] = window.document.frmedita.lppf1209.value;
		//params[103] = window.document.frmedita.bnombre.value;
		//params[104] = window.document.frmedita.blistar.value;
		document.getElementById('div_f1209detalle').innerHTML = '<div class="GrupoCamposAyuda"><div class="MarquesinaMedia">Procesando datos, por favor espere.</div></div><input id="paginaf1209" name="paginaf1209" type="hidden" value="' + params[101] + '" /><input id="lppf1209" name="lppf1209" type="hidden" value="' + params[102] + '" />';
		xajax_f1209_HtmlTabla(params);
	}
	...
	function objinicial() {
		document.getElementById("masi09consec").focus();
	}
...
?>
<form id="frmimpp" name="frmimpp" method="post" action="p1209.php" target="_blank" style="display:none">
<input id="r" name="r" type="hidden" value="1209" />
<input id="id1209" name="id1209" type="hidden" value="<?php echo $_REQUEST['masi09id']; ?>" />
...
<?php
echo $objForma->htmlExpande(1209, $_REQUEST['boculta1209'], $ETI['bt_mostrar'], $ETI['bt_ocultar']);
$sEstiloDiv = '';
if ($_REQUEST['boculta1209'] != 0) {
	$sEstiloDiv = ' style="display:none;"';
}
?>
</div>
<div id="div_p1209"<?php echo $sEstiloDiv; ?>>
<?php
}
...
if ($_REQUEST['paso'] != 2) {
?>
<input id="masi09consec" name="masi09consec" type="text" value="<?php echo $_REQUEST['masi09consec']; ?>" onchange="RevisaLlave()" class="cuatro" />
<?php
} else {
	echo html_oculto('masi09consec', $_REQUEST['masi09consec'], formato_numero($_REQUEST['masi09consec']));
}
?>
</label>
<?php
...
<label class="Label60"<?php echo $sOcultaId; ?>>
<?php
	echo html_oculto('masi09id', $_REQUEST['masi09id'], formato_numero($_REQUEST['masi09id']));
?>
</label>
<label class="Label130">
<?php
echo $ETI['masi09activa'];
?>
</label>
<label>
<?php
echo $html_masi09activa;
?>
</label>
<label class="L">
<?php
echo $ETI['masi09nombre'];
?>

<input id="masi09nombre" name="masi09nombre" type="text" value="<?php echo $_REQUEST['masi09nombre']; ?>" maxlength="200" class="L" placeholder="<?php echo $ETI['ing_campo'] . $ETI['masi09nombre']; ?>" />
</label>
<label class="txtAreaS">
<?php
echo $ETI['masi09cuerpo'];
?>
<textarea id="masi09cuerpo" name="masi09cuerpo" placeholder="<?php echo $ETI['ing_campo'] . $ETI['masi09cuerpo']; ?>"><?php echo $_REQUEST['masi09cuerpo']; ?></textarea>
</label>
<label class="Label130">
<?php
echo $ETI['masi09unidadfuncional'];
?>
</label>
<label>
<?php
...
if ($bConExpande) {
	//Este es el cierre del div_p1209
?>
<div class="salto1px"></div>
</div>
<?php
...
<label>
<input id="bnombre" name="bnombre" type="text" value="<?php echo $_REQUEST['bnombre']; ?>" onchange="paginarf1209()" autocomplete="off" />
</label>
<label class="Label90">
<?php
...
<label class="Label160">
<?php
echo $ETI['msg_masi09consec'];
?>
</label>
<label class="Label90">
<?php
echo '<b>' . $_REQUEST['masi09consec'] . '</b>';
?>
</label>
<div class="salto1px"></div>
<label class="Label160">
<?php
echo $ETI['msg_masi09consec_nuevo'];
// onchange="RevisaConsec()"
?>
</label>
<label class="Label90">
<input id="masi09consec_nuevo" name="masi09consec_nuevo" type="text" value="<?php echo $_REQUEST['masi09consec_nuevo']; ?>" class="cuatro" />
</label>
<div class="salto1px"></div>
...
<input id="titulo_1209" name="titulo_1209" type="hidden" value="<?php echo $sTituloModulo; ?>" />
<?php
if ($bBloqueTitulo) {
?>
<div class="titulos">
