...
$xajax->register(XAJAX_FUNCTION, 'f273_EnviaFactorDeserta');
$xajax->register(XAJAX_FUNCTION, 'f273_EnviaEncuestaDes');
...
if ($_REQUEST['paso'] == 0) {
	if (isset($_GET['u']) != 0) {
		...
		if (count($aURL) < 3) {
			...
			if ($bDebug) {
				$sDebug = $sDebug . fecha_microtiempo() . ' Trayendo encuesta ' . $iMes . ' - ' . $idEncuesta . ' - ' . $idRespuesta . '<br>';
			}
			list($html_encuesta, $sDebugE) = f273_HTMLForm_Encuesta($iMes, $idEncuesta, $idRespuesta, $objDB, $bDebug);
...
<head>
	...
	<script>
		...
		function opcion_combo_aure73factordeserta() {
			let $aParametros = new Array();
			$aParametros[100] = window.document.frmencuesta.iMes.value;
			$aParametros[101] = window.document.frmencuesta.aure73id.value;
			$aParametros[102] = window.document.frmencuesta.aure73factordeserta.value;
			xajax_f273_EnviaFactorDeserta($aParametros);
		}
	</script>
</head>
