	...
	if($sError == '') {
		...
		if (count($aCodigo) < 3) {
			...
			if (!$objDB->bexistetabla($sTabla05)) {
				...
				$sSQL = 'SELECT saiu05estado, saiu05fecharespdef, saiu05horarespdef, saiu05minrespdef, saiu05respuesta FROM ' . $sTabla05 . ' WHERE saiu05id = ' . $sIdSolicitiud . ' AND saiu05numref = "' . $saui05numref . '"';
				...
				if ($objDB->nf($tabla) > 0) {
					...
					$saiu05respuesta=$fila['saiu05respuesta'];
					$sTablaAnotacion05 = 'saiu06solanotacion_' . $aCodigo[0];
					$sSQL = 'SELECT saiu06consec, saiu06anotacion, saiu06fecha, saiu06hora, saiu06minuto, saiu06idorigen, saiu06idarchivo 
	...
	*/
	...
	if ($sError==''){
		if ($saiu05tiemprespdias > 0) {
			$iFechaBase = $sContenedor . $DATA['saiu05dia'];
